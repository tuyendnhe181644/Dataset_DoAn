...
int main(void) {
    ...
    for (uint64_t i = 0; i < N; i++) {
        scanf("%"PRIu64"%"PRIu64, &reward[i].A, &reward[i].B);
    }

    qsort(reward, (size_t)N, sizeof(_reward), compare_f);
    ...
    for (uint64_t daysLeft = 1; daysLeft <= M; daysLeft++) {
        while (pos < N) {
            if (reward[pos].A <= daysLeft) {
                BHEAP_VAL_TYPE val;
        ...
        if (bHeap_GetTopVal() != NULL) {
            sum += reward[bHeap_GetTopVal()->idx].B;
        }
