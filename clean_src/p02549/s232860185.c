...
void solve(){
    ...
    seg_op=seg_op_sum;
    seg_e=seg_e_sum;
    s=seg_init(n,dp,seg_op,seg_e);
    // rep(i,0,n) printf("%lld ",seg_get(s,i));PN;
    // rep(i,0,s->_n) printf("%lld ",s->node[i]);PN;
    // printf("%lld\n",seg_prod(s,0,n-1));
    rep(i,1,n){
        rep(j,0,k){
            if(i-lr[j].a<0) continue;
            // ll val=seg_get(s,i);
            ll val=s->get(s,i);
            if(i-lr[j].b<0){
                // val+=seg_prod(s,0,i-lr[j].a);
                val+=s->prod(s,0,i-lr[j].a);
            }else{
                // val+=seg_prod(s,i-lr[j].b,i-lr[j].a);
                val+=s->prod(s,i-lr[j].b,i-lr[j].a);
            }
            // printf("%lld ",val);
            val%=MOD2;
            s->update(s,i,val);
