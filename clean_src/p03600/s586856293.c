...
// void push_strHeap(ll val, ll len , ll kind){
...
//         if(i==y) continue;
//         if(d[i]) continue;
//         if(det[y][i]>1e15) continue;
//         dfs(i,x,cost+det[y][i]);
//     }
//     d[y]=0;
//     return;
// }
...
void WarshallFloyd(ll n){
    rep(k,0,n){
        rep(i,0,n){
            ...
            rep(j,0,n){
                if(j==k) continue;
                time[i][j] = min2(time[i][j],time[i][k]+time[k][j]);
            }
        }
    }
}
void WarshallFloyd2(ll n){
    rep(k,0,n){
        rep(i,0,n){
            if(i==k) continue;
            rep(j,0,n){
...
int main(void){
    ...
    rep(i,0,n){
        rep(j,0,n){
            ...
            //     heap[cnt].y=i;
            //     heap[cnt].x=j;
            //     push_strHeap(a[i][j],cnt++,0);
            // }
        }
    }
    // rep(i,0,n) rep(j,0,n) det[i][j]=INF;
    // while(cnt){
    //     ll y=heap[0].y;
    //     ll x=heap[0].x;
    //     ll p=pop_strHeap(cnt--,0);
    //     cur=p+1;
    //     dfs(y,x,0);
    //     if(cur<p){
    //         printf("-1\n"); return 0;
    ...
    rep(i,0,n-1){
        rep(j,i+1,n){
            // if(time[i][j]>1e15) printf("-1 ");
            // else printf("%lld ",time[i][j]);
            if(time[i][j]>1e15) continue;
            ans+=time[i][j];
        }
    }
    printf("%lld\n",ans);
