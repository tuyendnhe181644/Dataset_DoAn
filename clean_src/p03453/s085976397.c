...
//頂点数、スタート、辺の情報、辺の数を引いて最短距離を返す
//辺は有向辺なので、無向なら両方の向きを入れること
//O((E+V)logV)
ll daikusutorappp[100010];
ll daikusutora222[100010];//追加
void daikusutora2(ll n,ll s,E*e,ll ecnt){
	...
	rep(i,0,n)daikusutora222[i]=i==s?1:0;//追加	
	rep(i,0,n)PQpush(i);
	while(heapN){
		ll mv=PQpop();
		for(ll t=es[mv];e[t].s==mv;t++){
			if(daikusutorappp[e[t].g]>daikusutorappp[mv]+e[t].c){
				...
				daikusutora222[e[t].g]=daikusutora222[mv];//追加
				PQchange(e[t].g);
			}else if(daikusutorappp[e[t].g]==daikusutorappp[mv]+e[t].c){//追加
				daikusutora222[e[t].g]+=daikusutora222[mv];//追加
...
//		printf("%lld %lld %lld\n",i,tcost[i],tcnt[i]);
	...
	//辺iで出会う⇔両端をx,yとして、s-x-y-tの間が真ん中
	rep(i,0,2*m){
