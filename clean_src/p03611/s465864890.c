...
int main(void) {
    ...
    qsort(a, N, sizeof(uint64_t), compare_u64_asc);

    size_t rangeSizeMax = 0;
    size_t rangeEnd = 1;
    for (size_t rangeBegin = 0; rangeBegin < N; rangeBegin++) {
        while (rangeEnd < N && a[rangeEnd] - a[rangeBegin] <= 2) {
            rangeEnd++;
        }

        if (rangeSizeMax < rangeEnd - rangeBegin) {
            rangeSizeMax = rangeEnd - rangeBegin;
        }
    }
