...
long int pow_long_int(int base, int exponent)
{
	int i=1;
...
long int convert_adic_int(char num[], unsigned int p_adic, unsigned int q_adic)
{
	...
	while(tmp>0)
	{
		answer=answer+(tmp%q_adic)*pow_long_int(10, i);
		tmp=tmp/q_adic;
		i=i+1;
	}
...
int main(void)
{
	...
	int gain[1024];
	fill_int_array(gain, 0, 1023, 0);

	int open[6][3];
	fill_int_2dim_array(6, 3, open, 0, 5, 0, 2, 0);

	int conflict[N+1];

	rep(l, 1, 1023)
	{
		fill_int_array(conflict, 0, N, 0);

		sprintf(sub_char, "%d", l);
		sub=convert_adic_int(sub_char, 10, 2);
		...
		rep(i, 1, N)
		{
			gain[l]+=P[i][conflict[i]];
		}
	}


	sort_des_int(gain, 1, 1023);
