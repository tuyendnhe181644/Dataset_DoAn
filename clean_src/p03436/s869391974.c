...
int main(void){
    ...
    for(loop1=0;loop1<H;loop1++){
        ...
        for(loop2=0;loop2<W;loop2++){
            if(in_c[loop2]=='.'){
                map_data[loop1][loop2]=INFTY;
            }else if(in_c[loop2]=='#'){
                map_data[loop1][loop2]=-1;
            }
        }
        scanf("%c",&c_temp);
    }
    ...
    for(loop1=0;loop1<1000;loop1++){
        for(loop2=0;loop2<H;loop2++){
            for(loop3=0;loop3<W;loop3++){
                if(map_data[loop2][loop3]==loop1){
                    //↑
                    if(loop2!=0){
                        if(map_data[loop2-1][loop3]==INFTY){
                            map_data[loop2-1][loop3]=loop1+1;
                            count_int[loop1+1]++;
                        }
                    }
                    //←
                    if(loop3!=0){
                        if(map_data[loop2][loop3-1]==INFTY){
                            map_data[loop2][loop3-1]=loop1+1;
                            count_int[loop1+1]++;
                        }
                    }
                    //↓
    ...
    /*for(loop1=0;loop1<H;loop1++){
        for(loop2=0;loop2<W;loop2++){
            printf("%4d",map_data[loop1][loop2]);
        }
        printf("\n");
    }*/
    
    if(map_data[H-1][W-1]==INFTY){
        printf("-1\n");
        END;
    }
    ...
    for(loop1=0;loop1<300;loop1++){
        if(count_int[loop1]>=2){
            ans+=count_int[loop1]-1;
        }
    }
    */
    ans=H*W-map_data[H-1][W-1]-1;
    
    for(loop1=0;loop1<H;loop1++){
        for(loop2=0;loop2<W;loop2++){
            if(map_data[loop1][loop2]==-1){
                ans--;
            }
        }
    }
