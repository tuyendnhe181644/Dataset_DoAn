...
ull solve(){
	...
	ull i = 0;
	ull maxloop = (lena / gcd(lena, lenb)) * lenb;
	while (i < maxloop && s[i % lena] == t[i % lenb]) i++;

	// printf("%llu\n", i);

	if (s[i % lena] > t[i % lenb]) {
		num = fullfill(n, lenb, lena);
		for (i = 0; i < num; i++) {
			printf("%s", t);
		}
		num = (n - lenb * num) / lena;
	...
	// int x = strncmp(s, t, min(lena, lenb)) 
	// if (x < -1
	// 	num = fullfill(n, lena, lenb);
	// 	for (i = 0; i < num; i++) {
	// 		printf("%s", s);
	...
	// 	} else {
	// 		ull i = 0;
	// 		while ((i == (ull)lena * (ull)lenb) && s[i % (ull)lena] == t[i % (ull)lenb]) i++;
	// 		if (s[i % lena] > t[i % lenb]) {
	// 			num = fullfill(n, lenb, lena);
	// 			for (i = 0; i < num; i++) {
	// 				printf("%s", t);
