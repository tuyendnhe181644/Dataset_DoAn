...
#define R_SIZE (300000)

static char S[R_SIZE];
static uint64_t head = R_SIZE - 1;
...
int main(void) {
    ...
    for (uint64_t i = 0; i < Q; i++) {
        ...
        if (t == 1) {
            ...
            if ((F == 1 && inverted) || (F == 2 && !inverted)) {
                S[tail] = C[0];
                tail = (tail + 1) % R_SIZE;
            }
            else {
                S[head] = C[0];
                head = (head + R_SIZE - 1) % R_SIZE;
            }
        }
    }
