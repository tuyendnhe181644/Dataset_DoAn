...
sll beg = 0, end = 0;
sll currx = 0;
sll calc (sll x, sll v) {
	return lines[v].a * x + lines[v].b;
}
sll lmin (sll x) {
	currx = x;
	while (beg + 1 < end) {
		if (calc(x, beg) >= calc(x, beg + 1)) {
			beg++;
		} else {
			break;
		}
...
bool isvalid (hwll v1, hwll v2, hwll v3) {
	return (v3.b - v2.b) * (v2.a - v1.a) < (v2.b - v1.b) * (v3.a - v2.a);
}
void ladd (sll a, sll b) {
	lines[end++] = (hwll){a, b};
	while (beg + 3 <= end) {
		if (!isvalid(lines[end - 3], lines[end - 2], lines[end - 1])) {
			lines[end - 2] = lines[end - 1];
			end--;
		} else {
			break;
		}
...
ull solve () {
	...
	for (i = 0; i < n; i++) {
		if (i) {
			dp[i] = lmin(a[i]) + a[i] * a[i] + (sll)k;
		} else {
			dp[i] = 0;
		}
		ladd(-2 * a[i], a[i] * a[i] + dp[i]);
		// printf("%lld\n", dp[i]);
	}
	result = dp[n - 1];
