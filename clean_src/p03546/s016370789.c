...
int main(void){
    ...
    int change_data[10][10];
    for(loop1=0;loop1<=9;loop1++){
        for(loop2=0;loop2<=9;loop2++){
            scanf("%d",&change_data[loop1][loop2]);
        }
    }
    
    //各変換に対して最適ルートを探索したい… [どうやって？]
    //「n」を「1」に変換するにはコストがこれだけかかりますよとリスト化したい
    //確実に到達できることがわかりきった重み付き有向グラフ？
    //重みが全て正、これはすなわち
    //ダイクストラ法を使えと
    ...
    bool kakutei[10]={0};
    long last_kakutei=1;
    format_array(10,cost,99999);
    cost[1]=0; //出発地点をcost0とできる
    kakutei[1]=1;
    ...
    while(1){
        ...
        //今、確定したところから直接繋がってる点までのcostを計算
        //ただし、未確定に対してのみ行う
        for(loop1=0;loop1<=9;loop1++){
            if(kakutei[loop1]==0){
                cost[loop1]=sel_min(cost[loop1],cost[last_kakutei]+change_data[loop1][last_kakutei]);
            }
        }
        //未確定の中での最小のやつを確定
        long kouho;
        long kouho_cost=INFTY;
        for(loop1=0;loop1<=9;loop1++){
            if(kakutei[loop1]==0){
                if(cost[loop1]<kouho_cost){
                    kouho_cost=cost[loop1];
                    kouho=loop1;
                }
            }
        }
