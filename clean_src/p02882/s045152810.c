...
int main(void) {
    ...
    scanf("%"PRIu64"%"PRIu64"%"PRIu64, &a, &b, &x);

    if (2 * x <= a * a * b) {
        const double r = (double)(2 * x) / (double)(a * b);

        printf("%.10lf\n", atan((double)b / r) * 180 / PI);

    }
    else {
        const double r = (double)(2 * (a * a * b - x)) / (double)(a * a);

        printf("%.10lf\n", atan(r / (double)a) * 180 / PI);
    }
