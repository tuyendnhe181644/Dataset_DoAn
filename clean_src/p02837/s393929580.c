...
static uint64_t A[15];
static uint64_t x[15][14], y[15][14];
...
int main(void) {
    ...
    for (uint64_t i = 0; i < N; i++) {
        ...
        for (uint64_t j = 0; j < A[i]; j++) {
            scanf("%"PRIu64"%"PRIu64, &x[i][j], &y[i][j]);
            x[i][j]--;
        }
    }

    const uint64_t mx = (UINT64_C(0x1) << N) - 1;
    uint64_t maxH = 0;

    for (uint64_t bf = 0; bf <= mx; bf++) {
        bool matched = true;
        for (uint64_t p = 0; p < N && matched; p++) {
            if (((bf >> p) & UINT64_C(0x1)) != 0) {
                for (uint64_t ai = 0; ai < A[p]; ai++) {
                    if (y[p][ai] == 0) {
                        if (((bf >> x[p][ai])& UINT64_C(0x1)) != 0) {
                            matched = false;
                            break;
                        }
                    }
        ...
        if (matched) {
            const uint64_t pCnt = popcntU64(bf);
