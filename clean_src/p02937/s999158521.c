...
ll cnt[26]={},al[26][100000];

int main(void){
    // fgets(s,sizeof(s),stdin);
    rep(i,0,26) rep(j,0,1e5) al[i][j]=INF;
    char s[1151154];
    char t[1151154];
    ll ns,nt;
    ll ans=0;
    ...
    ll tmp=lower_bound(al[now],0,1e5,0,2);
    if(nt==1){
        printf("%lld\n",al[now][tmp]+1); return 0;
    }
    p=al[now][tmp];
    ans+=p+1;
    rep(i,1,nt){
        ...
        ll u=lower_bound(al[now],0,1e5,p+1,2);
        // printf("%lld %lld ",p,u);
        if(al[now][u]>1e8){
            ans+=ns-p-1;
            ans+=al[now][0]+1;
