...
long long repdp(long long v,long long bk,rs g[],mkj x[]){
  ...
  if(bk!=-1){
    ...
    for(i=x[v].st;i<x[v].st+x[v].kz;i++){
      ...
      ne=g[i].id;
      dc[ne]=nc-sc[p];
      dp[ne]=(np*modular_inverse(calcnCr(nc-1,sc[p])))%mod;
      dp[ne]=(dp[ne]*modular_inverse(sp[p]))%mod;
      calcfl[ne]=1;
    }
  }
