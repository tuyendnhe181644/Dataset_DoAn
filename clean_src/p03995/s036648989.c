...
ull solve () {
	...
	for (i = 0; i < n; i++) {
		if (!check1[b[i]] || !check2[a[i]]) {
			min1 = c1[b[i]] = 0;
			min2 = c2[a[i]] = c[i];
			if (!dfs1(b[i]) || !dfs2(a[i])) goto fail;
			if (min1 + min2 < 0) goto fail;
		}
	}

	// sll downx = (glen1 ? c[g1[0].a] - c1[g1[0].a] + min1 : VERYBIG);
	// sll downy = (glen1 ? c[g1[0].a] - c2[g1[0].a] + min2 : VERYBIG);
	// sll minv = c[g1[0].a] - downx - downy;
	// for (i = 0; i < n; i++) {
	// 	// printf("%lld+%lld.\n", c1[b[i]], c2[a[i]]);
	// 	if (check1[b[i]] || check2[a[i]]) {
	// 		if (c1[b[i]] + c2[a[i]] != c[i]) {
	// 			// fprintf(stderr, "WTF\n");
	// 			abort();
	// 			goto fail;
	// 		}
	// 		// if (c2[a[i]] + min1 < 0) goto fail;
	// 		// if (c1[b[i]] + min2 < 0) goto fail;
	// 	} else {
	// 		if (-min1 > min2 + c[i]) goto fail;
	// 	}
	// }
