...
typedef struct { int id; ll k1, k2; } QUE;
QUE que[24][QSIZE+5];
int qsize[24];
...
void enq(int qid, int id, ll k1, ll k2)
{
	int i, max;
	QUE qt;
...
int main() 
{
	...
	genperm();
	for (i = 0; i < N; i++) {
		for (j = 0; j < K; j++) f[j] = in();
		for (j = 0; j < sz[K]; j++) {
			tbl[i][j].k1 = f[idx[j][0]];
			if (K >= 2) tbl[i][j].k1 = (tbl[i][j].k1 << 17) | f[idx[j][1]];
			if (K >= 3) {
				tbl[i][j].k2 = f[idx[j][2]];
				if (K == 4) tbl[i][j].k2 = (tbl[i][j].k2 << 17) | f[idx[j][3]];
			} else tbl[i][j].k2 = 0;
		}
	}
	...
	while (done < N) {
		while (top < end) {
			a = q[top++];
			for (i = 0; i < sz[K]; i++) enq(i, a, tbl[a][i].k1, tbl[a][i].k2);
		}
