...
int main(){
	unsigned char ubW_D_ring_text[STRING_MAX]; 	
	unsigned char ubW_D_look_text[STRING_MAX]; 	
	unsigned short usW_D_ring_length;   	
	unsigned short usW_D_look_length;   	
	unsigned short usW_F_frg_main = FRG_OFF;  	
...
unsigned short s_sarch_text(unsigned char * ring, unsigned short r_length, unsigned char * look, unsigned short l_length)
{
	unsigned char twi_ring[STRING_MAX * TEICE];
	unsigned short usW_F_frg_fanc = FRG_OFF;
	unsigned short usW_C_i, usW_C_j;
	...
	for(usW_C_i = LOOP_RESET;usW_C_i < l_length;usW_C_i++){
