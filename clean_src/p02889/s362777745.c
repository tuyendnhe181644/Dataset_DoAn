...
#define the_number_of_vertex 515
#define the_number_of_edge 25154
typedef ll type_gr;
...
ll cnt[315][315];
// char buf[1800000];
// #define rd(x)x=0;for(;;){k=buf[sz++];if(k<'0')break;x=x*10+k-'0';}
void cin(ll *n){ 
    scanf("%lld",&(*n)); 
}
...
void solve(){
    ...
    cin(&l);
    // scanf("%s",s);
    rep(i,0,n) rep(j,0,n) tim[i][j]=INF;
    rep(i,0,n) tim[i][i]=0;
    ll a,b,c;
    rep(i,0,m){
        cin(&a);
    ...
    rep(i,0,n){
        dijkstra(i);
        rep(j,0,n){
            if(ans[j]==INF) cnt[i][j]=-1;
            else cnt[i][j]=ans[j]-1;
        }
    }
    ll q;
    cin(&q);
