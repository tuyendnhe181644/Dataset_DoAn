...
int main(void){
  ...
  c=1;
  for(i=1;i<m;i++){
    if(dat[c-1].st==dat[i].st && dat[c-1].fi==dat[i].fi){
      dat[c-1].val+=dat[i].val;
    }
    else{
      dat[c]=dat[i];
      c++;
  ...
  for(i=1;i<=n;i++){
    while(dat[t].fi==i){
      //printf("[%lld %lld] + %lld\n",dat[t].st,i,dat[t].val);
      addquery(dat[t].st,i+1,dat[t].val,0,0,ssize);
      t++;
    }
    l=maxquery(0,ssize,0,0,ssize);
    upd(i+1,l);
  }
