...
/*
signed compair(const void *_a, count void *_b){
	pair a = *(pair *)_a, b = *(pair *)_b;
}
*/
signed main(){
	int N, M, i, j;
	...
	pair **rs = (pair **)malloc(sizeof(pair *) * (N + 1));
	for(i = 0; i <= N; i++){
		count[i] = 0;
	}
	for(i = 0; i < M; i++){
	...
	for(i = 0; i <= N; i++){
		rs[i] = (pair *)malloc(sizeof(pair) * count[i]);
		count[i] = 0;
	}
	for(i = 0; i < M; i++){
		rs[r[i]][count[r[i]]] = (pair){l[i], a[i]};
		count[r[i]]++;
	}
	segment_tree *t = make_segment_tree(-inf, 0, max, sum, rep_max);
	update(0, 1, -inf, 0, t);
	for(i = 1; i <= N; i++){
		update(i, i + 1, -inf, query(0, i, t), t);
		for(j = 0; j < count[i]; j++){
			update(rs[i][j].l, i, rs[i][j].a, -inf, t);
			update(i, i + 1, rs[i][j].a, -inf, t);
		}
	}
	printf("%lld\n", query(0, N + 1, t));
	return 0;
