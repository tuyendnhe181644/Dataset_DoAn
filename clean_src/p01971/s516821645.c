...
int main(){
	...
	int *sub_tree_num = (int *)malloc(sizeof(int) * N);
	for(i = 0; i < N; i++){
		sub_tree_num[i] = 1;
	}
	for(i = N - 1; i > 0; i--){
		sub_tree_num[p[i]] += sub_tree_num[i];
	}
/*	printf("sub_tree_num:\n");
	for(i = 0; i < N; i++){
		printf("%d  ", sub_tree_num[i]);
	}
	printf("\n");
*/	int *vp = (int *)malloc(sizeof(int) * N);
	int *vp_sub = (int *)malloc(sizeof(int) * N);
	vp[0] = 0;
	vp_sub[0] = 1;
	for(i = 1; i < N; i++){
		vp[i] = vp_sub[p[i]];
		vp_sub[i] = vp[i] + 1;
		vp_sub[p[i]] += sub_tree_num[i];
	}
/*	printf("vp:\n");
	for(i = 0; i < N; i++){
		printf("%d  ", vp[i]);
...
*/
	...
	for(i = 0; i < N; i++){
		update(vp[i], vp[i] + 1, 0, c[i], t);
	}
	int G_num;
	for(i = 0; i < Q; i++){
//		printf("i = %d\n", i);
		update(vp[v[i]], vp[v[i]] + sub_tree_num[v[i]], -1, 1, t);
		G_num = query(0, N, t);
//		printf("G_num = %d\n", G_num);
		if(G_num > N / 2){
			printf("broccoli\n");
