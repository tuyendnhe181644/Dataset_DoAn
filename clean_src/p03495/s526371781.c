...
static ValInfo vi[200001];

static int compare_f(const void* a, const void* b) {
    return ((ValInfo*)a)->num < ((ValInfo*)b)->num ? -1 : 1;
}
...
int main(void) {
    ...
    qsort(vi, (size_t)N + 1, sizeof(ValInfo), compare_f);

    uint64_t modSortCnt = 0;
    uint64_t modCnt = 0;

    for (uint64_t i = 0; i <= N && K < sortCnt - modSortCnt; i++) {
        if (0 < vi[i].num) {
            modSortCnt++;
            modCnt += vi[i].num;
        }
