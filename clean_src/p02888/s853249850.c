...
static uint64_t L[2000];

#define INVV (UINT64_C(100000))

static uint64_t searchUB(const uint64_t base, const uint64_t size, const uint64_t ub) {
    if (size == 1) {
        return ub < L[base] ? INVV : base;
    }

    const uint64_t c = base + size / 2;

    if (L[c] <= ub) {
        if (1 < base + size - c) {
            const uint64_t rr = searchUB(c + 1, base + size - c - 1, ub);
        ...
        if (1 <= c - base) {
            const uint64_t lr = searchUB(base, c - base, ub);
...
int main(void) {
    ...
    qsort(L, (size_t)N, sizeof(uint64_t), compare_u64_asc);

    uint64_t sum = 0;
    for (uint64_t i = 0; i < N - 2; i++) {
        for (uint64_t j = i + 1; j < N - 1; j++) {
            const uint64_t ub = searchUB(j + 1, N - j - 1, MIN(L[i] + L[j] - 1, 1000));
