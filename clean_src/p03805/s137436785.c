...
int main(void)
{
	...
	fill_int_array(N, N, adjacency_matrix, 0, N-1, 0, N-1, 0);

	rep(i, 0, M-1)
	{
		adjacency_matrix[a[i]-1][b[i]-1]=adjacency_matrix[a[i]-1][b[i]-1]+1;
		adjacency_matrix[b[i]-1][a[i]-1]=adjacency_matrix[b[i]-1][a[i]-1]+1;
	}

	rep(i, 1, N-1)
	{
		answer=answer+graph_number_of_hamilton_path_sub(N, adjacency_matrix, 0, i);
	}
