...
ull solve () {
	...
	ull totalplus = bsum - asum;

	if (plus1 > totalplus || plus2 > totalplus) goto fail;

	ull addition2 = totalplus - plus2;
	ull addition1 = totalplus - plus1;

	// printf("%llu vs %llu\n", addition2, addition1);
	if (addition2 * 2 == addition1) {
		goto success;
	} else {
		goto fail;
	}
