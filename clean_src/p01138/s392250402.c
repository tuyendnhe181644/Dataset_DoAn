...
int main(void){
    ...
    while(1){
        ...
        rep(i,0,n){
            scanf("%s",s);
            scanf("%s",t);
            ll start= ((s[0]-'0')*10+s[1]-'0')*60*60 + ((s[3]-'0')*10+s[4]-'0')*60 + (s[6]-'0')*10+s[7]-'0';
            ll fin= ((t[0]-'0')*10+t[1]-'0')*60*60 + ((t[3]-'0')*10+t[4]-'0')*60 + (t[6]-'0')*10+t[7]-'0';
            rep(j,start,fin) time[j]++;
        }
        ll ans=0;
        rep(i,0,24*60*60) ans=max2(ans,time[i]);
        printf("%lld\n",ans);
    }
