...
typedef struct { int t, x, p; } TBL;
TBL tbl[3003];

int cmp(TBL *a, TBL *b) { return a->t - b->t; }
...
int main()
{
	...
	for (i = 0; i < n; i++) tbl[i].x = in(), tbl[i].t = in(), tbl[i].p = in();
	qsort(tbl, n, sizeof(TBL), cmp);
	...
	for (i = 0; i < n; i++) {
		if (ABS(tbl[i].x - xl) <= tbl[i].t * v) add_edge(a, i<<1, 1, 0);
		if (ABS(tbl[i].x - xr) <= tbl[i].t * v) add_edge(b, i<<1, 1, 0);
		add_edge((i<<1)+1, sink, 1, 0);
		add_edge(i<<1, (i<<1)+1, 1, -tbl[i].p);
		for (j = i+1; j < n; j++) {
			if ((tbl[j].t-tbl[i].t)*v >= ABS(tbl[i].x-tbl[j].x)) add_edge((i<<1)+1, j<<1, 1, 0);
		}
	}
