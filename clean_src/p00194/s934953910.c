...
int getHV(char *p, int *h)
{
	int k;
	*h = *p - 'a' + 1, p += 2;
	k = *p++ & 0xf;
	if (*p >= '0') k = (k<<3) + (k<<1) + (*p & 0xf);
	return k;
}
...
int main()
{
	...
	while (scanf("%d%d", &H, &V) && H > 0) {
		...
		scanf("%d%d", &D, &n); while (n-- > 0) {
			scanf("%s%d", b1, &k);
			v1 = getHV(b1, &h1), light[h1][v1] = k;
		}
		scanf("%d", &n); while (n-- > 0) {
			scanf("%s%s", b1, b2);
			v1 = getHV(b1, &h1), v2 = getHV(b2, &h2);
		...
		scanf("%s%s", b1, b2);
		s[1] = getHV(b1, s), g[1] = getHV(b2, g);	// start, goal

		top = end = Q;
		end->h = s[0], end->v = s[1], end->t = 0, end->d = E, end++;
		while (top < end) {
			h1 = top->h, v1 = top->v, t1 = top->t, d1 = top->d, top++;
//			printf("(%d,%d), t %d, d %d\n", h1, v1, t1, d1);
			for (k = 0; k < 4; k++) {
				...
				time[h2][v2][t2][d2] = 1;
				end->h = h2, end->v = v2, end->t = t2, end->d = d2, end++;
			}
		}
