...
static bool isSorted(const uint64_t N, const uint64_t* num) {
...
int main(void) {
    ...
    bool isSortable = isSorted(N, p);
    for (uint64_t i = 0; i < N-1 && !isSortable; i++) {
        for (uint64_t j = i + 1; j < N && !isSortable; j++) {
            uint64_t tmp = p[i];
            p[i] = p[j];
            p[j] = tmp;
