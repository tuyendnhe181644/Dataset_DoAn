...
int main(void) {
    ...
    uint64_t r = M * powU64WMod(M - 1, N - 1, MOD_VAL) % MOD_VAL;
    
    uint64_t combL[2] = { 1,1 };

    for (uint64_t i = 1; i <= K; i++) {
        combL[0] = combL[0] * (N - i) % MOD_VAL;
        combL[1] = combL[1] * i % MOD_VAL;

        r = (r + (((combL[0] * modinvU64(combL[1], MOD_VAL) % MOD_VAL) % MOD_VAL) * M % MOD_VAL) * powU64WMod(M - 1, N - i - 1, MOD_VAL) % MOD_VAL) % MOD_VAL;
    }
