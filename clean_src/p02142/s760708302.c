...
//辺の情報を個別に持つタイプ
typedef struct edge{ll s,g,a,b;}E;
...
void readgraph(){
	//適宜変える
	ll n,m;
	scanf("%lld%lld%lld%lld",&n,&m,&start,&goal);
	start--,goal--;
	rep(i,0,m){
		ll x,y,a,b;
...
ll calc(ll t,ll a,ll b){
	ll l=max(t,sqrt(b)-a-3);
	ll ans=INF;
	rep(i,l,l+5)ans=min(ans,i+(b+i+a-1)/(i+a));
	return ans;
}
...
void daikusutora2(ll s){
	...
	while(heapN){
		...
		for(ll t=g.id[mv];t<g.ecnt&&g.e[t].s==mv;t++){
			ll time=calc(daikusutorappp[mv],g.e[t].a,g.e[t].b);
			if(daikusutorappp[g.e[t].g]>time){
				daikusutorappp[g.e[t].g]=time;
...
int main(){
	readgraph();
	daikusutora2(start);
	printf("%lld\n",daikusutorappp[goal]==INF?-1:daikusutorappp[goal]);
}
