...
static uint64_t getNumE(uint64_t v) {
    uint64_t cnt = 0;
...
#define THR (UINT64_C(1000000000000000000))

static void u128_Mul(uint64_t* const c, const uint64_t* a, const uint64_t b) {
    uint64_t r[2] = { 0,0 };

    r[1] = a[1] * b;
    r[1] += (a[0] >> 32) * (b >> 32);
    r[1] += ((a[0] >> 32) * (b & UINT64_C(0xFFFFFFFF))) >> 32;
    r[1] += ((a[0] & UINT64_C(0xFFFFFFFF)) * (b >> 32)) >> 32;
    r[0] = (a[0] & UINT64_C(0xFFFFFFFF)) * (b & UINT64_C(0xFFFFFFFF));
    if (r[0] + (((a[0] >> 32) * (b & UINT64_C(0xFFFFFFFF))) << 32) < r[0]) {
        r[1]++;
    }
    r[0] += ((a[0] >> 32) * (b & UINT64_C(0xFFFFFFFF))) << 32;
    if (r[0] + (((a[0] & UINT64_C(0xFFFFFFFF)) * (b >> 32)) << 32) < r[0]) {
        r[1]++;
    }
    r[0] += ((a[0] & UINT64_C(0xFFFFFFFF)) * (b >> 32)) << 32;
