...
int main(void){
  ...
  long long dp[524288],nx[524288];
  long long doubling[64][262144];
  long long res[524288],rp;
  bool fl[524288]={0};
  scanf("%lld%lld",&n,&k);
  //fprintf(stderr,"ok\n");
  for(i=0;i<n;i++){
  ...
  for(i=1;i<64;i++){
    for(j=0;j<n;j++){
      ...
      if(doubling[i][j]>=llinf){
        doubling[i][j]=llinf;
      }
      //fprintf(stderr,"[%lld]",doubling[i][j]);
    }//fprintf(stderr,"\n");
  }
  //fprintf(stderr,"ok\n");
  w=0;
  for(i=63;i>=0;i--){
    if((w+doubling[i][w%n])>n*k){continue;}
    w+=doubling[i][w%n];
  }
  rp=0;
  for(i=w;i<n*k;i++){
