...
void solve(){
    ...
    rep(i,0,n){
        ...
        while(1){
            if(cnt==k){
                // printf("%lld %lld\n",i,sc[cnt]);
                ans=max2(ans,sc[cnt]); break;
            }
            if(d[cur]){
                ll rest=k-cnt;
                ll cy=cnt-d[cur]+1;
                ll cycle_cost= sc[cnt]-sc[ d[cur]-1 ];
                ll st=d[cur]-1;
                ll tmp= sc[cnt];
                if(cycle_cost>=0){
                    tmp = sc[cnt] + (rest/cy)*cycle_cost;
                }
                // printf("%lld\n",cur);
                ...
                // }PN;
                // rep(j, 0 , cy%rest + 1  ){
                //     ll score = tmp + (sc[st+ j ]-sc[st]);
                //     ans=max2(ans,score);
                //     // printf("%lld %lld\n",i,score);
                // }
                rep(j, 0 , cy+1  ){
                    ll score = tmp + (sc[st+ j ]-sc[st]);
                    if(j>rest%cy && (sc[cnt]-sc[st]) ){
                        score-= (sc[cnt]-sc[st]);
                    }
                    ans=max2(ans,score);
                    // printf("%lld %lld\n",i,score);
            ...
            d[cur]=cnt+1;
            cur = p[cur];
            sc[cnt+1] = sc[cnt]+c[cur];
            // printf("%lld %lld\n",cur,sc[cnt+1]);
            ans=max2(ans,sc[cnt+1]);
            cnt++;
        }
        // PN;
        rep(j,0,cnt+1){
