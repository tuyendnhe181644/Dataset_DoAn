...
static int64_t P[5000], C[5000], cT[5000], cSumA[5000], cSecMax[5001], cChecked[5000];
        ...
        if (0 < cSize) {
            ...
            for (int64_t i = 0; i < cSize; i++) {
                if (i == 0) {
                    ...
                    cSumA[i] = cSumA[i - 1] + cT[i];
                }

                //printf("s %"PRId64"\n", cSumA[i]);
            }

            for (int64_t k = 1; k <= cSize; k++) {
                for (int64_t i = 0; i < cSize; i++) {
                    const int64_t l = (i + cSize - 1) % cSize;
                    const int64_t r = (i + k - 1) % cSize;
                    if (l < r) {
                        cSecMax[k] = MAX(cSecMax[k], cSumA[r] - cSumA[l]);
                    }
                    else if (l == r) {
                        cSecMax[k] = MAX(cSecMax[k], cSumA[cSize - 1]);
                    }
                    else {
                        cSecMax[k] = MAX(cSecMax[k], cSumA[cSize - 1] - cSumA[l] + cSumA[r]);
                    }
                }
            ...
            int64_t cSecMaxMax = INT64_MIN;

            if (K < cSize) {
                for (int64_t k = 1; k <= K; k++) {
                    cSecMaxMax = MAX(cSecMaxMax, cSecMax[k]);
                }
            }
            else {
                if (cSum <= 0) {
                    ...
                    if (K % cSize == 0) {
                        ...
                        cSecMaxMax += cSum * ((K / cSize) - 1);
                    }
                    else {
                        cSecMaxMax = 0;
                        for (int64_t k = 1; k <= K % cSize; k++) {
                            cSecMaxMax = MAX(cSecMaxMax, cSecMax[k]);
                        }

                        cSecMaxMax += cSum * (K / cSize);
                    }
                }
            }
