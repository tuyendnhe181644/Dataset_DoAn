...
#define TREE_NODE_PARENT_NONE ((size_t)1000000000)

static void tree_Init(const uint64_t N) {
    for (uint64_t i = 0; i < N; i++) {
        tree_Node[i].cnt = 0;
        tree_Node[i].parent = TREE_NODE_PARENT_NONE;
...
typedef struct {
    ...
    size_t* nextNode;
    size_t numNextNodes;
    size_t maxNumNextNodes;
    size_t* edgeIdx;
    uint64_t color;
} NW_Node;

#define NW_INIT_ALLOC (128)

static NW_Node nw_Node[100000];

#define NW_NODE_PARENT_NONE ((size_t)1000000000)

static void nw_Init(const uint64_t N) {
    for (uint64_t i = 0; i < N; i++) {
        nw_Node[i].cnt = 0;
        nw_Node[i].parent = NW_NODE_PARENT_NONE;
        nw_Node[i].color = 0;
        nw_Node[i].numNextNodes = 0;
        nw_Node[i].nextNode = malloc(sizeof(size_t) * NW_INIT_ALLOC);
        nw_Node[i].edgeIdx = malloc(sizeof(size_t) * NW_INIT_ALLOC);
        nw_Node[i].maxNumNextNodes = NW_INIT_ALLOC;
    }
}

static void nw_AddEdge(const size_t *node, const size_t edgeIdx) {
    for (size_t i = 0; i < 2; i++) {
        if (nw_Node[node[i]].maxNumNextNodes <= nw_Node[node[i]].numNextNodes) {
            nw_Node[node[i]].nextNode = realloc(nw_Node[node[i]].nextNode, sizeof(size_t) * nw_Node[node[i]].maxNumNextNodes * 2);
            nw_Node[node[i]].edgeIdx = realloc(nw_Node[node[i]].edgeIdx, sizeof(size_t) * nw_Node[node[i]].maxNumNextNodes * 2);
            nw_Node[node[i]].maxNumNextNodes *= 2;
        }

        nw_Node[node[i]].nextNode[nw_Node[node[i]].numNextNodes] = node[1 - i];
        nw_Node[node[i]].edgeIdx[nw_Node[node[i]].numNextNodes] = edgeIdx;
        nw_Node[node[i]].numNextNodes++;
    }
}

static uint64_t edgeColor[100000];

static uint64_t coloring(const size_t tNode, const size_t parent) {
    uint64_t color = 1;

    uint64_t KMax = 0;

    for (uint64_t i = 0; i < nw_Node[tNode].numNextNodes; i++) {
        if (nw_Node[tNode].nextNode[i] != parent) {
            if (color == nw_Node[tNode].color) {
                color++;
            }
            nw_Node[nw_Node[tNode].nextNode[i]].color = color;
            edgeColor[nw_Node[tNode].edgeIdx[i]] = color;

            if (KMax < color) {
                KMax = color;
            }

            const uint64_t K = coloring(nw_Node[tNode].nextNode[i], tNode);
...
int main(void) {
    ...
    for (uint64_t i = 0; i < N - 1; i++) {
        ...
        nw_AddEdge((size_t[]) { a, b }, (size_t)i);
    }

    const uint64_t K = coloring(0, NW_NODE_PARENT_NONE);
