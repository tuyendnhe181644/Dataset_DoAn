...
int cmp1( const void *p, const void *q ) {
    if( ((fr*)p) ->a == ((fr*)q)->a ) return ((fr*)q) ->b - ((fr*)p)->b; 
...
ll ans=0,head=0,tail=0,n,minw=INF;
fr nap[105];
ll place[4];
// int used[105];


void dfs(ll piv, ll cur_s, ll cur_w, ll rest){
    // printf("%lld %lld %lld %lld\n",piv,cur_s,cur_w,rest);
    // printf("%lld\n",cur_s);
    if(rest<cur_w){
        ans=max2(ans,cur_s);
    }else{
        rep(i,cur_w,minw+4){
            if(rest<i){
                ...
                ll tmp;
                if(i==cur_w) tmp=piv;
                else tmp=place[i-minw];
                // printf("tmp:%lld\n",tmp);
                if(tmp==-1){
                    ans=max2(ans,cur_s);
                }else if(tmp+1==n){
                    ans=max2(ans,cur_s+nap[tmp].b);
                }else{
                    dfs(tmp+1,cur_s+nap[tmp].b,nap[tmp+1].a,rest-nap[tmp].a);
                }                
            }
        }
    }
...
int main(void){
    ...
    place[0]=0;
    rep(i,1,n){
        if(nap[i-1].a!=nap[i].a){
            place[nap[i].a-minw]=i;
        }
    }
    // rep(i,0,4) printf("%lld ",place[i]);
    // PN;
    // printf("%lld\n",minw);
    dfs(0,0,minw,w);
