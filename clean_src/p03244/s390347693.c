...
int main(void) {
    ...
    qsort(c[0], 100000, sizeof(CC), compare_f);
    qsort(c[1], 100000, sizeof(CC), compare_f);

    uint64_t sum = 0;

    if (c[0][0].val != c[1][0].val) {
        sum = n - c[0][0].cnt - c[1][0].cnt;
    }
    else {
        sum = n - MAX(c[0][0].cnt + c[1][1].cnt, c[0][1].cnt + c[1][0].cnt);
    }
