...
sll solve () {
	...
	for (i = ulen - 1; i >= 0; i--) {
		...
		for (j = 0; j <= ulen; j++) {
			dp[i][j] = dq[i + 1][j - 1] * us[i] % MOD;

			dq[i][j] = (dq[i + 1][j] + dp[i][j]) % MOD;
	...
	for (i = 0; i < ulen; i++) {
		result += (us[i] - 1) * bitpow(n - 1, k - 1, MOD) % MOD;

		for (j = 2; j <= ulen; j++) {
			result += bitpow(n - 1, k - j, MOD) * (dp[i][j] * frac[j - 1] % MOD) % MOD;
		}
		result %= MOD;
	}
	...
	result = (MOD + (n * N1K % MOD) - result) % MOD;
