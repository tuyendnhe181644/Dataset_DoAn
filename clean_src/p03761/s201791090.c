...
int main(void){
    ...
    for(loop1=0;loop1<how_data;loop1++){
        scanf("%s",data[loop1]);
        for(loop2=0;loop2<strlen(data[loop1]);loop2++){
            how_ch[loop1][data[loop1][loop2]-97]++;
        }
    }
    int set[26]={0};
    for(loop1=0;loop1<26;loop1++){
        set[loop1]=how_ch[0][loop1];
        for(loop2=0;loop2<how_data;loop2++){
            if(set[loop1]>how_ch[loop2][loop1]){
                set[loop1]=how_ch[loop2][loop1];
            }
        }
    }
    for(loop1=0;loop1<26;loop1++){
        for(loop2=0;loop2<set[loop1];loop2++){
            printf("%c",97+loop1);
        }
    }
    END;
