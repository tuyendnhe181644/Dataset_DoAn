...
ull rank[N_MAX + M_MAX];
ull parent[N_MAX + M_MAX];
...
ull solve () {
	...
	for (i = 0; i < n; i++) {
		scanf("%llu", &langnum[i]);
		langindex[i] = (i ? (langindex[i - 1] + langnum[i - 1]) : 0);

		for (j = 0; j < langnum[i]; j++) {
			ull x;
			scanf("%llu", &x);
			x--;
			l[langindex[i] + j] = x;
	...
	const ull lsum = langindex[n - 1] + langnum[n - 1];

	j = 0;
	for (i = 0; i < lsum; i++) {
		if (j < n - 1 && i >= langindex[j + 1]) j++;

		arr[i].a = l[i];
		arr[i].b = j;
		// printf("%llu: %llu/%llu\n", i, l[i], j);
	}
	qsort(arr, lsum, sizeof(hwll), phwllABcomp);

	uf_init(n);
	// printf("%lld: %llu/%llu\n", 0ULL, arr[0].a, arr[0].b);
	for (i = 1; i < lsum; i++) {
		// printf("%llu: %llu/%llu\n", i, arr[i].a, arr[i].b);
		if (arr[i].a == arr[i - 1].a) {
			// printf("%llu and %lld united\n", arr[i].b, arr[i - 1].b);
			uf_union(arr[i].b, arr[i - 1].b);
		}
	}
	...
	for (i = 1; i < n; i++) {
		// printf("%lld: %llu\n", i, uf_find(i));
		if (uf_find(i) != uf_find(i - 1)) goto fail;
	}
	goto success;
	...
	// for (i = 1; i < n; i++) {
	// 	if (uf_find(i - 1) != uf_find(i)) goto fail;
	// }
	// goto success;
