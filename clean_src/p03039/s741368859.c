...
static int siRow, siCol;										// 行・列
static int siKCnt;												// 駒数
...
)
{
	...
	sscanf(lc1Buf, "%d%d%d", &siRow, &siCol, &siKCnt);
	siMCnt = siRow * siCol;

	// 階乗リスト - 作成
	fMakeModFact(siMCnt);
	fMakeModFactR(siMCnt);

	// ２駒の配置回数
	int liTCnt = fGetnCk(siMCnt - 2, siKCnt - 2);

	// 総和 - 横方向
	int liSum = 0;
	int liXCnt = fGetModMulti(liTCnt, siRow);
	liXCnt = fGetModMulti(liXCnt, siRow);
	for (i = 0; i < siCol; i++) {
		int liCost = i * 2 - siCol + 1;
		liCost = fGetModMulti(liCost, i);
		liCost = fGetModMulti(liCost, liXCnt);
		liSum = fGetModSum(liSum, liCost);
	}

	// 総和 - 縦方向
	int liYCnt = fGetModMulti(liTCnt, siCol);
	liYCnt = fGetModMulti(liYCnt, siCol);
	for (i = 0; i < siRow; i++) {
		int liCost = i * 2 - siRow + 1;
		liCost = fGetModMulti(liCost, i);
		liCost = fGetModMulti(liCost, liYCnt);
		liSum = fGetModSum(liSum, liCost);
	}
