...
static uint64_t A[100000], B[100000], C[100000];
static uint64_t sum[100000];
static uint64_t sumSum[100000];
...
int main(void) {
    ...
    qsort(A, (size_t)N, sizeof(uint64_t), compare_u64_asc);
    qsort(B, (size_t)N, sizeof(uint64_t), compare_u64_asc);
    qsort(C, (size_t)N, sizeof(uint64_t), compare_u64_asc);

    uint64_t pos = 0;

    for (uint64_t i = 0; i < N; i++) {
        while (C[pos] <= B[i] && pos < N) {
            pos++;
        }
    ...
    for (uint64_t ofs = 0; ofs < N; ofs++) {
        if (ofs == 0) {
            ...
            sumSum[N - ofs - 1] = sumSum[N - ofs] + sum[N - ofs - 1];
        }
    }
