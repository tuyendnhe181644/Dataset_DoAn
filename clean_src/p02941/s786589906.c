...
int main(void) {
    ...
    bool ableToAchieve = true;

    bHeap_Init(_bHeap_Compare);

    scanf("%"PRIu64, &N);
    for (uint64_t i = 0; i < N; i++) {
        scanf("%"PRIu64, &(status[i].A));
    }
    for (uint64_t i = 0; i < N; i++) {
        scanf("%"PRIu64, &(status[i].B));
        status[i].val = status[i].B;

        if (status[i].A < status[i].B) {
            BHeap_Val val;
    ...
    while (ableToAchieve) {
        BHEAP_VAL_TYPE* idxP = bHeap_GetTopVal();

        if (idxP == NULL) {
            break;
        }
        else {
            const uint64_t idx = idxP->idx;

            const uint64_t left = status[(idx + N - 1) % N].val;
            const uint64_t center = status[idx].val;
            const uint64_t right = status[(idx + 1) % N].val;
            
            const uint64_t remain = center - status[idx].A;

            if (remain < left + right) {
                ableToAchieve = false;
            }
            else {
                bHeap_RemoveTop();

                const uint64_t addCnt = remain / (left + right);

                cnt += addCnt;

                status[idx].val -= addCnt * (left + right);

                if (status[idx].A < status[idx].val) {
                    BHeap_Val newVal;
