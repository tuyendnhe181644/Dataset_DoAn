...
int main(void){
    In(N);
    In(K);
    //サイコロを振る→勝負未確定なら1/2で負け確定、1/2で継続
    //出目から何連続で表が要求されるか求める。
    //とりあえず全Nの分母のみを求める。
    long bunbo[N+1];
    long bunshi[N+1];
    format_array(N+1,bunbo,0);
    format_array(N+1,bunshi,1);
    for(loop1=1;loop1<=N;loop1++){
        //まずloop1が出る確率は1/N、要求される表の回数はシミュレーション
        bunbo[loop1]=N;
        long now_score=loop1;
        while(now_score<K){
            now_score*=2;
            bunbo[loop1]*=2;
        }
        ...
        //通分
        if(loop1!=1){
            bunshi[loop1]=bunbo[1]/bunbo[loop1];
        }
    }
    long bunshi_sum=0;
    for(loop1=1;loop1<=N;loop1++){
        bunshi_sum+=bunshi[loop1];
    }
    printf("%.12Lf\n",(long double)bunshi_sum/(long double)bunbo[1]);
