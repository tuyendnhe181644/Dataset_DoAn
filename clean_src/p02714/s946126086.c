...
static uint64_t rSum[4000], gSum[4000], bSum[4000];

static int rgbi(const char S) {
    if (S == 'R') {
        return 0;
    }
    else if (S == 'G') {
...
int main(void) {
    ...
    rSum[N - 1] = (S[N - 1] == 'R' ? 1 : 0);
    gSum[N - 1] = (S[N - 1] == 'G' ? 1 : 0);
    bSum[N - 1] = (S[N - 1] == 'B' ? 1 : 0);

    for (uint64_t ofs = 1; ofs < N; ofs++) {
        const uint64_t i = N - ofs - 1;
