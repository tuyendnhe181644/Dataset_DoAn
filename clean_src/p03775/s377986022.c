...
static size_t getNumDigits(uint64_t val) {
    if (val == 0) {
        return 1;
    }
...
int main(void) {
    ...
    scanf("%"PRIu64"%"PRIu64, &N);

    size_t minMaxNumDigits = 10000;

    const uint64_t lim = (uint64_t)sqrt((double)N) + 1;
    for (uint64_t i = 1; i <= lim; i++) {
        if (N % i == 0) {
            const size_t maxNumDigits = MAX(getNumDigits(i), getNumDigits(N / i));
            if (maxNumDigits < minMaxNumDigits) {
                minMaxNumDigits = maxNumDigits;
            }
        }
    }
