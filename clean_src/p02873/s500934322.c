...
static uint64_t contLenList[500000];
static uint64_t contLenList_Size = 0;
...
int main(void) {
    ...
    for (size_t i = 1; i < len; i++) {
        if (S[i] != S[i - 1]) {
            contLenList[contLenList_Size] = contLen;
            contLenList_Size++;
            contLen = 1;
        }
        else {
    ...
    if (S[0] == '>') {
        sum += contLenList[0] * (contLenList[0] + 1) / 2;
        i++;
    }

    for (; i < contLenList_Size - 1; i += 2) {
        if (contLenList[i] < contLenList[i + 1]) {
            sum += (contLenList[i] - 1) * contLenList[i] / 2;
            sum += contLenList[i + 1] * (contLenList[i + 1] + 1) / 2;
        }
        else {
            sum += contLenList[i] * (contLenList[i] + 1) / 2;
            sum += (contLenList[i + 1] - 1) * contLenList[i + 1] / 2;
        }
    }

    if (i != contLenList_Size) {
        sum += contLenList[contLenList_Size - 1] * (contLenList[contLenList_Size - 1] + 1) / 2;
    }
