...
#define number_of_vertexes 251151
int ed_cnt[number_of_vertexes];
ll* gr[number_of_vertexes];
// ll* ed_num[number_of_vertexes];
...
void solve(){
    ...
    rep(i,0,n){
        ...
        cnt[i]=count(a,0,n-1,a[i]);
        rep(j,0,31){
            ll rest=(1<<j)-a[i];
            if(a[i]<1) continue;
            if(a[i]>rest) continue;
            if(rest>a[n-1]) continue;
            ll l=lower_bound(a,0,n-1,rest,1);
            ll r=upper_bound(a,0,n-1,rest,1);
            if(l==-1) continue;
            // cnt[l]=r-l+1;
            if(rest==a[i]){
                if(l==r) continue;
