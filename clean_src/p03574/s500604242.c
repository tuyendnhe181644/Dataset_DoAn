...
int main(void){
    ...
    In(yoko);
    char data[tate][yoko];
    int output[tate][yoko];
    scanf("\n");
    for(loop1=0;loop1<tate;loop1++){
        for(loop2=0;loop2<yoko;loop2++){
            scanf("%c",&data[loop1][loop2]);
            output[loop1][loop2]=0; //初期化をここでやる
        }
        scanf("\n");
    }
    for(loop1=0;loop1<tate;loop1++){
        for(loop2=0;loop2<yoko;loop2++){
            output[loop1][loop2]+=(loop1!=0 && loop2!=0 && data[loop1-1][loop2-1]=='#');//左上
            output[loop1][loop2]+=(loop1!=0 && data[loop1-1][loop2]=='#');//上
            output[loop1][loop2]+=(loop1!=0 && loop2!=yoko-1 && data[loop1-1][loop2+1]=='#');//右上
            output[loop1][loop2]+=(loop2!=0 && data[loop1][loop2-1]=='#');//左
            output[loop1][loop2]+=(loop2!=yoko-1 && data[loop1][loop2+1]=='#');//右
            output[loop1][loop2]+=(loop1!=tate-1 && loop2!=0 && data[loop1+1][loop2-1]=='#');//左下
            output[loop1][loop2]+=(loop1!=tate-1 && data[loop1+1][loop2]=='#');//下
            output[loop1][loop2]+=(loop1!=tate-1 && loop2!=yoko-1 && data[loop1+1][loop2+1]=='#');//右下
        }
    }
    for(loop1=0;loop1<tate;loop1++){
        for(loop2=0;loop2<yoko;loop2++){
