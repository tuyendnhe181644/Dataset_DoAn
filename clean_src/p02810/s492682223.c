...
ull solve () {
			...
			for (j = 0; j <= maxj; j++) {
				for (ki = 0; ki <= k; ki++) {
					...
					for (li = 0; li <= ki / i; li++) {
						...
						item = dp[i + 1][j - li][ki - i * li] * ncr(j, li, MOD) % MOD;
						dp[i][j][ki] += item;
					}
					dp[i][j][ki] %= MOD;
					dp[i][j][ki] = dp[i][j][ki] * bitpow(ki - j * (i - 1), cnt[i], MOD) % MOD;
	...
	for (j = 1; j <= maxj; j++) {
		for (ki = 1; ki <= k; ki++) {
			...
			item = dp[1][j][ki] * ncr(k - ki + 1, k - ki - j + 1, MOD) % MOD;
			...
			// printf("%lld %lld: %lld * %lld\n", j, ki, dp[1][j][ki], ncr(k - ki + 1, k - ki - j + 1, MOD));
		}
		result %= MOD;
	}
