...
#define the_number_of_edge 514123
int edge_cnt[the_number_of_edge];
ll* gr[the_number_of_edge];
...
void pushGraph(ll v1, ll v2, ll weight , ll is_dir){
    gr[v1] = (ll*) realloc( gr[v1] , sizeof(ll)*(edge_cnt[v1]*2+2) );    
    gr[v1][edge_cnt[v1]*2] = v2;
    gr[v1][edge_cnt[v1]*2+1] = weight;
    edge_cnt[v1]++;

    if(!is_dir){
        gr[v2] = (ll*) realloc( gr[v2] , sizeof(ll)*(edge_cnt[v2]*2+2) );
...
void dijkstra(ll start){
    ...
    while(size){
        ...
        rep(i,0,edge_cnt[piv]){
            if( ans[ gr[ heap[0].to ][i*2] ] > ans[ heap[0].to ] + gr[ heap[0].to ][i*2+1] ){
...
void solve(){
    ...
    rep(i,0,m){
        ...
        cin(&b);
        a--; b--;
        pushGraph(a*3,b*3+1,1,1);
        pushGraph(a*3+1,b*3+2,1,1);
        pushGraph(a*3+2,b*3,1,1);
    }
    cin(&s); s--;
    cin(&t); t--;
    dijkstra(s*3);
    if(ans[t*3]==INF||ans[t*3]%3){
        printf("-1\n");
    }else{
        printf("%lld\n",ans[t*3]/3);
    }
