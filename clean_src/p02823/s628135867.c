...
bool func (sll index) {
	if (k <= q + index) {
		sll bound = a[n - q];
		if (a[index] + m >= bound) return true;
		return false;
	}

	sll v = a[index] + m;
	if (v < a[n - q]) return false;

	sll x = k - q - index;
	sll len = 0;
	sll sum = 0;
	for (sll i = index + 1; i <= n - q; i++) {
		// printf("%lld,\n", v - a[i]);
		sum += b[len++] = v - a[i];
	}
	sll sum2 = 0;
	for (sll i = x - 1; i < len; i++) {
		sum2 += b[i];
	}
	sll maxv = smin(sum / x, sum2);
	// printf("%lld >= %lld\n", maxv, m);
	if (maxv >= m) return true;
	return false;
}
...
ull solve () {
	...
	if (h % 2 == w % 2) {
		...
		if (h > w) sw(&h, &w);
		result = smin(f(h, w), f(n + 1 - w, n + 1 - h));
	}
