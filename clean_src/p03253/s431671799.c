...
int main(void){
    ...
    rep(i,0,fac_cnt){
        ...
        // if(!factors[1][i]) continue;
        ll tmp=mod_fac[n-1+factors[1][i]]*mod_MOD1(finv[factors[1][i]]*finv[n-1]);
        // printf("%lld\n",mod_fac[n-1+factors[1][i]]);
        // printf("%lld\n",mod_MOD1( mod_fac[n-1+factors[1][i]] * finv[n-1] ));
        // printf("%lld\n",mod_MOD1(tmp));
        // printf("%lld\n",factors[1][i]);
        ans*=mod_MOD1(tmp);
        ans%=MOD1;
