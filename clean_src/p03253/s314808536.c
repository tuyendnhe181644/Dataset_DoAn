...
// ll inv_arr[200005],finv_arr[200005]
ll *mod_fac_arr;
ll *inv_arr,*finv_arr;
ll is_modinv_made=0,is_modfac_made=0;
ll makeinv(ll n , ll mod){ 
    rep(i,0,1){if(is_modinv_made++) return 0;}
    inv_arr = realloc(inv_arr, sizeof(ll)*2 );
    finv_arr = realloc(finv_arr, sizeof(ll)*2 );
    inv_arr[1]=1;finv_arr[0]=finv_arr[1]=1;
    rep(i,2,n+1){
...
ll make_mod_fac(ll n, ll mod){
    rep(i,0,1){ if(is_modfac_made++) return 0; }
    mod_fac_arr = realloc(mod_fac_arr, sizeof(ll)*2 );
    ll x=1; 
    mod_fac_arr[0]=mod_fac_arr[1]=x; 
    rep(i,2,n+1){ 
        x*=i%mod; 
        x%=mod; 
        mod_fac_arr = realloc(mod_fac_arr, sizeof(ll)*(i+1) );
        mod_fac_arr[i]=x; }
    return 0;
}
ll mod_fac(ll x, ll mod){
    make_mod_fac(2*1e5,mod);
    return mod_fac_arr[x];
}
...
int main(void){
    ...
    // }
    rep(i,0,fac_cnt){
        // ll tmp=mod_fac[n-1+factors[1][i]]*mod_MOD1(finv_arr[factors[1][i]]*finv_arr[n-1]);
        ll tmp=mod_fac(n-1+factors[1][i],MOD1)*mod_MOD1( mod_inv(factors[1][i],MOD1,1)*mod_inv(n-1,MOD1,1) );
        // ll tmp=mod_fac[n-1+factors[1][i]]*mod_MOD1( mod_inv(factors[1][i],MOD1,1)*mod_inv(n-1,MOD1,1) );
        ans*=mod_MOD1(tmp);
        ans%=MOD1;
    }
    printf("%lld\n",ans);
