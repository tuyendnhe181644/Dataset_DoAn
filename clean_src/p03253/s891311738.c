...
ll inv_arr[200005],finv_arr[200005],mod_fac[200005], is_modinv_made=0,is_modpow_made=0;
ll makeinv(ll n , ll mod){ 
    rep(i,0,1){if(is_modinv_made++) return 0;}
    inv_arr[1]=1;finv_arr[0]=finv_arr[1]=1;
    rep(i,2,n+1){  inv_arr[i]= mod - inv_arr[mod%i] * (mod / i) % mod;   finv_arr[i] = finv_arr[i - 1] * inv_arr[i] % mod;  }
    return 0; 
}
ll mod_inv(ll x, ll mod, ll is_fac ){
    makeinv(2*1e5,mod);
    return is_fac?finv_arr[x]:inv_arr[x];
    // if(is_modinv_made++) return inv_arr[x]; else{makeinv(2*1e5,mod); return inv_arr[x]; } return 0; 
}
ll mod_finv(ll x, ll mod){
    makeinv(2*1e5,mod);
    return inv_arr[x];
    // if(is_modinv_made++) return inv_arr[x]; else{makeinv(2*1e5,mod); return inv_arr[x]; } return 0; 
}
