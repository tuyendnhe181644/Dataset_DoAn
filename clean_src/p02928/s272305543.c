...
ll factor(ll n, ll is_new){ rep(i,0,fac_cnt){ ll cnt=0; rep(j,0,1){ while( ( cnt+= n %factors[0][i]==0 ? 1 : 0 )  && (n/=factors[0][i]) %factors[0][i]==0 ) continue;} factors[1][i]= is_new ? cnt : max2(factors[1][i],cnt);  if( factors[0][i]> n ) break; }    return n; }
...
int main(void){
    ...
    // printf("%lld %lld\n",tmp1,tmp2);
    ans= tmp1*k + tmp2*mod_MOD1( k*(k-1)/2 );
