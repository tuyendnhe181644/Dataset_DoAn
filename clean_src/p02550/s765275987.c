...
static uint64_t ss[100000];
static uint64_t found[100000];

int main(void) {
    uint64_t N, X, M;

    scanf("%"PRIu64"%"PRIu64"%"PRIu64, &N, &X, &M);

    for (uint64_t i = 0; i < M; i++) {
        found[i] = UINT64_MAX;
    }

    uint64_t intervalEnd = 1;

    ss[0] = X;
    found[X] = 0;
    uint64_t v = X;

    while (found[v * v % M] == UINT64_MAX) {
        ss[intervalEnd] = ss[intervalEnd - 1] + v * v % M;
    ...
    const uint64_t intervalStart = found[v * v % M];

    //printf("IS: %"PRIu64" / IE: %"PRIu64"\n", intervalStart, intervalEnd);
    ...
    if (N <= intervalStart) {
        ...
        //r += ss[intervalStart - 1];

        N -= intervalStart;

        r += N / (intervalEnd - intervalStart) * (ss[intervalEnd - 1] - (intervalStart == 0 ? 0 : ss[intervalStart - 1]));
        r += ss[intervalStart + N % (intervalEnd - intervalStart) - 1];
    }
