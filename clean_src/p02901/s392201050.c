...
int main(void) {
    ...
    for (uint64_t i = 0; i < M; i++) {
        scanf("%"PRIu64"%"PRIu64, &tb[i].a, &tb[i].b);
        for (uint64_t ci = 0; ci < tb[i].b; ci++) {
            scanf("%"PRIu64, &tb[i].c[ci]);
            tb[i].c[ci]--;
        }
    }
    ...
    {
        uint64_t pattern = 0;
        for (uint64_t i = 0; i < tb[0].b; i++) {
            pattern |= UINT64_C(0x1) << tb[0].c[i];
        }
    ...
    const uint64_t lim = powU64(2, N) - 1;

    for (uint64_t k = 1; k < M; k++) {
        uint64_t pattern = 0;
        for (uint64_t i = 0; i < tb[k].b; i++) {
            pattern |= UINT64_C(0x1) << tb[k].c[i];
        }

        const size_t prevIdx = (k - 1) % 2;

        for (uint64_t pp = 0; pp <= lim; pp++) {
            dp[k % 2][pp] = dp[prevIdx][pp];
        }

        for (uint64_t pp = 0; pp <= lim; pp++) {
            if (dp[prevIdx][pp] != UINT64_MAX) {
                if (dp[prevIdx][pp] + tb[k].a < dp[k % 2][(pp | pattern)]) {
                    dp[k % 2][(pp | pattern)] = dp[prevIdx][pp] + tb[k].a;
                }
            }
        }
    }

    if(dp[(M - 1) % 2][lim]==UINT64_MAX){
        puts("-1");
    }
    else {
        printf("%"PRIu64"\n", dp[(M - 1) % 2][lim]);
