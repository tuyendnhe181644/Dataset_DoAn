...
int union_find_root(struct union_find *p, int x){ return p->p_prev[x]==x?x:(p->p_prev[x]=union_find_root(p, p->p_prev[x])); }
int union_find_size(struct union_find *p, int x){ return p->p_size[union_find_root(p, x)]; }
