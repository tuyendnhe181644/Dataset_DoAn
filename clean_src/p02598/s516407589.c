...
// void swap_d(double *a, double *b){double c;c=*b;*b=*a;*a= c;}
...
// ll Heapcmp(double a, double b, ll kind){ return kind ? (a<b?1:0) : (a>b?1:0); }
// double heap[MAX_N];

// void pushHeap(double val, ll len , ll kind){
//     ll i=len; heap[i]=val;
//     while(i!=0){
//         if( Heapcmp( heap[i], heap[(i-1)/2] , llabs(kind-1) )  ) swap_d(&heap[i],&heap[(i-1)/2]);
//         i--;i/=2;
//     }
// }
...
int main(void){
    ...
    while(1){
        ...
        rep(i,0,n){
            ...
            if(a[i]>p){
                t+= a[i]%p==0?a[i]/p-1:a[i]/p;
                // t+=  a[i]/p;
            }
            // t+= a[i]%p==0?a[i]/p-1:a[i]/p+1;
            // t+=(ll) floor( (double) a[i]/p );
            // s=min2(s,)
        }
        // printf("%lld %lld\n",p,t);
        if(t<=k){
