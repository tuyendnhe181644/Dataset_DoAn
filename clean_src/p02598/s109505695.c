...
static uint64_t searchMaxLen(const uint64_t lenMin, const uint64_t lenMax, const uint64_t K, const uint64_t N) {
    //printf("Len-Min: %"PRIu64", Len-Max: %"PRIu64"\n", lenMin, lenMax);
    const uint64_t len = (lenMax + lenMin) / 2;
    uint64_t cnt = 0;
    for (uint64_t i = 0; i < N; i++) {
        cnt += (A[i] + len - 1) / len - 1;
    }
    ...
    if (lenMax - lenMin == 0) {
        return cnt <= K ? lenMin : UINT64_MAX;
    }
    else {
        if (cnt <= K) {
            const uint64_t lLen = searchMaxLen(lenMin, len, K, N);

            return lLen == UINT64_MAX ? len : lLen;
        }
        else {
            if (0 < lenMax - len) {
                return searchMaxLen(len + 1, lenMax, K, N);
            }
            else {
                return UINT64_MAX;
            }
