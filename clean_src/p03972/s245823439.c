...
ull solve () {
	...
	ull columns = 0, rows = 0;
	qsort(a, h, sizeof(ull), pullcomp);
	qsort(b, w, sizeof(ull), pullcomp);

	while (columns < h || rows < w) {
		if (columns < h && (rows >= w || a[columns] < b[rows])) {
			result += a[columns] * (w + 1 - rows);

			columns++;
		} else {
			result += b[rows] * (h + 1 - columns);
