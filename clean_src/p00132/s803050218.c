...
int size[12][3],pie[12][500][2],nsel[12],order[12],list[12],q[500],mk[25][25],cov[12][25][25];
...
void cover(int n,int x,int y,int dir){
	int i,px,py,ok=1;
	int hei=size[n][0],wid=size[n][1],num=size[n][2];
	for(i=0;i<num;i++){
		px=pie[n][i][0]; py=pie[n][i][1];
		if(dir==0)cov[n][x+px][y+py]=SET;
		if(dir==1)cov[n][x+py][y+hei-px-1]=SET;
		if(dir==2)cov[n][x+hei-px-1][y+wid-py-1]=SET;
		if(dir==3)cov[n][x+wid-py-1][y+px]=SET;
	}
}
...
out3:
				...
				//for(j=0;j<H;j++){for(k=0;k<W;k++){printf("%d",cov[i][j][k]);}printf("\n");}
			...
			for(scanf("%d",&PL);PL>0;PL--){
				...
				for(j=0;j<H;j++){
					for(k=0;k<W;k++){
						if(mk[j][k]==0 && map[j][k]=='.'){q1=0;goto out;}
					}
				}
