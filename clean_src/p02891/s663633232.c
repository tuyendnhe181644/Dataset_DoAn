...
static uint64_t contLength[100];
static char contChar[100];
static size_t contList_Size = 0;
...
int main(void) {
    ...
    for (uint64_t i = 1; i < len; i++) {
        if (S[i] == S[i - 1]) {
            ...
            contLength[contList_Size] = contLen;
            contChar[contList_Size] = S[i - 1];
            contList_Size++;
            contLen = 1;
        }
    }
    ...
    contLength[contList_Size] = contLen;
    contChar[contList_Size] = S[len - 1];
    contList_Size++;

    uint64_t sum = 0;

    if (1 < contList_Size) {
        if (contChar[0] == contChar[contList_Size - 1] && 2 <= contList_Size) {
            sum += (contLength[0] + contLength[contList_Size - 1]) / 2 * (K - 1);
            sum += contLength[0] / 2 + contLength[contList_Size - 1] / 2;
            for (uint64_t i = 1; i < contList_Size - 1; i++) {
                sum += contLength[i] / 2 * K;
            }
        }
        else {
            for (uint64_t i = 0; i < contList_Size; i++) {
                sum += contLength[i] / 2 * K;
            }
        }
    }
