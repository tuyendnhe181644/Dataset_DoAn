...
static uint64_t getLSD(uint64_t v) {
    return v % 10;
}

static uint64_t getMSD(uint64_t v) {
    while (10 <= v) {
        v /= 10;
    }
...
int main(void) {
    ...
    for (uint64_t A = 1; A <= N; A++) {
        const uint64_t aLSD = getLSD(A);
        const uint64_t aMSD = getMSD(A);
        ...
        if (aLSD != 0) {
            ...
            uint64_t bMin = aLSD * multiplier + aMSD;
            uint64_t bMax = (aLSD + 1) * multiplier - 10 + aMSD;

            //printf("%"PRIu64" %"PRIu64" %"PRIu64" %"PRIu64"\n", A, bMin, bMax, multiplier);
            ...
            while (bMin <= N) {
                ...
                multiplier *= 10;
                bMin = aLSD * multiplier + aMSD;
                bMax = (aLSD + 1) * multiplier - 10 + aMSD;
            }
        }
    }
