...
static Robot rb[100000];

static int compare_f(const void* a, const void* b) {
    const Robot* aP = (Robot*)a;
    const Robot* bP = (Robot*)b;

    return aP->X - aP->L < bP->X - bP->L ? -1 : 1;
}

static uint64_t dp[100000];

#define S_IV (UINT64_MAX)

static uint64_t searchR(const uint64_t base, const uint64_t size, const int64_t v) {
    if (size == 1) {
        return v <= rb[base].X - rb[base].L ? base : S_IV;
    }

    const uint64_t t = base + size / 2;
    if (rb[t].X - rb[t].L < v) {
        if (base + size - t - 1 == 0) {
            return S_IV;
        }
        else {
            return searchR(t + 1, base + size - t - 1, v);
        }
    }
    else {
        if (t - base == 0) {
            ...
            const uint64_t leftRes = searchR(base, t - base, v);

            return leftRes == S_IV ? t : leftRes;
        }
    }
}
...
int main(void) {
    ...
    scanf("%"PRIu64, &N);
    for (uint64_t i = 0; i < N; i++) {
        scanf("%"PRId64"%"PRId64, &rb[i].X, &rb[i].L);
        dp[i] = 0;
    }

    qsort(rb, (size_t)N, sizeof(Robot), compare_f);
    ...
    for (uint64_t i = 0; i < N; i++) {
        ...
        if (i < N - 1) {
            const uint64_t nextPos = searchR(i + 1, N - i - 1, rb[i].X + rb[i].L);
            if (nextPos != S_IV) {
                dp[nextPos] = dp[i];
            }
        }
