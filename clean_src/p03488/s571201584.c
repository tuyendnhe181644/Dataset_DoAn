...
void solve(){
    ...
    w[0][center]=1;
    h[0][center]=1;
    ll cntw=0,cnth=0,qh=0,qw=0,second=0;
    // rep(i,0,qcnt){
    //     printf("%lld ",q[i]);
    // }PN;
    rep(i,0,qcnt){
        if(q[i]==0){
            ...
            if(t){
                qw++;
                rep(j,-lr-5+center,lr+5+center){
                    if( w[cntw][j] ){
                        // printf("%lld %lld\n",i,j);
                        if(second) w[cntw^1][j-q[i]]=1;
                        w[cntw^1][j+q[i]]=1;
                        w[cntw][j]=0;
                    }
                }
    ...
    if( w[qw%2][center+x] && h[qh%2][center+y] ){
        YES;
    }else NO;
}
