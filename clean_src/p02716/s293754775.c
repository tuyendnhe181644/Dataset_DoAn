...
int main(void){
    ...
    // rep(i,2,n){
    //     if(i%2){
    //         dp2[i+1][0]=a[i]+max2(dp2[i-1][0],dp2[i-2][1]);
    //     }else{
    //         if(i==2) dp2[i+1][0]=a[i];
    //         else dp2[i+1][0]=a[i]+max3(dp2[i-1][0],dp2[i-2][0],dp2[i-3][1]);
    //         dp2[i+1][1]=a[i]+dp2[i-1][1];
    //     }
    // }
    dp2[0][1]=a[0];
    dp2[1][0]=a[1];
    rep(i,2,n){
        if(i%2){
            dp2[i][0]=a[i]+max2(dp2[i-2][0],dp2[i-3][1]);
        }else{
            if(n<4) dp2[i][0]=a[i];
            else dp2[i][0]=a[i]+max3(dp2[i-2][0],dp2[i-3][0],dp2[i-4][1]);
            dp2[i][1]=a[i]+dp2[i-2][1];
        }
    }
    // rep(i,0,n){
    //     printf("%lld %lld\n",dp2[i+1][0],dp2[i+1][1]);
    // }
    if(n%2==0) printf("%lld\n", max2(dp2[n-1][0],dp2[n-2][1]) );
    else printf("%lld\n", max3(dp2[n-1][0],dp2[n-2][0],dp2[n-3][1]) );
    return 0;
}
