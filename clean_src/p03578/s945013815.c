...
int main(void){
    In(how_have_task);
    long have_task[how_have_task];
    input_array(how_have_task,have_task);
    QS09(how_have_task,have_task);
    
    In(how_syutudai_task);
    long syutudai_task[how_syutudai_task];
    input_array(how_syutudai_task,syutudai_task);
    QS09(how_syutudai_task,syutudai_task);
    
    if(how_have_task<how_syutudai_task){
        printf("NO");
        END;
    }
    ...
    for(loop1=0;loop1<how_syutudai_task;loop1++){
restart:
        if(syutudai_task[loop1]<have_task[loop2]){
            printf("NO");
            END;
        }else if(syutudai_task[loop1]>have_task[loop2]){
            loop2++;
            goto restart;
        }else{
            loop2++;
