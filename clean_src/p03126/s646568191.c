...
void max_double_array_num(double array[], int min, int max, int answer[])
{
	int i=min;
	int count=0;
	double max_value=max_double_array(array, min, max);
...
void min_double_array_num(double array[], int min, int max, int answer[])
{
	int i=min;
	int count=0;
	double min_value=min_double_array(array, min, max);
...
int main()
{
	...
	int K[N+1];
	int A[N+1][M+1];
	fill_int_array(N+1, M+1, A, 0, N, 0, M, 0);

	rep(i, 1, N)
	{
		scanf("%d", &K[i]);
		scanf_int_array(A[i], 1, K[i]);
	}

	int count=0;
	rep(j, 1, M)
	{
		if_forall(i, 1, N, detect_int(A[i], 1, K[i], j)==1)
		{
			count=count+1;
		}
	}
