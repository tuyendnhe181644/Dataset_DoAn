...
typedef struct { char s, a; short st, at, f; } T;
T tbl[2002]; int m;
char city[101][18];
...
int dijkstra(int start, int tm)
{
	int i, j, s, d, g0, g1, ans;
	T *tp;
	...
	while (qsize) {
		...
		if (s <= 1) {
			if (s == 0) { if (g0) g0 = 0, ans += mk[0][tm]; }
			else        { if (g1) g1 = 0, ans += mk[1][tm];	}
			if (!(g0|g1)) return ans;
		}
		i = m; for (tp = tbl; i--; tp++) {
...
int main()
{
	...
	while (fgets(p=buf, 10, stdin) && *p != '0') {
		...
		i = m; for (tp = tbl; i--; tp++) {
			...
			if (u < ans) u += dijkstra(tp->a, tp->at + 30);
			if (u < ans) ans = u;
		}
		if (ans == INF) ans = 0;
		printf("%d\n", ans);
