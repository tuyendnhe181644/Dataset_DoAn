...
int main()
{
	...
	for (i = 0; i < T; i++) ee[i] = e[i] = in(), c[i] = in();
	for (i = 0; i < N; i++) h[i] = in(), a[i] = in();

	qsort(ee, T, sizeof(int), cmp);
	s = 0; for (i = 0; i < U; i++) s += ee[i];
	if (s > S || S-(D-1)*(s-O) < s) { puts("-1"); return 0; }

	memset(dp1, -1, sizeof(dp1)), dp1[0][0] = 0;
	for (i = 1; i <= N; i++) for (j = 0; j <= 1000; j++) for (s = 0; s*a[i-1] <= j; s++) {
