...
int main()
{
	...
	for (i = 0; i < n; i++) {
		...
		q = bp; while (isalpha(*bp)) bp++;
		*bp = 0, strcpy(s[i], q), insert(s[i], bp-q, i);
		bp++, x[i] = getint();
	}
	...
	while (m--) {
		...
		q = bp; while (isalpha(*bp)) bp++;
		*bp = 0, i = lookup(q, bp-q);

		q = ++bp; while (isalpha(*bp)) bp++;
		*bp = 0, j = lookup(q, bp-q);
		union_set(i, j);
	}
