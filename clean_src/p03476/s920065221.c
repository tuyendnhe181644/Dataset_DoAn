...
int main(void){
    //あらかじめ、全ての数に対して判定しておく
    
    bool niteiru[100010]={0};
    long ruiseki[100010]={0};
    for(loop1=1;loop1<100005;loop1++){
        ruiseki[loop1]=ruiseki[loop1-1];
        if(check_prime(loop1)==1 && check_prime((loop1+1)/2)){
            niteiru[loop1]=1;
            ruiseki[loop1]++;
            //printf("%ld\n",loop1);
        }
...
bool check_prime(long target){//素数判定、素数なら1
    bool flag=0;
    int loopA;
    for(loopA=2;loopA<=sqrt(target)+1;loopA++){
        if(target%loopA==0){
            flag=1;
            loopA=target;
        }
