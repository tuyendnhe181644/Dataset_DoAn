...
ll jud_prime(ll n){ rep(i,0,fac_cnt){ if(n<factors[0][i]*factors[0][i] || n==factors[0][i]) break; else if(n%factors[0][i]==0) n/=n; }  return n==1?0:1; }
...
int main(void){
    ...
    factor_pre(1e5);
    repp(i,1,1e5+1,2){
        ans[i]=jud_prime(i)*jud_prime((i+1)/2);
    }
    rep(i,1,1e5+1){
        ans[i]+=ans[i-1];
    }
    // rep(i,0,10) printf("%lld ",ans[i]);
    // PN;
    // rep(i,0,10) printf("%lld ",jud_prime(i+1) );
    // PN;
    rep(i,0,q){
        cin(&l);
        cin(&r);
