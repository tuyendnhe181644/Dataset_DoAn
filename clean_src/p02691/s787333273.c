...
static HMap hMap[HASH_MAP_SIZE];

static void HMap_Init(void) {
    for (uint64_t i = 0; i < HASH_MAP_SIZE; i++) {
        hMap[i].strListSize = 0;
...
static void HMap_Reg(const char* str, const uint64_t len) {
    uint32_t h[5];

    sha1(str, len, h);

    const uint64_t fh = h[0] % HASH_MAP_SIZE;

    for (uint64_t i = 0; i < hMap[fh].strListSize; i++) {
        if (strcmp(hMap[fh].str[i], str) == 0) {
            hMap[fh].cnt[i]++;
            return;
        }
    }

    strcpy(hMap[fh].str[hMap[fh].strListSize], str);
    hMap[fh].cnt[hMap[fh].strListSize] = 1;

    hMap[fh].strListSize++;
}

static uint64_t cnt[200000];
