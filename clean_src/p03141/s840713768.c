...
int cp(const long *a, const long *b){
    if(a[2]>b[2]){
        return -1;
    }else if(a[2]==b[2]){
        return 0;
...
int main(void){
    long how_menu;
    scanf("%ld",&how_menu);
    long data[how_menu][3];
    for(loop1=0;loop1<how_menu;loop1++){
        for(loop2=0;loop2<2;loop2++){
            scanf("%ld",&data[loop1][loop2]);
        }
        data[loop1][2]=data[loop1][0]+data[loop1][1];
    }
    //高橋くんは、a+bが大きいのから食べようとする。
    //青木さんは、a+bが大きいのから食べようとする。
    //2人とも同じ優先度となる。
    qsort(data,how_menu,sizeof(long)*3,cp);
    for(loop1=0;loop1<how_menu;loop1++){
        //printf("%ld %ld %ld\n",data[loop1][0],data[loop1][1],data[loop1][2]);
    }
