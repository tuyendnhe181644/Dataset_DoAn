...
static uint64_t p[100000];
static char S[100000][3];
static bool isAC[100000];
static uint64_t penaltyCnt[100000];

int main(void) {
    uint64_t N, M;
    uint64_t acCnt = 0;
    uint64_t penaltyCntSum = 0;

    scanf("%"PRIu64"%"PRIu64, &N, &M);
    for (uint64_t i = 0; i < M; i++) {
        scanf("%"PRIu64"%s", &p[i], S[i]);
    }
    ...
    for (uint64_t i = 0; i < M; i++) {
        if (strstr(S[i], "AC") != NULL) {
            if (!isAC[p[i]]) {
                isAC[p[i]] = true;
                acCnt++;
                penaltyCntSum += penaltyCnt[p[i]];
            }
        }
        else {
            if (!isAC[p[i]]) {
