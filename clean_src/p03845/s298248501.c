...
int main(void){
    ...
    int data_q[how_q];
    int loop1;
    input_array(how_q,data_q);
    int how_drink;
    scanf("%d",&how_drink);
    int data_drink[how_drink][2];
    input_array2(how_drink,2,data_drink);
    int sum=array_sum(how_q,data_q);
    for(loop1=0;loop1<how_drink;loop1++){
        printf("%d\n",sum-(data_q[data_drink[loop1][0]-1]-data_drink[loop1][1]));
    }
    return 0;
