...
int main(void) {
	...
	s_cin(s); sl = strlen(s); s[sl] = 0x00;
	s_cin(t); tl = strlen(t); t[tl] = 0x00;
	 
	qsort(s, sl, sizeof(char), s_asorts);
	qsort(t, tl, sizeof(char), s_dsorts);
...
// 162 D-RGB Triplets
//static char s[4001];
//static int  r[4000], g[4000], b[4000];
//int main(void)
//{
//	int n, i, j;
//	long cnt = 0;
...
//	s_cin(s);
	...
	//
	////RBG R BG
	//printf("\n r:%d g:%d b:%d %ld", rp, gp, bp,rp*gp*bp);
	//printf("\n"); for (i = 0; i < rp; i++)  printf("%d ",r[i]);
	//printf("\n"); for (i = 0; i < gp; i++) printf("%d ",g[i]);
	//printf("\n"); for (i = 0; i < bp; i++) printf("%d ",b[i]);
