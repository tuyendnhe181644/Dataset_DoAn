...
static double searchAllPath(const uint64_t N, const uint64_t curDepth, const uint64_t prevNode, const int64_t *x,const int64_t *y, const double curSum, bool* visited) {
    if (curDepth < N) {
        double sum = 0;
        for (uint64_t i = 0; i < N; i++) {
            if (!visited[i]) {
                ...
                if (0 < curDepth) {
                    additionalLength = sqrt((double)(ABSS(x[i], x[prevNode]) * ABSS(x[i], x[prevNode]) + ABSS(y[i], y[prevNode]) * ABSS(y[i], y[prevNode])));
                }
                sum += searchAllPath(N, curDepth + 1, i, x, y, curSum + additionalLength, visited);
                visited[i] = false;
            }
        }
...
int main(void) {
    ...
    int64_t x[8], y[8];
    bool visited[8];
    uint64_t lengthSum[8];

    scanf("%"PRIu64, &N);
    for (uint64_t i = 0; i < N; i++) {
        scanf("%"PRId64"%"PRId64, &x[i], &y[i]);
        visited[i] = false;
    }

    printf("%.10lf\n", searchAllPath(N, 0, 0, x, y, 0, visited) / (double)permU64(N));
