...
/* bool is_cross(Vector v1, Vector v2) { */
/*     return (cross(sub_point(v1.end, v1.start), sub_point(v2.start, v1.start)) * */
/*             cross(sub_point(v1.end, v1.start), sub_point(v2.end, v1.start)) < RADIUS) && */
/*         (cross(sub_point(v2.end, v2.start), sub_point(v1.start, v2.start)) * */
/*          cross(sub_point(v2.end, v2.start), sub_point(v1.end, v2.start)) < RADIUS); */
/* } */

#define is_cross(v1, v2) \
    ((cross(sub_point(v1.end, v1.start), sub_point(v2.start, v1.start)) * \
      cross(sub_point(v1.end, v1.start), sub_point(v2.end, v1.start)) < RADIUS) && \
     (cross(sub_point(v2.end, v2.start), sub_point(v1.start, v2.start)) * \
      cross(sub_point(v2.end, v2.start), sub_point(v1.end, v2.start)) < RADIUS))
...
/*     int x; */
/*     int y; */
/*     Vector vectors[MAX_INPUT][MAX_INPUT] = {}; */
/*     for (i = 0; i < n; i++) { */
/*         for (j = 0; j < n; j++) { */
/*             vectors[i][j] = cross_point(points[i], points[j]); */
/*         } */
/*     } */
/*     int max_counts = 1; */
/*     int counts; */
...
/*                 counts = 1; */
/*                 for (y = 0; y < n; y++) { */
/*                     if (is_cross(vectors[i][j], vectors[x][y])) { */
/*                         counts++; */
/*                     } */
/*                 } */
