...
int i,j,n;
void check(double x,double y,int p,int q){
	int i; double px,py,a=2.0;
	for(i=0;i<n;i++){
		if(i!=p && i!=q){
			px=pot[i][0];py=pot[i][1];
			if((px-x)*(px-x)+(py-y)*(py-y)<=1){a+=pot[i][2];}
		}
	}
	if(a>ans)ans=a;
}
	...
	while(scanf("%d",&n)*n){
		...
		for(i=0;i<n-1;i++){
			...
			for(j=i+1;j<n;j++){
				...
				if(d==4)check((xp+xq)/2,(yp+yq)/2,i,j);
				else if (d<4){
					d=sqrt(1-d/4);
					a=atan2(xq-xp,yp-yq);
					check((xp+xq)/2+d*cos(a),(yp+yq)/2+d*sin(a),i,j);
					check((xp+xq)/2-d*cos(a),(yp+yq)/2-d*sin(a),i,j);
				}
			}
		}
