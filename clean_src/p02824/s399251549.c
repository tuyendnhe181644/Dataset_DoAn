...
static uint64_t ASum[100000];

int main(void) {
    uint64_t N, M,V,P;

    scanf("%"PRIu64"%"PRIu64"%"PRIu64"%"PRIu64, &N, &M, &V, &P);
    for (uint64_t i = 0; i < N; i++) {
        scanf("%"PRIu64, &A[i]);
    ...
    qsort(A, (size_t)N, sizeof(uint64_t), compare_u64_desc);
    ...
    if (V <= P) {
        const uint64_t thr = A[P - 1];

        uint64_t cnt = P;

        for (uint64_t idx = P; idx < N; idx++) {
            if (thr <= A[idx] + M) {
                cnt++;
            }
            else {
        ...
        for (uint64_t idx = P; idx < N; idx++) {
            if (thr <= A[idx] + M) {
                ...
                addCnt += (P - 1) * M;
                addCnt += (A[idx] + M) * (idx + 1) - ASum[idx];
                if (1 < P) {
                    addCnt -= (A[idx] + M) * (P - 1) - ASum[P - 2];
                }
                addCnt += (N - idx - 1) * M;
