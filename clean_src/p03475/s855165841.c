...
int main(void){
    In(how_station);
    int next_distance[how_station-1];
    int next_time[how_station-1];
    int how_interval[how_station-1];
    
    for(loop1=0;loop1<how_station-1;loop1++){
        scanf("%d %d %d",&next_distance[loop1],&next_time[loop1],&how_interval[loop1]);
    }
    
    for(loop1=0;loop1<how_station-1;loop1++){
        long time=0;
        for(loop2=loop1;loop2<how_station-1;loop2++){
            if(next_time[loop2]>time){
                time=next_time[loop2];
            }
            while(time%how_interval[loop2]!=0)time++;
            //printf("出発%d駅 %d秒\n",loop2,time);
            time+=next_distance[loop2];
        }
        printf("%ld\n",time);
    }
