...
static Q_Entry q[Q_SIZE];
static uint64_t q_Tail = 0;
static uint64_t q_Size = 0;
...
static void q_Add(const uint64_t r, const uint64_t c, const uint64_t d) {
    q[q_Tail].r = r;
    q[q_Tail].c = c;
    q[q_Tail].d = d;
...
static void q_Deq(uint64_t* r, uint64_t* c,uint64_t *d) {
    *r = q[(q_Tail + Q_SIZE - q_Size) % Q_SIZE].r;
    *c = q[(q_Tail + Q_SIZE - q_Size) % Q_SIZE].c;
    *d = q[(q_Tail + Q_SIZE - q_Size) % Q_SIZE].d;

    q_Size--;
}

static uint64_t q_Resolve(const uint64_t gr, const uint64_t gc) {
    while (true) {
        if (q_Size == 0) {
            putchar('a');
            while (true);
        ...
        if (0 < c && S[r][c - 1] == '.' && !visited[r][c - 1]) {
            q_Add(r, c - 1, d + 1);
        }
        if (0 < r && S[r - 1][c] == '.' && !visited[r - 1][c]) {
            q_Add(r - 1, c, d + 1);
        }
        if (r < H - 1 && S[r + 1][c] == '.' && !visited[r + 1][c]) {
            q_Add(r + 1, c, d + 1);
        }
        if (c < W - 1 && S[r][c + 1] == '.' && !visited[r][c + 1]) {
            q_Add(r, c + 1, d + 1);
        }
    }
}
...
int main(void) {
    ...
    for (uint64_t sr = 0; sr < H; sr++) {
        for (uint64_t sc = 0; sc < W; sc++) {
            if (S[sr][sc] == '.') {
                for (uint64_t gr = 0; gr < H; gr++) {
                    for (uint64_t gc = 0; gc < W; gc++) {
                        if (S[gr][gc] == '.') {
                            q_Init();

                            q_Add(sr, sc, 0);

                            const uint64_t d = q_Resolve(gr, gc);
