...
int main(void){
    resheap();
    long long i,j,n,m,k,a,b,c,d,e,h,w,r,l,t,q,dist[4096];
    char s[64][64];
    rs g[524288];
    mkj x[524288];
    scanf("%lld%lld",&h,&w);
    ...
    for(i=1;i<=h;i++){
      for(j=1;j<=w;j++){
        for(k=0;k<4;k++){
          if(s[i][j]=='.'&&s[i+dx4[k]][j+dy4[k]]=='.'){
            g[c].st=i*w+j;
            g[c].fi=(i+dx4[k])*w+(j+dy4[k]);
            g[c].kr=1;
            c++;
          }
        }
    ...
    for(i=1;i<=h;i++){
      for(j=1;j<=w;j++){
        dijkstra(i*w+j,h*w+w,dist,g,x);
        for(k=0;k<=h*w+w;k++){
          if(dist[k]!=llinf){r=llmax(dist[k],r);}
        }
      }
    }
    printf("%lld\n",r);
