...
static void AllFreeProcess_Void(HeapTree_t **pt_tree_point);				/* free  malloc data all */
...
int main(void)
{
	...
	AllFreeProcess_Void(&pt_root_pointer);				 /* All free */
...
/***********************************************************************************
 ++module <AllFreeProcess_Void>
 ++outline
	 free  malloc data all
 ++arguments
	<HeapTree_t>	<**pt_tree_point>	[Heap Sort Data]
 ++supp
 	<return>
 	  nothing
 ++end_module
************************************************************************************/
static void AllFreeProcess_Void(HeapTree_t **pt_tree_point)
{
	HeapTree_t *pt_temp = (*pt_tree_point);				/* Calcurate Pointer */
	HeapTree_t *pt_rest_point[MAX_DEPTH + 1];			/* rest saved free pointer */
	int rest_num = 0;									/* rest saved free pointer num */
	int loop;
	
	do
	{
		if(pt_temp->left != NULL)						/* If Data Exist */
		{
			pt_rest_point[rest_num] = pt_temp->left;	/* save Data Pointer */
			rest_num++;
		}
		else
		{
		...
		if(pt_temp->right != NULL)						/* If Data Exist */
		{
			pt_rest_point[rest_num] = pt_temp->right;	/* save Data Pointer */
			rest_num++;
		}
		else
		{
