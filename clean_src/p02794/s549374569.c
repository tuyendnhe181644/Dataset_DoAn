...
int main()
{
	...
	int prev[51], q[51], head, tail;
	long long bit[51] = {1}, P[21] = {};
	...
	for (i = 0; i < M; i++) {
		...
		for (; w != u; w = u, u = prev[u]) P[i] |= bit[adj[u][w]];
	}
	
	int j, sum[2];
	long long n, ans = (long long)1 << N - 1, white;
	for (n = 1; n < bit[M]; n++) {
		white = 0;
		for (i = 0; i < M; i++) {
			if ((n | bit[i]) == n) white |= P[i];
		}
		for (j = 1, sum[0] = 0; j < N; j++) sum[0] += white / bit[j] % 2;
