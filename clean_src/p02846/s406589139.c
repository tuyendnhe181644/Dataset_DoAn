...
int main(void){
    In(T1);In(T2);In(A1);In(A2);In(B1);In(B2);
    long ans=0;
    //高橋君が先行する際を正とした距離の変化をみる
    //最初のT1分で(A1-B1)*T1メートルの差がつく
    //次のT2分経過後、(A2-B2)*T2メートルだけ差が変化する(大小関係による)
    //(A1>B1 && A2>B2) || (A1<B1 && A2<B2)の場合は離れる一方で出会うことはない。
    if((A1>B1 && A2>B2) || (A1<B1 && A2<B2)){
        printf("0\n");
        END;
    }
    //考えられるパターン
    //distance= 10 → 0 → 10 →　0…　この場合はinf
    //10 5 15 10 20 …この場合は0
    //10 -8 2 -20 -10　…この場合は2
    //T1→T2→T1→T2　までのdistanceの推移を見れば求められるのでは
    long distance1=(A1-B1)*T1; //sample1...-2
    long distance2=distance1+(A2-B2)*T2; //10
    long distance3=distance2+(A1-B1)*T1; //8
    long distance4=distance3+(A2-B2)*T2; //18
    ...
    //distance1が負の場合ここまでのif文に引っかかってなければ必ずdistance2は正
    //この場合必ずdistance1<distance3となる
    //distance1の値とdistance1～distance3の差から何回負の値(0も含めて)になれるか
    if(distance1<0){
        if(distance3>0){
            printf("1\n");
            END;
        ...
        //-100→-80の場合は、-100/-20=5
        //-9→-7の場合は、-9,-7,-5,-3,-1の5回　9/2の切り上げ=5
        //負数の割り算はしたくないので正にする。
        distance1*=-1;
        distance3*=-1;
        ans=roundingup((double)distance1/((double)distance1-distance3));
    }
    if(distance1>0){
        if(distance3<0){
            printf("1\n");
        ...
        //100→80の場合は5
        //9→7の場合は、9,7,5,3,1の5回　9/2の切り上げ=5
        ans=roundingup((double)distance1/((double)distance1-distance3))*2-1;
        if(distance1%(distance1-distance3)==0){
            ans++;
        }
    }
    printf("%ld",ans);
