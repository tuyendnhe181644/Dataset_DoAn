...
int main(void){
    ...
    //最大公約数を大きくするには？
    //N=1 , P=24 ならば、そのまま24
    //N=2 , P=24 ならば、6と4にわけて、ans=2。 8と3にわけたら1になる 12と2にわけても同じ
    //まずはPを素因数分解すべき？
    //Pの最大値が10^12、因数の数の最大は50個以下(2^50>10^13)
    long insuu_list[50]={0}; //0をNULLとして扱う
    long ct=0; //素因数の数
    long diver=2;
    while(diver!=P+1){
        if(P%diver==0){
...
diverover:
    //ここまでで因数リストは完成
    //問題文から、素因数リストの素因数を掛け合わせるなどして、
    //N個の因数にできる(素ではなくなる)
    //最大公約数は、リストの数字がもってる「共通の因数」の最大値
    //どれだけ「共通の因数」を持たせられるか。
    //例えば共通の因数がなければ(数学的に言葉おかしい)(3と8とか)、1となる。
    //例えば、64を2つに分解する時、2と32なら2だが、8と8なら8である。
    //つまり、1箇所に持たせない。
    //同じ因数がN個あれば、それを共通の因数とできる　←これ重要
    //嬉しいことに、先ほどの処理で、リストの因数リストは小さい→大きいにソート済
    
    //for(loop1=0;loop1<ct;loop1++)printf("%d\n",insuu_list[loop1]);
    ;
    
    long ans=1;
    int temp=insuu_list[0];
    int renzoku=1;
    for(loop1=1;loop1<ct;loop1++){
        if(temp==insuu_list[loop1]){
            renzoku++;
            if(renzoku==N){
                ans*=insuu_list[loop1];
                renzoku=1;
                temp=insuu_list[loop1+1];
                loop1++;
            }
        }else{
            temp=insuu_list[loop1];
            renzoku=1;
        }
    }
    printf("%ld",ans);
