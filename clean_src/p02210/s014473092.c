	...
	for (i = 0; i < n - 1; i++) {
		...
		for (j = 0; j < 4; j++) {
			for (ki = i; ki < n; ki++) {
				...
				if (b[ki] < VERYBIG) dp[j + 1][b[ki]] += dp[j][ki];
			}
			...
			// for (ki = i; ki < n; ki++) printf("%lld ", dp[j + 1][ki]);
			// 	puts("");
		}
		// puts("");
		...
		for (ki = i; ki < n; ki++) {
			sll rest = 0;
			for (j = 0; j < i; j++) rest += a[j];
			for (j = ki; j < n; j++) rest += a[j];

			if (rest + k < sum) continue;

		// printf("%lld %lld valid\n", i, ki);
