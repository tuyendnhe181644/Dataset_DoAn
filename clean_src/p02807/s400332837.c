...
static uint64_t x[100000];
static uint64_t probTable[100001];
...
int main(void) {
    ...
    const uint64_t N1 = permU64WMod(N - 1, MOD_VAL);
    ...
    for (uint64_t i = 0; i < N; i++) {
        if (i == 0) {
            ...
            probTable[i] = (probTable[i - 1] + N1 * modinvU64(i + 1, MOD_VAL) % MOD_VAL) % MOD_VAL;
        }
    }
    ...
    for (uint64_t i = 0; i < N - 1; i++) {
        res = (res + (x[i + 1] - x[i]) * probTable[i]) % MOD_VAL;
    }
