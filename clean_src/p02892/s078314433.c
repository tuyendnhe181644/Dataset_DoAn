...
ll bipartite(ll x, ll col){
    if(d[x]){
        if(d[x]==col) return 1;
        else return 0;
    }
    d[x]=col;
    ll is_bip=1,t=col==1?2:1;
    rep(i,0,ed_cnt[x]){
        is_bip&=bipartite(gr[x][i*2],t);
    }
    return is_bip;
}
...
ll WarshallFloyd(ll n ){
    // rep(i,0,n){ rep(j,0,n) printf("%lld ",s[i][j]); PN; }
    rep(k,0,n){
        rep(i,0,n){
            if(i==k) continue;
            rep(j,0,n){
                if(j==k) continue;
                if(s[i][k]>1e15) continue;
                // printf("%lld %lld %lld %lld %lld\n",k,i,j,time[i][k],time[k][j]);
                s[i][j] = min2(s[i][j],s[i][k]+s[k][j]);
            }
        }
    }
    return 0;
...
int main(void){
    ...
    // rep(i,0,n){
    //     rep(j,0,n){
    //             if( s[i][j]<1e15 ) printf("%lld ",s[i][j]);
    //             else printf("-1 ");
    //     }PN;        
    // } PN;
    WarshallFloyd(n);
