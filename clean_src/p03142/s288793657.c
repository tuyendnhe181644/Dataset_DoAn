...
int main(){
	...
	for(i = N - 1; i > 0; i--){
		now_SCC = dag->sorted_SCC[i];
		minval = N;
		for(j = 0; j < now_SCC->prev_num; j++){
			if(now_SCC->prev[j]->val < minval){
				minval = now_SCC->prev[j]->val;
				thennum = now_SCC->prev[j]->vertexs[0];
			}
		}
		ans[now_SCC->vertexs[0]] = thennum;
	}
	ans[dag->sorted_SCC[0]->vertexs[0]] = -1;
	for(i = 0; i < N; i++){
		printf("%d\n", ans[i] + 1);
	}
	return 0;
