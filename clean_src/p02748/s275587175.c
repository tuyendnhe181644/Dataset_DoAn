...
static uint64_t a[100000], b[100000];
...
static MSet ms[100000];

static int compare_f(const void* a, const void* b) {
    const MSet* aP = (MSet*)a;
    const MSet* bP = (MSet*)b;
...
int main(void) {
    uint64_t A, B, M;
    uint64_t bMin = UINT64_MAX;

    scanf("%"PRIu64"%"PRIu64"%"PRIu64, &A, &B, &M);
    ...
    for (uint64_t i = 0; i < M; i++) {
        scanf("%"PRIu64"%"PRIu64"%"PRIu64, &ms[i].x, &ms[i].y, &ms[i].c);

        ms[i].x--;
        ms[i].y--;
    }

    qsort(ms, (size_t)M, sizeof(MSet), compare_f);

    uint64_t msPtr = 0;
    uint64_t setMin = UINT64_MAX;

    for (uint64_t ai = 0; ai < A; ai++) {
        if (a[ai] + bMin < setMin) {
            setMin = a[ai] + bMin;
        }

        while (msPtr < M && ms[msPtr].x == ai) {
            if (a[ai] + b[ms[msPtr].y] - ms[msPtr].c < setMin) {
                setMin = a[ai] + b[ms[msPtr].y] - ms[msPtr].c;
            }
