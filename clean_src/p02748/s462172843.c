...
int main(){
  LL A,B,M,i,ans=INF,mina=INF,minb=INF;
  scanf("%lld%lld%lld",&A,&B,&M);
  LL a[A+1],b[B+1],x[M],y[M],c[M];
  FOR(i,1,A+1)scanf("%lld",&a[i]);
  FOR(i,1,B+1)scanf("%lld",&b[i]);
  rep(i,M)scanf("%lld%lld%lld",&x[i],&y[i],&c[i]);
  rep(i,M)ans=MIN(ans,a[x[i]]+b[y[i]]-c[i]);
  FOR(i,1,A+1)mina=MIN(mina,a[i]);
  FOR(i,1,B+1)minb=MIN(minb,b[i]);
