...
static char a[101], b[101], c[101];
int main(void) {
	int al, bl, cl;

	scanf("%s", a); al = strlen(a); a[al] = 'A';
	scanf("%s", b); bl = strlen(b); b[bl] = 'B';
	scanf("%s", c); cl = strlen(c); c[cl] = 'C';
	...
	while (1) {
		switch (tmp) {
		case 'a':tmp = a[ap]; al--; ap++; break;
		case 'b':tmp = b[bp]; bl--; bp++; break;
		case 'c':tmp = c[cp]; cl--; cp++; break;
		default: printf("%c\n", tmp);
		     	 return 0;
		}
	}
