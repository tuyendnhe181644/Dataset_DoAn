...
ll mod_inv(ll x, ll mod, ll is_fac ){ makeinv(2*1e6,mod);  return is_fac?finv_arr[x]:inv_arr[x]; }
ll mod_fac(ll x, ll mod){  make_mod_fac(2*1e6,mod);  return mod_fac_arr[x];  }
...
int main(void){
    ...
    rep(i,0,k+1){
        ll t1=modpow(25,i,MOD1)*modpow(26,k-i,MOD1)%MOD1;
        ll t2=mod_fac(i+n-1,MOD1)*mod_inv(i,MOD1,1)%MOD1*mod_inv(n-1,MOD1,1)%MOD1;
        ans+=t1*t2%MOD1;
        ans%=MOD1;
    }
