...
ll m_mul4(ll a, ll b, ll c, ll d, ll mod){ return m_mul2(a*b%mod,c*d%mod,mod); }
ll m_mul5(ll a, ll b, ll c, ll d, ll e, ll mod){ return m_mul3(a*b%mod,c*d%mod,e,mod); }
...
int main(void){
    ...
    rep(i,0,k+1){
        ans+=m_mul5( m_p(25,i,MOD1) 
                    , m_p(26,k-i,MOD1)
                     , m_f(i+n-1,MOD1) 
                     , m_inv(i,MOD1,1)
                     , m_inv(n-1,MOD1,1) 
                     ,MOD1  );
        // ll t1=m_p(25,i,MOD1)*m_p(26,k-i,MOD1)%MOD1;
        // ll t2=m_f(i+n-1,MOD1)*m_inv(i,MOD1,1)%MOD1*m_inv(n-1,MOD1,1)%MOD1;
        // ans+=t1*t2%MOD1;
        ans%=MOD1;
    }
