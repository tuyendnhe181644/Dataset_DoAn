...
static uint64_t A[300000];
static uint64_t bitCnt[300000][60];
...
int main(void) {
    ...
    for (uint64_t ofs = 0; ofs < N; ofs++) {
        if (ofs != 0) {
            for (uint64_t b = 0; b < 60; b++) {
                bitCnt[N - 1 - ofs][b] = bitCnt[N - ofs][b];
            }
        }

        for (uint64_t b = 0; b < 60; b++) {
            if (((A[N - 1 - ofs] >> b)& UINT64_C(0x1)) != 0) {
                bitCnt[N - 1 - ofs][b]++;
            }
        }
    }
    ...
    for (uint64_t i = 0; i < N - 1; i++) {
        for (uint64_t b = 0; b < 60; b++) {
            const uint64_t bMod = (UINT64_C(0x1) << b) % MOD_VAL;
            if (((A[i] >> b)& UINT64_C(0x1)) == 0) {
                sum = (sum + bitCnt[i + 1][b]* bMod % MOD_VAL) % MOD_VAL;
            }
            else {
                sum = (sum + ((N - i - 1 - bitCnt[i + 1][b]) * bMod) % MOD_VAL) % MOD_VAL;
            }
        }
    }
