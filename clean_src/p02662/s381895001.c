...
static uint64_t dp[2][2][3001][3000];
static uint64_t A[3000];

int main(void) {
    uint64_t N, S;

    scanf("%"PRIu64"%"PRIu64, &N, &S);
    for (uint64_t i = 0; i < N; i++) {
        scanf("%"PRIu64, &A[i]);
    }
    ...
    for (uint64_t i = 0; i <= S; i++) {
        dp[0][0][i][0] = (i == 0 ? 1 : 0);
        dp[1][0][i][0] = (i == 0 ? 1 : 0);
        dp[1][1][i][0] = (i == A[0] ? 1 : 0);
    }

    for (uint64_t i = 1; i < N; i++) {
        for (uint64_t s = 0; s <= S; s++) {
            dp[0][0][s][i] = (dp[0][0][s][i - 1] + dp[1][0][s][i - 1] + dp[1][1][s][i - 1]) % MOD_VAL;
            dp[1][0][s][i] = (dp[0][0][s][i - 1] + dp[1][0][s][i - 1] + dp[1][1][s][i - 1]) % MOD_VAL;
            if (s < A[i]) {
                dp[1][1][s][i] = 0;
            }
            else {
                dp[1][1][s][i] = (dp[0][0][s - A[i]][i - 1] + dp[1][0][s - A[i]][i - 1] + dp[1][1][s - A[i]][i - 1]) % MOD_VAL;
            }
        }
    }

    printf("%"PRIu64"\n", (dp[0][0][S][N - 1] + dp[1][0][S][N - 1] + dp[1][1][S][N - 1]) % MOD_VAL);
