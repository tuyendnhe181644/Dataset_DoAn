...
void UFmake(ll n, UF b[]){ 
    rep(i,0,n){ b[i].par = i; b[i].num = i; b[i].hei = 1; b[i].size=1; }
}

ll UFroot(ll n, ll a,UF b[]){ return a==b[a].par?a:UFroot(n,b[a].par,b); }
ll UFsame(ll n, ll x, ll y, UF b[]){ return UFroot(n,x,b) == UFroot(n,y,b) ? 1 : 0; }
ll UFsize(ll n, ll x, UF b[]){
    return b[UFroot(n,x,b)].size;
}
void UFunite(ll n, ll x, ll y, UF b[]){
    ll xroot=UFroot(n,x,b) , yroot=UFroot(n,y,b) , siz=UFsize(n,x,b)+UFsize(n,y,b);
    if(b[xroot].hei > b[yroot].hei) b[yroot].par = xroot;
    else{ b[xroot].par = yroot;  b[yroot].hei = max2( b[yroot].hei , b[xroot].hei+1 );}
    b[UFroot(n,x,b)].size=siz;
}
...
int main(void){
    ...
    ans[m-1]=n*(n-1)/2;
    rrep(i,m-2,0){
        if( UFsame(n,a[i+1]-1,b[i+1]-1,br) ){
            ans[i]=ans[i+1];
        }else{
            ans[i]=ans[i+1]-UFsize(n,a[i+1]-1,br)*UFsize(n,b[i+1]-1,br);
            UFunite(n,a[i+1]-1,b[i+1]-1,br);
        }
    }
    rep(i,0,m){
        printf("%lld\n",ans[i]);
