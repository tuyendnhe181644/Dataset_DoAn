...
int main()
{
	...
	N = in();
	bp = block; for (i = 0; i < N; i++) {
		bp->id = i, bp->ab = a[i] = in(), bp->f = 0, bp++;
		bp->id = i, bp->ab = b[i] = in(), bp->f = 1, bp++;
	}
	sz = bp - block;

	qsort(block, sz, sizeof(T), cmp);

	V = (N + sz) + 2;
	k = 0, bp = block; for (i = 0; i < sz; i++) {
		if (i > 0 && bp->ab != (bp-1)->ab) k = i;
		j = bp->id;
		if (bp->f) add_edge(k+2, sz+2+j, 1, -a[j]);
		else       add_edge(k+2, sz+2+j, 1, -b[j]);
		bp++;
	}
