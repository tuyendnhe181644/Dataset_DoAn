...
void check_lim(int i)
{
	if (hi[i] < lim[i]) return;
	if (lim[i] == 0) {
		lim[i] = 16, hi[i] = 0;			// 16 ?
...
void segmentArrangement(int n, PP start, PP goal)
{
	...
	for (i = 0; i < n; i++) for (j = 0; j < i; j++) {
		if (intersect(line[i], line[j])) tbl[sz++] = crossPoint(&k, line[i], line[j]);
	}
...
double dijkstra(PP start, PP goal)
{
	...
	while (qsize) {
		...
		for (i = 0; i < hi[s]; i++) {
			e = to[s][i], nt = t + vabs(vsub(tbl[s], tbl[e]));
