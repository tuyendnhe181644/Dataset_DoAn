...
ll factor_pre(){  rep(i,0,1){ if(is_factor_prepared++) return 0; }  ll tmp=(1e6)+5, fac_tmp[tmp];  rep(i,0,tmp){fac_tmp[i]=i?2*i+1:2;}  rep(i,1,tmp){if(fac_tmp[i]){  repp(j,3,tmp/(2*i+1)+1,2 ){  if( j*(2*i+1)<tmp ) fac_tmp[ (j*(2*i+1)-1)/2 ]=0;  }  }else continue;}  rep(i,0,tmp){if(fac_tmp[i]){  rep(j,0,2){ factors[j] = realloc( factors[j] , sizeof(ll)*( fac_cnt +1 ) );  factors[j][j?fac_cnt++:fac_cnt]=j?0:fac_tmp[i];  }  } }  return 0;  }
...
int main(void){
    ...
    // ll a++
    // scanf("%s",s);
    rep(i,0,1e7+3) ppp[i][0]=1;
    rep(i,0,1e7+3) ppp[i][1]=i;
    // ll [n];
    factor_pre();
    rep(i,0,fac_cnt){
        ...
        repp(j,factors[0][i],1e7+3,factors[0][i]){
            ll t=j,p=0;
            while(ppp[j][1]%factors[0][i]==0){
                ppp[j][1]/=factors[0][i]; p++;
            }
            ppp[j][0]*=p+1;
        }
    }
