...
void clbig(long long ver,long long pc,long long col[],rs g[],mkj x[]){
  long long i;
  if(col[ver]!=0){return;}
  col[ver]=3-pc;
...
int main(void){
  resuf();
  long long i,j,n,m,d[524288],k,a[524288],b[524288],c,h,w,r=0,l,t;
  long long res[524288];
  long long rc[524288]={0};
  long long vf[524288];
  sd dat[524288];
  rs g[524288];
  mkj x[524288];
  ...
  for(i=0;i<m;i++){
    t=dat[i].node;
    if(find(a[t])==find(b[t])){continue;}
    uni(a[t],b[t]);
    res[t]=dat[i].val;
    vf[a[t]]=llmin(dat[i].val,vf[a[t]]);
    vf[b[t]]=llmin(dat[i].val,vf[b[t]]);
    g[c].st=a[t];
    g[c].fi=b[t];
    g[c].kr=1;
    c++;
