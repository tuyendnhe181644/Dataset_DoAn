...
static bool containsDislikeValue(uint64_t val, const uint64_t listSize, const uint64_t* const dislikeList) {
    while (0 < val) {
        const uint64_t digit = val % 10;

        for (uint64_t i = 0; i < listSize; i++) {
            if (digit == dislikeList[i]) {
                return true;
            }
        }
...
int main(void) {
    ...
    while (true) {
        if (!containsDislikeValue(payFee, K, D)) {
            break;
        }
