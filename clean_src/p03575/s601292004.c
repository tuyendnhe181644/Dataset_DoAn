...
void dfs(long vertex,long habuku);
...
long how_tyouten;
long how_hen;
long hen_data[100][2]={{0}};
bool flag[100]={0};


int main(void){
    //グラフが連結かどうかを、1本ずつ除いて見る
    scanf("%ld %ld\n",&how_tyouten,&how_hen);
    input_array2(how_hen,2,hen_data);
    //0-indexにする
    for(loop1=0;loop1<how_hen;loop1++){
        hen_data[loop1][0]--;
        hen_data[loop1][1]--;
    }
    
    //連結かを判定するにはどうすればいいか？
    //深さ優先探索とかいうのをやってみようかな
    //参考文献
    //http://www.hongo.wide.ad.jp/~jo2lxq/dm/lecture/07.pdf
    //http://wikiwiki.jp/kut-pg/?%A5%A2%A5%EB%A5%B4%A5%EA%A5%BA%A5%E0%CA%D9%B6%AF%B2%F1%2F%C3%B5%BA%F7
    ...
    for(loop1=0;loop1<how_hen;loop1++){
        ...
        dfs(0,loop1);
        for(loop2=0;loop2<how_tyouten;loop2++){
            if(flag[loop2]==0){
                ans++;
                //printf("%ld\n",loop2);
                break;
...
void dfs(long vertex,long habuku){
    //既に到達済み
    if(flag[vertex]){
        return; //再帰のアレをアレするためのアレ
    }
    
    //到達済みとして記憶
    flag[vertex]=1;
    
    //vertexと連結している頂点へ訪問、ただしhabukuをはぶく
    for(int i=0;i<how_hen;i++){
        if(i!=habuku){
            if(hen_data[i][0]==vertex){
                dfs(hen_data[i][1],habuku);
            }else if(hen_data[i][1]==vertex){
                dfs(hen_data[i][0],habuku);
            }
        }
    }
    return ;
