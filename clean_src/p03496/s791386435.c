...
int main(void){
    //絶対値が一番大きいものを振り回せばいいのでは？
    In(how_data);
    long data[how_data];
    input_array(how_data,data);
    
    long max=array_max(how_data,data);
    long min=array_min(how_data,data);
    ...
    }else if(max<=0){//全てが0以下である
        printf("%ld\n",how_data-1);
        for(loop1=how_data;loop1>1;loop1--){
            printf("%ld %ld\n",loop1,loop1-1);
        }
    }else if(max>=labs(min)){ //正負混ざってて全部正にする場合
        printf("%ld\n",how_data-1+how_data);
        for(loop1=0;loop1<how_data;loop1++){
            printf("%ld %ld\n",array_search(how_data,data,max)+1,loop1+1);
        }
        for(loop1=0;loop1<how_data-1;loop1++){
            printf("%ld %ld\n",loop1+1,loop1+2);
        }
    }else if(max<labs(min)){ //正負混ざってて全部負にする場合
        printf("%ld\n",how_data-1+how_data);
        for(loop1=0;loop1<how_data;loop1++){
            printf("%ld %ld\n",array_search(how_data,data,min)+1,loop1+1);
        }
        for(loop1=how_data;loop1>1;loop1--){
            printf("%ld %ld\n",loop1,loop1-1);
        }
