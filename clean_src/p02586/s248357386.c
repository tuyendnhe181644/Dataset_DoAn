...
static uint64_t dp[3000][3000][4], vMap[3000][3000];

int main(void) {
    uint64_t R, C, K;

    scanf("%"PRIu64"%"PRIu64"%"PRIu64, &R, &C, &K);
    ...
    for (uint64_t i = 0; i < K; i++) {
        uint64_t r, c, v;
        scanf("%"PRIu64"%"PRIu64"%"PRIu64, &r, &c, &v);
        ...
        for (uint64_t c = 0; c < C; c++) {
            for (uint64_t n = 0; n <= 3; n++) {
                if (n == 0) {
                    ...
                    if (0 < c) {
                        dp[r][c][n] = MAX(dp[r][c][n], MAX(dp[r][c - 1][n], dp[r][c - 1][n - 1] + vMap[r][c]));
                    }

                    if (0 < r) {
                        dp[r][c][n] = MAX(dp[r][c][n], dp[r - 1][c][3] + vMap[r][c]);
                    }
                }
