...
void solve(){
    ...
    rep(i,0,r){
        rep(j,0,c){
            ...
            rep(k,0,4){
                dp[i+1][j+1][0]= max2(dp[i+1][j+1][0],dp[i][j+1][k]) ;
                dp[i+1][j+1][1]= max2(dp[i+1][j+1][1],dp[i][j+1][k]+it[i][j]) ;
            }

            rep(k,0,4){
                dp[i+1][j+1][k]= max2(dp[i+1][j+1][k],dp[i+1][j][k]) ;
            }
            rep(k,0,3){
                dp[i+1][j+1][k+1]= max2( dp[i+1][j+1][k+1] , dp[i+1][j][k]+it[i][j] );
            }
            // rep(k,0,4){
            //     // dp[i+1][j+1][k]= dp[i+1][j][k];
            //     dp[i+1][j+1][k]= max2(dp[i+1][j+1][k],dp[i+1][j][3]) ;
            //     dp[i+1][j+1][k]= max2(dp[i+1][j+1][k],dp[i+1][j][3]) ;
            // }
            // rep(l,0,3){
            //     dp[i+1][j+1][l+1]= max2( dp[i+1][j+1][l+1] , dp[i+1][j][l]+it[i][j] );
            //     dp[i+1][j+1][l+1]= max2( dp[i+1][j+1][l+1] , dp[i][j+1][l]+it[i][j] );
            // }
        }
        // rep(j,0,c){
        //     // rep(k,0,4){
        //         dp[i+2][j+1][0]=dp[i+1][j+1][3];
        //     // }
        // }
    }
    // rep(i,0,r){
