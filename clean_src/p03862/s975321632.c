...
int main(void){
    ...
    for(loop1=1;loop1<how_data;loop1++){
        if(data[loop1]+data[loop1-1]>target){
            long over=(data[loop1]+data[loop1-1]-target);
            if(over<=data[loop1]){
                ans+=(data[loop1]+data[loop1-1]-target);
                data[loop1]-=(data[loop1]+data[loop1-1]-target);
            }else{
                ans+=(data[loop1]+data[loop1-1]-target);
                data[loop1]=0;
                data[loop1-1]=target;
