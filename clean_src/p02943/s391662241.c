...
char smaybe[N_MAX], stmp[N_MAX];

void make (sll l, sll r) {
	ull remain = k;
	char cmin = s[l];
	sll i;
		...
		if (!remain) {
			...
			for (i = 0; i < n; i++) {
				...
				if (v < n) {
					...
					stmp[n - 1 - i] = smaybe[n - 1 - (v - n)];
				}
			}

			if (r == n) {
				l = smax((sll)n - (r - l) * 2, 0);
			} else {
				l = (sll)n - (r - l);
				r = n;
			}
...
ull solve () {
	...
	if (k >= 13) {
		...
		while (i < n) {
			...
			strcpy(smaybe, s);
			make(j, i);
			if (strcmp(smaybe, t) < 0) strcpy(t, smaybe);
		}
	}
	puts(t);
