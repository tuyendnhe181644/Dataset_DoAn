...
static uint64_t seq[200000];

static uint64_t mg(const uint64_t v1, const uint64_t v2, uint64_t* cnt) {
    (*cnt)++;
...
static uint64_t shrink(const uint64_t d, uint64_t c, uint64_t* cnt) {
    if (d == 0) {
        *cnt += c - 1;
        return 0;
    }
        ...
        if (2 <= c) {
            *cnt += c / 2;
            const uint64_t sv = shrink(2, c / 2, cnt);
        ...
        if (2 <= c) {
            *cnt += c / 2;
            const uint64_t sv = shrink(4, c / 2, cnt);
        ...
        if (2 <= c) {
            *cnt += c / 2;
            const uint64_t sv = shrink(6, c / 2, cnt);
        ...
        if (2 <= c) {
            *cnt += c / 2;
            const uint64_t sv = shrink(8, c / 2, cnt);
        ...
        if (2 <= c) {
            *cnt += c / 2 * 2;
            const uint64_t sv = shrink(1, c / 2, cnt);
        ...
        if (2 <= c) {
            *cnt += c / 2 * 2;
            const uint64_t sv = shrink(3, c / 2, cnt);
        ...
        if (2 <= c) {
            *cnt += c / 2 * 2;
            const uint64_t sv = shrink(5, c / 2, cnt);
        ...
        if (2 <= c) {
            *cnt += c / 2 * 2;
            const uint64_t sv = shrink(7, c / 2, cnt);
        ...
        if (2 <= c) {
            *cnt += c / 2 * 2;
            const uint64_t sv = shrink(9, c / 2, cnt);
...
int main(void) {
    ...
    for (uint64_t i = 0; i < M; i++) {
        uint64_t d, c;
        scanf("%"PRIu64"%"PRIu64, &d, &c);

        seq[i] = shrink(d, c, &sum);
    }

    for (uint64_t i = 1; i < M; i++) {
        seq[i] = mg(seq[i], seq[i - 1], &sum);
    }
