...
static void uf_Merge(const size_t a, const size_t b) {
    ...
    if (aRoot != bRoot) {
        ...
        uf_Node[bRoot].size += uf_Node[aRoot].size;
    }
}
...
static PList fList[100000];
static PList bList[100000];

static void List_Add(PList* l, const uint64_t idx, const uint64_t no) {
    if (l[idx].maxSize == 0) {
        l[idx].no = (uint64_t*)malloc(sizeof(uint64_t) * LIST_INIT_SIZE);
        l[idx].size = 0;
        l[idx].maxSize = LIST_INIT_SIZE;
    }
    else if (l[idx].size == l[idx].maxSize) {
        l[idx].no = (uint64_t*)realloc(l[idx].no, sizeof(uint64_t) * l[idx].maxSize*2);
        l[idx].maxSize *= 2;
    }

    l[idx].no[l[idx].size] = no;
    l[idx].size++;
}
...
int main(void) {
    ...
    scanf("%"PRIu64"%"PRIu64"%"PRIu64, &N, &M, &K);
    ...
    for (uint64_t i = 0; i < M; i++) {
        ...
        List_Add(fList, A, B);
        List_Add(fList, B, A);
        uf_Merge((size_t)A, (size_t)B);
    }
    for (uint64_t i = 0; i < K; i++) {
        uint64_t C, D;
        scanf("%"PRIu64"%"PRIu64, &C, &D);
        C--; D--;
    ...
    for (uint64_t i = 0; i < N; i++) {
        const size_t selfRoot = uf_GetRoot((size_t)i);
        uint64_t numCandidate = uf_Node[selfRoot].size - 1;

        for (uint64_t f = 0; f < fList[i].size; f++) {
            if (uf_GetRoot((size_t)(fList[i].no[f])) == selfRoot) {
                numCandidate--;
            }
        }
        for (uint64_t b = 0; b < bList[i].size; b++) {
            if (uf_GetRoot((size_t)(bList[i].no[b])) == selfRoot) {
                numCandidate--;
            }
        }

        printf("%"PRIu64" ", numCandidate);
    }
