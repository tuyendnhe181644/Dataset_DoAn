	...
	while (N = in()) {
	    ...
	    for (s = 0; s < 1<<M; s++) {
			...
			while (lb + 1 < ub) {
				mi = (lb + ub) >> 1;
				make_edge(s, mi);
				if ((t1 = minCostFlow(source, sink, N) + mi*B*cnt) < 0) goto next;
				make_edge(s, mi-1);
				if ((t2 = minCostFlow(source, sink, N) + (mi-1)*B*cnt) < 0) goto next;
