...
short hi[1002], *to[1002], *no[1002]; char *re[1002];
short dist[1002][1002][2];
...
int dijkstra(int *rno, int start, int goal)
{
	...
	while (qsize) {
		...
		if (r == 0) {
			for (i = 0; i < hi[s]; i++) {
				e = to[s][i], k = no[s][i], f = re[s][i];
				if (f == 0) {
					if (dist[e][k][0] > t+1) dist[e][k][0] = t+1, enq(e, t+1, 0);
				} else {
					if (dist[s][k][1] > t+1) dist[s][k][1] = t+1, enq(e, t+1, k);
				}
			}
		} else {
			for (i = 0; i < hi[s]; i++) {
				e = to[s][i], k = no[s][i], f = re[s][i];
				if (f) continue;
				if (dist[e][k][1] > t+1) dist[e][k][1] = t+1, enq(e, t+1, r);
			}
		}
	}
	return -1;
...
int main()
{
	...
	while (N = in()) {
		...
		j = 0; for (i = 1; i <= M; i++) {
			a = memo[j++], b = memo[j++];
			k = hi[a]++, to[a][k] = b, no[a][k] = i, re[a][k] = 0;
			k = hi[b]++, to[b][k] = a, no[b][k] = i, re[b][k] = 1;
		}
		free(memo);
