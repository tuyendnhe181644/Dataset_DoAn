...
signed main(){
	...
	build_tree(t);
	int *black = (int *)malloc(sizeof(int) * N);
	int *white = (int *)malloc(sizeof(int) * N);
	tree_vertex *nowv;
	for(i = N - 1; i >= 0; i--){
		nowv = t->sorted_v[i];
		black[nowv->num] = 1;
		...
		for(j = 0; j < nowv->chilnum; j++){
			black[nowv->num] = MOD(black[nowv->num] * white[nowv->children[j]->num]);
			white[nowv->num] = MOD(white[nowv->num] * (black[nowv->children[j]->num] + white[nowv->children[j]->num]));
		}
	}
	printf("%lld\n", MOD(black[0] + white[0]));
	return 0;
