...
int main(){
    ...
    int min = abs_val(753, (s[0] - '0') * 100 + (s[1] - '0') * 10 + (s[2] - '0'));
    for(int i = 1; i + 2 < strlen(s); ++i){
        if(min > abs_val(753, (s[i] - '0') * 100 + (s[i+1] - '0') * 10 + (s[i+2] - '0'))){
            min = abs_val(753, (s[i] - '0') * 100 + (s[i+1] - '0') * 10 + (s[i+2] - '0'));
        }
    }
