...
int main(void){
    ...
    rep(i,2,n-1){
        ll l1=1,r1=i-1,l2=i+1,r2=n-1;
        ll piv1[2],piv2[2];
        piv1[0]=upper_bound(sum,l1,r1,sum[i]/2,2);
        if(piv1[0]==-1){
            piv1[1]=lower_bound(sum,l1,r1,sum[i]/2,2);
        }else piv1[1]=piv1[0]+1;

        piv2[0]=upper_bound(sum,l2,r2,(sum[n]-sum[i])/2+sum[i],2);
        if(piv2[0]==-1){
            piv2[1]=lower_bound(sum,l2,r2,(sum[n]-sum[i])/2+sum[i],2);
        }else piv2[1]=piv2[0]+1;
        // printf("%lld\n",i);
        // printf("%lld %lld %lld %lld\n",piv1[0],piv1[1],piv2[0],piv2[1]);
        rep(j,0,2){
            if(piv1[j]==-1) continue;
            rep(k,0,2){
                if(piv2[k]==-1) continue;
                ll num[4];
                num[0]=sum[piv1[j]];
                num[1]=sum[i] - sum[piv1[j]];
                num[2]=sum[piv2[k]] - sum[i];
                num[3]=sum[n] - sum[piv2[k]];
                // printf("%lld ",num[0]);
                // printf("%lld ",num[1]);
                // printf("%lld ",num[2]);
                // printf("%lld\n",num[3]);
                sortup(num,4);
                ans = min2(ans, ABS(num[0]-num[3]) );
            }
        }
        // printf("||%lld||\n",ans );
    }
