...
ull solve () {
	...
	for (i = 2; i + 2 <= n; i++) {
		...
		ull rsum = acc[n - 1] - lsum;
		ok = i - 1;
		ng = n - 1;
		while (ok + 1 < ng) {
			sll med = (ok + ng) / 2;
			ull psum = acc[med] - acc[i - 1];
			if (psum * 2 < rsum) {
				ok = med;
			} else {
				ng = med;
		...
		// printf("%llu,%llu,%llu,%llu\n", hoge[0], hoge[1], hoge[2], hoge[3]);

		qsort(hoge, 4, sizeof(ull), pullcomp);
		maybe = hoge[3] - hoge[0];
		if (i == 2 || maybe < result) result = maybe;
	}
