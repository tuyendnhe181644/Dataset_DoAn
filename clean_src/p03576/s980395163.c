...
int main(void){
    ...
    //長方形のサイズを小さくするにはどうするのっと
    //この長方形のサイズは、選択した点の(xの最大-xの最小)*(yの最大-yの最小)
    //総当たりだとどうなる？ ncr(50,25)=126410606437752だから無理
    //あ、まてよ 点の数はそんな大きくない
    //で、how_mustを満たした面積の最小を求めればいいね
    
    long ans=9223372036854775807;   //signed longの最大
    
    for(loop1=0;loop1<how_ten-1;loop1++){ //loop1はx1
        for(loop2=0;loop2<how_ten-1;loop2++){ //loop2はy1
            for(loop3=loop1+1;loop3<how_ten;loop3++){ //loop3はx2
                for(loop4=loop2+1;loop4<how_ten;loop4++){ //loop4はy2
                    //条件を満たしているかチェック
                    //つまり、xがloop1～loop3の間かつyがloop2～loop4の間
                    long count=0;
