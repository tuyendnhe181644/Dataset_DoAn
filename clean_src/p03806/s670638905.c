...
int main(void){
    ...
    rep(i,0,siz) rep(j,0,siz) dp[i][j]=INF;
    dp[0][0]=0;
    rep(i,0,n){
        // rep(j,0,siz){
            // if(a[i]*j>=siz||b[i]*j>=siz) break;
            // ll aa=a[i]*j;
            // ll bb=b[i]*j;
            rrep(k,siz-1,0){
                rrep(l,siz-1,0){
                    if(dp[k][l]>1e15) continue;
                    if(k+a[i]>=siz||l+b[i]>=siz) continue;
                    dp[k+a[i]][l+b[i]]=min2(dp[k][l]+c[i],dp[k+a[i]][l+b[i]]);
                }
            }
        // }
    }
    rep(i,1,siz){
        if(ma*i>=siz||mb*i>=siz) break;
        if(dp[ma*i][mb*i]>1e15) continue;
        ans=min2(dp[ma*i][mb*i],ans);
    }
