...
sll seg[1234567], base = 524288;
sll seg_add (ull index, sll x) {
	index += base;
	while (index) {
		seg[index] += x;
		index /= 2;
...
sll seg_find_ (ull l, ull r, ull xl, ull xr, ull i) {
	if (r <= xl || xr <= l) return 0;
	if (l <= xl && xr <= r) return seg[i];

	ull med = (xl + xr) / 2;
	sll vl = seg_find_(l, r, xl, med, i * 2);
	sll vr = seg_find_(l, r, med, xr, i * 2 + 1);
	return vl + vr;
}
