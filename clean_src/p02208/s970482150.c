...
int main(void){
  heap=malloc(sizeof(data)*4194304);
  long long x,y,z,n,m,s,t,i,j,nd,res=llinf;
  long long *a=malloc(sizeof(long long)*524288);
  long long *b=malloc(sizeof(long long)*524288);
  long long *c=malloc(sizeof(long long)*524288);
  long long *d=malloc(sizeof(long long)*524288);
  long long *df=malloc(sizeof(long long)*524288);
  long long fc=0,tc=0;
  rs *gf=malloc(sizeof(rs)*524288);
  mkj *xf=malloc(sizeof(mkj)*524288);
  scanf("%lld%lld%lld%lld%lld%lld%lld",&x,&y,&z,&n,&m,&s,&t);
  for(i=1;i<=n;i++){
    if(i==s){nd=0;}else{nd=1;}
  ...
  qsort(gf,fc,sizeof(gf[0]),sortfnc);
  makemkj(gf,xf,fc);
  dijkstra(b[s],500000,df,gf,xf);
  res=df[d[t]+x+y];
