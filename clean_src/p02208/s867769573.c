...
ull solve () {
	...
	qsort(xy, (xn + xm) * 2, sizeof(hwll), phwllABcomp);

	i = j = 0;
	while (i <= n) {
		gin[i] = j;
		while (j < (xn + xm) * 2 && xy[j].a == i) j++;
		i++;
	}

	sll e1 = xs - 1, e2 = xn + xt - 1;

	result = VERYBIG;

	dijk(a[e1]);
	result = smin(result, smin(d[a[e2]], d[b[e2]]));
	dijk(b[e1]);
	result = smin(result, smin(d[a[e2]], d[b[e2]]));
...
int32_t main (int argc, char *argv[]) {
	...
	scanf("%lld%lld%lld%lld", &xn, &xm, &xs, &xt);
	for (i = 0; i < xn; i++) {
