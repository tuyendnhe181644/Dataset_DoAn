...
// init with
ll ans[2541];
typedef struct{ ll from , to , cost; } graph_info;

graph_info edge[5412];
//https://qiita.com/wakimiko/items/69b86627bea0e8fe29d5
ll Bellman_Ford(ll start, ll vertex , ll edges){
    rep(i,0,vertex) ans[i]=INF;
    ll cnt=0,update=1;
    ans[start] = 0;

    rep(i,0,vertex){
        rep(j,0,edges){
            if( ans[edge[j].from]==INF ) continue;
            ans[ edge[j].to ] = min2( ans[ edge[j].to ] , ans[edge[j].from] + edge[j].cost );
        }
        // rep(j,0,vertex){
        //     printf("%lld ",ans[j]);
        // }PN;
    ...
    rep(i,0,vertex){
        rep(j,0,edges){
            if( ans[edge[j].from]==INF ) continue;
            if( ans[ edge[j].to ] > ans[edge[j].from] + edge[j].cost ){
                ans[ edge[j].to ] = -INF;
            }
        }
        // rep(j,0,vertex){
    ...
    // while(update){
    //     update = 0;
    //     //負の閉路があってもv-1の頂点に影響なければセーフ
    //     if(cnt++>vertex) return 1;
    //     // if(cnt++==vertex) return 0;
    //     rep(i,0,edges){
    //         if( ans[edge[i].from]!=INF  &&  ans[ edge[i].to ] > ans[edge[i].from] + edge[i].cost ){
    //             if(cnt>vertex && edge[i].to==vertex-1 ) return 0;
    //             update=1;
    //             ans[ edge[i].to ] = ans[edge[i].from] + edge[i].cost;
    //             // if(cnt==vertex) return 0;
    //         }
    //     }
    //     // cnt++;
...
// long long Bellman_Ford(long long a,long long b,long long n,long long m,rs g[],mkj x[]){
//   long long i,j,dist[524288];
//   for(i=0;i<=n;i++){dist[i]=llinf;}
//   dist[a]=0;
//   for(i=0;i<n;i++){
//     for(j=0;j<m;j++){
//       if(dist[g[j].st]==llinf){continue;}
//       dist[g[j].fi]=llmin(dist[g[j].st]+g[j].kr,dist[g[j].fi]);
//       //printf("%lld:%lld\n",g[j].fi,dist[g[j].fi]);
//     }
//   }
//   for(i=0;i<n;i++){
//     for(j=0;j<m;j++){
//       if(dist[g[j].st]==llinf){continue;}
//       if((dist[g[j].st]+g[j].kr)<dist[g[j].fi]){
//         dist[g[j].fi]=-llinf;
//       }
//     }
//   }
