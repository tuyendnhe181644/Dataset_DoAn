...
long long n;
long long c[524288];
long long tc,cnt[524288]={0};
long long res[524288]={0};
void rep(long long v,long long pv,rs g[],mkj x[]){
  long long i,j,st,vv;
  tc++;
  st=tc-cnt[c[v]];
...
int main(void){
  ...
  g[2*n-2].st=0;g[2*n-2].fi=1;
  g[2*n-1].st=1;g[2*n-1].fi=0;
  qsort(g,2*n,sizeof(g[0]),sortfnc);
  makemkj(g,x,2*n);
  tc=0;
  rep(0,-1,g,x);
  for(i=1;i<=n;i++){
