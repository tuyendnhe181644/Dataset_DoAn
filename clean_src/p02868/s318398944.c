...
int pid=0,pnum=0,rpid[NUM];LL cost[NUM],rcost[NUM],cpid[NUM],que[NUM];
...
int main(){
  int N,M,i,ans=0;
  scanf("%d%d",&N,&M);
  int L[N+M-1],R[N+M-1],C[N+M-1];
  FOR(i,0,N-1)L[i]=i+2,R[i]=i+1,C[i]=0;
  FOR(i,N-1,N+M-1)scanf("%d%d%d",&L[i],&R[i],&C[i]);
  Dijkstra(N,N+M-1,L,R,C,1);
