...
int string_distance(char *s1, int w1, char *s2, int w2)
{
	...
	i = w1+1; while (i--) dp[i][0] = i;
	i = w2+1; while (i--) dp[0][i] = i;
...
int main()
{
	...
	source = 0, sink = sz + M + 1, V = sink + 1;

	for (i = 0; i < sz; i++) {
		add_edge(source, i+1, 1, 0);
		for (j = 0; j < M; j++) {
			k = web[i].c * string_distance(web[i].s, web[i].w, dic[j].s, dic[j].w);
			add_edge(i+1, sz+j+1, 1, k);
			if (i == 0) add_edge(sz+j+1, sink, 1, 0);
		}
	}
