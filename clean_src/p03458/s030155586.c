...
void cha(ll *a, ll *b, ll k, ll white){
    if(white==0){
        *a-=k;
    }
    ll x=mod_p(*a,k);
    ll y=mod_p(*b,k);
    if( llabs((*a+*b)-(x+y))/k %2 ){
        y+=k;
    }
    *a=x;
    *b=y;
...
void solve(){
    ...
    // rep(i,0,k*2){
    //     rep(j,0,k){
    //         printf("%lld ",pre_rui[i][j]);
    //     }PN;
    // }
    rui_make(2*k,k);
    // rep(i,0,k*2+1){
    //     rep(j,0,k+1){
    //         printf("%lld ",ruiseki[i][j]);
    //     }PN;
    // }
    rep(i,0,2*k){
        rep(j,0,k){
            ll val=0;
            val+=rui_get(i+k,j-k,i+k+k,j,2*k,k);
            // printf("%lld\n",val);
            val+=rui_get(i,j,i+k,j+k,2*k,k);
            // printf("%lld\n",val);
            val+=rui_get(i-k,j-k,i,j,2*k,k);
            // printf("%lld\n",val);
            val+=rui_get(i-2*k,j,i-k,j+k,2*k,k);
            // printf("%lld\n",val);
            ans=max(ans,val);
        }
        // PN;
