...
int main(void){
  ...
  sb[n]=b[n];
  qsort(sa,n+1,sizeof(int),sortfncsj);
  qsort(sb,n+1,sizeof(int),sortfncsj);
  for(i=0;i<=n;i++){
    if(sa[i]!=sb[i]){printf("-1\n");return 0;}
  }
  coordinate_comp(a,n+1);
  coordinate_comp(b,n+1);
  for(i=0;i<=n;i++){
    if(a[i]==b[i]){continue;}
    r++;
    uni(a[i],b[i]);
