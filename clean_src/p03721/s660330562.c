...
static NumVal numVal[100000];

static int compare_f(const void* a, const void* b) {
    return ((NumVal*)a)->a < ((NumVal*)b)->a ? -1 : 1;
}
...
int main(void) {
    ...
    for (uint64_t i = 0; i < N; i++) {
        scanf("%"PRIu64"%"PRIu64, &numVal[i].a, &numVal[i].b);
    }

    qsort(numVal, (size_t)N, sizeof(NumVal), compare_f);

    uint64_t sum = 0;

    for (uint64_t i = 0; i < N; i++) {
        if (sum < K && K <= sum + numVal[i].b) {
            printf("%"PRIu64"\n", numVal[i].a);
            break;
        }
