...
ll factor(ll n, ll new_common_plus){ rep(i,0,1){if(!is_factor_prepared){factor_pre();is_factor_prepared=1;}}  rep(i,0,fac_cnt){ ll cnt=0; rep(j,0,1){ while( ( cnt+= n %factors[0][i]==0 ? 1 : 0 )  && (n/=factors[0][i]) %factors[0][i]==0 ) continue; } factors[1][i]= new_common_plus==1 ? cnt :  new_common_plus==2 ? max2(factors[1][i],cnt) : factors[1][i]+cnt ;  if( factors[0][i]> n ) break; } return n; }
ll judge_prime(ll n){ rep(i,0,1){if(!is_factor_prepared){factor_pre();is_factor_prepared=1;}} rep(i,0,fac_cnt){ if(n<factors[0][i]*factors[0][i] || n==factors[0][i]) break; else if(n%factors[0][i]==0) n/=n; }  return n==1?0:1; }
...
int main(void){
    ...
    rep(j,0,fac_cnt){
        // printf("%lld ",factors[1][j]);
        if(factors[1][j]){
            ans=ans/factors[0][j]*(factors[0][j]-1);
        }
    }
    printf("%lld\n",ans);
    return 0;
