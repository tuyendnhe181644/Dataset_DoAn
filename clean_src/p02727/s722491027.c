...
static uint64_t p[100000], q[100000], r[100000];

int main(void) {
    uint64_t X, Y, A, B, C;

    scanf("%"PRIu64"%"PRIu64"%"PRIu64"%"PRIu64"%"PRIu64, &X, &Y, &A, &B, &C);
    ...
    qsort(p, (size_t)A, sizeof(uint64_t), compare_u64_desc);
    qsort(q, (size_t)B, sizeof(uint64_t), compare_u64_desc);
    qsort(r, (size_t)C, sizeof(uint64_t), compare_u64_desc);

    uint64_t cSum = 0;
    uint64_t cCnt = MIN(X + Y, C);
    ...
    uint64_t bSeq = 0;
    uint64_t abSum = 0;
    for (uint64_t abCnt = 0; abCnt <= X + Y; abCnt++) {
        if (abCnt + cCnt == X + Y) {
            if (sumMax < abSum + cSum) {
                sumMax = abSum + cSum;
            }
