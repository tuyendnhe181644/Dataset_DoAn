...
ull solve () {
	...
	qsort(b, n + 1, sizeof(ull), pullcomp);
	i = 0;
	while (i < n + 1) {
		j = i;
		while (j < n + 1 && b[j] == b[i]) j++;
	...
	for (i = 0; i < n; i++) {
		...
		for (j = 0; j < 26; j++) {
			...
			ull mj = seekdp(bccu);
			if (mapi[mj] != bccu) continue;

			dp[mi] = umin(dp[mi], dp[mj] + 1);
		}
	}

	result = dp[seekdp(a[n])];
