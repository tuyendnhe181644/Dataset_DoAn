...
ll dfs(ll st, ll x){
    // printf("%lld\n",x);
    if(d[x]){
        if(d[x]==st&&!dist[x]) return INF;
        else return dist[x];
    }
    ll ans=1;
    d[x]=st;
    rep(i,0,ed_cnt[x]){
        ll p=dfs(st,gr[x][i*2]);
        ans= max2(ans,p+1);
    }
    dist[x]=ans;
    return ans;
...
int main(void){
    ...
    rep(i,0,n){
        rep(j,0,n-1){
            cin(&a[i][j]); a[i][j]--;
            if(!j) continue;
            ll v1= min2( a[i][j-1]*n+i , i*n+a[i][j-1] );
            ll v2= min2( a[i][j]*n+i , i*n+a[i][j] );
            pushGraph(v1,v2,1,1,1);
        }
    }
    rep(i,0,n-1){
        rep(j,i+1,n){
