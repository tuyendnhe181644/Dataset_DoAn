...
#define number_of_vertexes 1151151
...
void solve(){
    ...
    rep(i,0,n){
        ...
        rep(j,1,n-1){
            ll u,v;
            if(i>a[i][j-1]) u=a[i][j-1]*n+i;
            else u=i*n+a[i][j-1];
            
            if(i>a[i][j]) v=a[i][j]*n+i;
            else v=i*n+a[i][j];
            in[v]++;
            pushGraph(u,v,1,1,1 );
        }
    ...
    if(topological(n*(n-1)/2)){
        rep(i,0,n){
            rep(j,i+1,n){
                // printf("%lld ",d[i*n+j]);
                ans=max2(ans,d[i*n+j]);
            }
            // PN;
        }
    }else{
