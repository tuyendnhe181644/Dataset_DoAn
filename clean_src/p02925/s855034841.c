...
static uint64_t A[1000][999];
static uint64_t nextPos[1000];

static size_t Q[2][1000];
static size_t Q_Size[2] = { 0,0 };

static void Q_Add(const size_t qId, const size_t idx) {
    Q[qId][Q_Size[qId]] = idx;

    Q_Size[qId]++;
}

static void Q_Init(const size_t qId) {
    Q_Size[qId] = 0;
}
...
int main(void) {
    ...
    while (true) {
        ...
        for (size_t i = 0; i < Q_Size[day % 2]; i++) {
            size_t pair[2];

            pair[0] = Q[day % 2][i];
            pair[1] = (size_t)(A[pair[0]][nextPos[pair[0]]]);
            ...
            for (size_t i = 0; i < 2; i++) {
                if (nextPos[pair[i]] < N - 1) {
                    const size_t nextMatch = (size_t)(A[pair[i]][nextPos[pair[i]]]);
                    if (!matchMade[nextMatch] && pair[i] == A[nextMatch][nextPos[nextMatch]]) {
                        Q_Add((day + 1) % 2, pair[i]);

                        matchMade[pair[i]] = true;
                        matchMade[nextMatch] = true;
                    }
                }
            }
        }

        processed += Q_Size[day % 2] * 2;
