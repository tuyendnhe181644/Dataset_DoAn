...
bool gumozi(char *target){
    if(strlen(target)%2!=0)return 0;
    char a[300]={'\0'};
    char b[300]={'\0'};
    for(int loopA=0;loopA<strlen(target)/2;loopA++){
        a[loopA]=target[loopA];
        b[loopA]=target[strlen(target)/2+loopA];
    }
    //printf("a= %s b= %s\n",a,b);
...
int main(void){
    //ぐう文字はstrlenが偶数でなければならない
    char s[300]={'\0'};
    scanf("%s",s);
    s[strlen(s)-1]='\0';
    while(gumozi(s)!=1){
        //printf("%s is not gumozi\n",s);
        s[strlen(s)-1]='\0';
    }
    //printf("%s is gumozi\n",s);
