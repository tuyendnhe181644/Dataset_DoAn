...
void run (void) {
  i32 q;
  scanf ("%" SCNi32, &q);
  heap *small = new_binary_heap (sizeof (i32), cmp_max);
  heap *large = new_binary_heap (sizeof (i32), cmp_min);
  i64 sum_small = 0;
  i64 sum_large = 0;
  i64 sum_b = 0;
  while (q--) {
    ...
    if (op == 1) {
      ...
      sum_b += b;
      push (small, &a); sum_small += a;
      push (large, &a); sum_large += a;
      i32 x, y;
      top (small, &x);
      top (large, &y);
      if (x <= y) continue;
      pop (small, &x); sum_small -= x;
      pop (large, &y); sum_large -= y;
      push (small, &y); sum_small += y;
      push (large, &x); sum_large += x;
    } else {
      i32 x;
      top (small, &x);
      i64 f = sum_b + (sum_large - sum_small) / 2;
