...
void dfs(ll from, ll x){
    d[x]=1;
    rep(i,0,ed_cnt[x]){
        ll to=gr[x][i*2];
        if(to==from) continue;
        if(d[to]){
            if(set_count(x+m,to)) continue;
            ans[cnt][0]=x;
            ans[cnt][1]=to;
            cnt++;
            out[x]++;
...
int main(void){
    ...
    dfs2(-1,0);
    // rep(i,0,n){
    //     set_insert(m*3,ed_cnt[i]*POW(10,6)+i);
    // }
    // ll k=set_min(m*3)%POW(10,6);
    // dfs2(k);
    rep(i,0,m){
        printf("%lld %lld\n",ans[i][0]+1,ans[i][1]+1);
    }
