...
signed main(){
	...
	for(i = 1; i < N; i++){
		scanf("%lld%lld", &x, &y);
		set_edge_tree(x - 1, y - 1, 1, t);
	}
	build_tree(t);
	int *dp1 = (int *)malloc(sizeof(int) * N);
	int *dp2 = (int *)malloc(sizeof(int) * N);
	int **mul = (int **)malloc(sizeof(int *) * N);
	int **mul_rev = (int **)malloc(sizeof(int *) * N);
	tree_vertex *nowv;
	for(i = N - 1; i >= 0; i--){
		nowv = t->sorted_v[i];
		dp1[nowv->num] = 1;
		for(j = 0; j < nowv->chilnum; j++){
			dp1[nowv->num] = MOD(dp1[nowv->num] * (dp1[nowv->children[j]->num] + 1));
		}
		mul[nowv->num] = (int *)malloc(sizeof(int) * (nowv->chilnum + 2));
		mul[nowv->num][0] = 1;
		for(j = 1; j <= nowv->chilnum; j++){
			mul[nowv->num][j] = MOD(mul[nowv->num][j - 1] * (dp1[nowv->children[j - 1]->num] + 1));
		}
		mul_rev[nowv->num] = (int *)malloc(sizeof(int) * (nowv->chilnum + 2));
		mul_rev[nowv->num][nowv->chilnum + 1] = 1;
		for(j = nowv->chilnum; j > 0; j--){
			mul_rev[nowv->num][j] = MOD(mul_rev[nowv->num][j + 1] * (dp1[nowv->children[j - 1]->num] + 1));
		}
	}
	dp2[0] = 1;
	for(i = 0; i < N; i++){
		nowv = t->sorted_v[i];
		for(j = 0; j < nowv->chilnum; j++){
			dp2[nowv->children[j]->num] = MOD(dp2[nowv->num] * MOD(mul[nowv->num][j] * mul_rev[nowv->num][j + 2]) + 1);
		}
	}
	for(i = 0; i < N; i++){
		printf("%lld\n", MOD(dp1[i] * dp2[i]));
