...
ull solve () {
	...
	// ull twinsym = 0;
	// ull colsym = 0, rowsym = 0;
	// bool iscolsym = true, isrowsym = true;
	...
	// 		bool c2 = (s[i][w - 1 - j] == 'S');
	// 		bool c3 = (s[h - 1 - i][j] == 'S');
	// 		bool c4 = (s[h - 1 - i][w - 1 - j] == 'S');

	// 		bool isc = (c1 == c3) && (c2 == c4);
	// 		bool isr = (c1 == c2) && (c3 == c4);
	// 		iscolsym &= isc;
	// 		isrowsym &= isr;

	// 		ull columnpair = ((c1 && c3) ? 1 : 0) + ((c2 && c4) ? 1 : 0);
	// 		ull rowpair = ((c1 && c2) ? 1 : 0) + ((c3 && c4) ? 1 : 0);
	...
	// // printf("twin: %llu, col: %llu, row: %llu\n", twinsym, colsym, rowsym);
	// result = (iscolsym ? 0 : vua) + (isrowsym ? 0 : vub);
	// result += (vua + vub + umax(vua, vub)) * twinsym;
	// result += umax(vua * (colsym), vub * (rowsym));
	...
	bool isleft = true;
	for (i = 0; i <= noon; i++) {
		if (times[i] > 2) goto fail;
		if (i == 0 || i == noon) {
			if (times[i] > 1) goto fail;
			if (times[i]) dpos[i] = true;
		} else {
			ull left = i;
			ull right = noon * 2 - i;
			if (times[i] == 2) {
				dpos[left] = dpos[right] = true;
			} else if (times[i]) {
				dpos[isleft ? left : right] = true;
				isleft = !isleft;
			}
		}
	}
