...
int playerComp(player_t x,player_t y) {
	if(x.fishNum>y.fishNum)return 1;
	if(x.fishNum<y.fishNum)return -1;
...
RedBlackTreeNode* RedBlackTreeNode_new(player_t key) {
...
int RedBlackTree_insertInternal(RedBlackTreeNode** now,player_t key) {
...
void RedBlackTree_insert(player_t key) {
	RedBlackTree_insertInternal(&root,key);
	root->isRed=0;
}

RedBlackTreeNode* RedBlackTree_searchPtr(RedBlackTreeNode* now,player_t key) {
...
int RedBlackTree_remove(player_t key) {
...
player_t RedBlackTree_getMax(void) {
	player_t nothing={1,0};
	RedBlackTreeNode* max=RedBlackTree_searchMaxNode(root);
	return max==NULL?nothing:max->key;
}

int player_num[1000001];

int main(void) {
	int n,q;
	int i;
	int a,v;
	player_t playerBuffer;
	scanf("%d%d",&n,&q);
	for(i=0;i<q;i++) {
		scanf("%d%d",&a,&v);
		playerBuffer.id=a;
		playerBuffer.fishNum=player_num[a];
		RedBlackTree_remove(playerBuffer);
		playerBuffer.fishNum+=v;
		player_num[a]=playerBuffer.fishNum;
		RedBlackTree_insert(playerBuffer);
		playerBuffer=RedBlackTree_getMax();
		if(playerBuffer.fishNum==0)playerBuffer.id=1;
		printf("%d %d\n",playerBuffer.id,playerBuffer.fishNum);
	}
	return 0;
}