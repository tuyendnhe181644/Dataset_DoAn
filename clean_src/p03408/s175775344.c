...
int main(void) {
    ...
    for (uint64_t i = 0; i < N; i++) {
        if (!checked[i]) {
            uint64_t plusCnt = 1;
            uint64_t minusCnt = 0;
            for (uint64_t sI = i + 1; sI < N; sI++) {
                if (strlen(s[sI]) == strlen(s[i]) && strstr(s[sI], s[i]) != NULL) {
                    plusCnt++;
                    checked[sI] = true;
                }
            }
            ...
            for (uint64_t tI = 0; tI < M; tI++) {
                if (strlen(t[tI]) == strlen(s[i]) && strstr(t[tI], s[i]) != NULL) {
                    minusCnt++;
                }
            }

            if (minusCnt < plusCnt && max < plusCnt - minusCnt) {
                max = plusCnt - minusCnt;
            }
