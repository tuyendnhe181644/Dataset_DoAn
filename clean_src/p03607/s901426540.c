...
void fill_long_int_array(long int array[], int min, int max, long int element)
{
	int i=min;
...
void max_int_array_num(int array[], int min, int max, int answer[])
{
	...
	fill_int_array(answer, 0, max-min, -1);
...
void max_int_array_table(int array[], int min, int max, int size_1, int size_2, int answer[size_1][size_2])
{
	...
	fill_int_array(answer[0], 0, size_2-1, 0);
	for(i=1; i<=size_1-1; i=i+1)
	{
		answer[i][0]=MIN_INT;
		fill_int_array(answer[i], 1, size_2-1, -1);
	}
...
int count_differentelement_char(char array[], int min, int max)
{
	int i=min;
	char pickup[max];
	fill_char_array(pickup, 0, max, '\0');
	int answer=0;
