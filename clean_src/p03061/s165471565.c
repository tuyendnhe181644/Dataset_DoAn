...
void segmake(int seg_num, int n, ll a[]){
...
ll seg_query_inside(int seg_num, int ql, int qr, int x, int l, int r){
    ...
    // return 
    //     (l>=ql && r<=qr) ? seg[seg_num][x]: 
    //     (l>qr || r<ql) ? seg_def[seg_num]():
    //     seg_ope[seg_num](
    //         seg_query_inside(seg_num,ql,qr,2*x+1,l,(l+r)/2),
    //         seg_query_inside(seg_num,ql,qr,2*x+2,(l+r)/2+1,r)
    //     );
}
ll seg_query(int seg_num, int l, int r){ return seg_query_inside(seg_num,l,r,0,0,seg_size-1); }


void segupdate(int seg_num, int x, ll val){
    ll c=seg_size-1+x;
    seg[seg_num][c]=val;
    while(c>0){
        c--; c/=2;
...
int main(void){
    ...
    seg_ope[0]=seg_ope_gcd;
    seg_def[0]=seg_def_gcd;
    segmake(0,n,a);

    ans=seg_query(0,1,n-1);
    ans=max2( ans , seg_query(0,0,n-2) );

    rep(i,1,n-1){
        ans = max2( ans , gcd( seg_query(0,0,i-1) , seg_query(0,i+1,n-1) ) );
    }
