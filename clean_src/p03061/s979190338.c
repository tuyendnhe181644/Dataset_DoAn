...
void segmake(int set_num, int n, ll a[]){
    while(seg_size<n) seg_size*=2;
    ll t=seg_size;
    rep(i,0,t){
        if(i<n) seg[set_num][t-1+i]=a[i]; 
        else    seg[set_num][t-1+i]=seg_def[0]();        
    }
    while(t/=2) 
        rep(i,0,t) 
            seg[set_num][t+i-1] = seg_ope[set_num](seg[set_num][2*(t+i)],
            seg[set_num][2*(t+i)-1]);
}
ll seg_query_inside(int set_num, int ql, int qr, int x, int l, int r){
    return 
        (l>=ql && r<=qr) ? seg[set_num][x]: 
        (l>qr || r<ql) ? seg_def[set_num]():
        seg_ope[set_num](
            seg_query_inside(set_num,ql,qr,2*x+1,l,(l+r)/2),
            seg_query_inside(set_num,ql,qr,2*x+2,(l+r)/2+1,r)
        );
}
ll seg_query(int set_num, int l, int r){
    return seg_query_inside(set_num,l,r,0,0,seg_size-1);
}


void segupdate(int set_num, int x, ll val){
    ll c=seg_size-1+x;
    seg[set_num][c]=val;
    while(c>0){
        c--; c/=2;
        seg[set_num][c] = seg_ope[set_num](seg[set_num][2*(c+1)],
        seg[set_num][2*(c+1)-1]);
    }
}
