...
//     rep(i,0,t) seg[t-1+i]=i<n?a[i]:a[n-1];
//     while(t/=2) rep(i,0,t) seg[t+i-1]=gcd(seg[2*(t+i)],seg[2*(t+i)-1]); 
// }
...
//     // if(l>=from && r<=to) return seg[a];
//     // if( to<(l+r)/2+1 ) return seggcd(from,to,2*a+1,l,(l+r)/2);
//     // if( from  > (l+r)/2 ) return seggcd(from,to,2*a+2,(l+r)/2+1,r);
//     // return gcd( seggcd(from,to,2*a+2,(l+r)/2+1,r) , seggcd(from,to,2*a+1,l,(l+r)/2) ) ;
// }
...
//     while(c>0){
//         c--; c/=2;
//         seg[c]=gcd(seg[2*(c+1)],seg[2*(c+1)-1]);
//     }
// }
...
ll seg_ope_max(ll a, ll b){
    return max2(a,b);
}
ll seg_ope_min(ll a, ll b){
    return min2(a,b);
}
ll seg_ope_sum(ll a, ll b){
    return a+b;
}
ll seg_ope_gcd(ll a, ll b){
    return gcd(a,b);
}
...
void segmake(int n, ll a[]){
    ...
    // while(t/=2) rep(i,0,t) seg[t+i-1]=max2(seg[2*(t+i)],seg[2*(t+i)-1]); 
    while(t/=2) rep(i,0,t) seg[t+i-1]= seg_ope(seg[2*(t+i)],seg[2*(t+i)-1]);
}
// void segmake(ll n, ll a[n]){
//     while(seg_size<n) seg_size*=2;
//     ll t=seg_size;
//     rep(i,0,t) seg[t-1+i]=i<n?a[i]:-INF;
//     while(t/=2) rep(i,0,t) seg[t+i-1]=max2(seg[2*(t+i)],seg[2*(t+i)-1]); 
...
ll segmin(ll from, ll to, ll a, ll l, ll r){
    return (l>=from && r<=to) ? seg[a] : (l>to || r<from) ? -INF : max2(segmin(from,to,2*a+1,l,(l+r)/2),segmin(from,to,2*a+2,(l+r)/2+1,r));
}
ll seggcd(ll from, ll to, ll a, ll l, ll r){
    return (l>=from && r<=to) ? seg[a] : (to<(l+r)/2+1) ? seggcd(from,to,2*a+1,l,(l+r)/2) : (from>(l+r)/2) ? seggcd(from,to,2*a+2,(l+r)/2+1,r) : gcd( seggcd(from,to,2*a+2,(l+r)/2+1,r) , seggcd(from,to,2*a+1,l,(l+r)/2) );
}
...
//     while(c>0){
//         c--; c/=2;
//         seg[c]=max2(seg[2*(c+1)],seg[2*(c+1)-1]);
//     }
// }
/*-----------------------segment tree(end)-----------------------------------------*/
