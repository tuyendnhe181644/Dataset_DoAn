...
int main(void) {
    ...
    const uint64_t sLen = (uint64_t)strlen(S);
    const uint64_t tLen = (uint64_t)strlen(T);
    uint64_t minUnmatchCnt = UINT64_MAX;

    for (uint64_t ofs = 0; ofs <= sLen - tLen; ofs++) {
        uint64_t umc = 0;
        for (uint64_t c = 0; c < tLen; c++) {
            if (S[ofs + c] != T[c]) {
                umc++;
            }
        }

        minUnmatchCnt = MIN(minUnmatchCnt, umc);
    }

    printf("%"PRIu64, minUnmatchCnt);
