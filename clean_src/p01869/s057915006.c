...
int calc(long long n, int i, int s)
{
	...
	if (n == 16) { if (s+4 > ans) ans = s+4; return 1; }
	if (n < 21) return 0;
	...
	for ( ; p[i] <= b; i++) {
		...
		m = n / k, a = 1;
		while (!(m % k)) m /= k, a++;
		if (m == 1) { if (s+a > ans) ans = s+a; return 1; }
		if (m < k) m *= k, a--;
		while (a >= 0) {
			if (calc(m, i+1, s+a)) return 1;
			m *= k, a--;
		}
	}
	return 0;
...
void init()
{
	...
	insert( 1767928088070144LL, 7);
	insert(12751787892400128LL, 4);
	p[2] = p[6] = p[14] = 0;
}
