...
int main()
{
	int x, y, k, i, f, t, nx, ny, nk, ans;

	memset(d, INF, sizeof(d));
	d[XBASE][YBASE][0] = 0;
	q[0].x = XBASE, q[0].y = YBASE, q[0].k = 0, top = 0, end = 1;
	while (top != end) {
		x = q[top].x, y = q[top].y, k = q[top++].k;

		f = (x+y) & 1, t = d[x][y][k]+1;
		for (i = 0; i < 3; i++) {
			nx = x + dx[f][i], ny = y + dy[f][i], nk = tr[k][dk[f][i]];
			if (nx >= 0 && nx <= XMAX && ny >= 0 && ny <= YMAX && d[nx][ny][nk] > t) {
				d[nx][ny][nk] = t;
