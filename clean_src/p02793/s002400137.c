...
static uint64_t A[10000];
static divisorExist[1000001];
...
int main(void) {
    ...
    for (uint64_t i = 2; i <= 1000000; i++) {
        divisorExist[i] = false;
    }

    for (uint64_t i = 0; i < N; i++) {
        uint64_t dc[1001];
        for (uint64_t j = 2; j <= 1000; j++) {
            dc[j] = 0;
        }
    ...
    for (uint64_t v = 2; v <= 1000; v++) {
        for (uint64_t m = 0; m < tdc[v]; m++) {
            aLcm = aLcm * v % MOD_VAL;
        }
    }

    for (uint64_t v = 1001; v <= 1000000; v++) {
        if (divisorExist[v]) {
            aLcm = aLcm * v % MOD_VAL;
        }
    }
    ...
    for (uint64_t i = 0; i < N; i++) {
        res = (res + aLcm * modinvU64(A[i], MOD_VAL)) % MOD_VAL;
    }
