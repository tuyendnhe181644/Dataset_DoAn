...
int cmp_node_x (const void *a, const void *b) {
  i32 d = ((node *)a)->x - ((node *)b)->x;
  if (d != 0) return d < 0 ? -1 : 1;
  d = ((node *)a)->index - ((node *)b)->index;
  return d == 0 ? 0 : d < 0 ? -1 : 1;
}
...
void run (void) {
  ...
  for (i32 i = 0; i < n; ++i) {
    i32 a, b, x;
    scanf ("%" SCNi32 "%" SCNi32 "%" SCNi32, &a, &b, &x);
    s[i] = (node) {a - x, x, i};
    t[i] = (node) {b - x, x, i};
  }
  qsort (s, n, sizeof (node), cmp_node_time);
  qsort (t, n, sizeof (node), cmp_node_time);
  heap *now = new_binary_heap (sizeof (node), cmp_node_x);
  heap *del = new_binary_heap (sizeof (node), cmp_node_x);
  i32 i = 0, j = 0;
  while (q--) {
    i32 d;
    scanf ("%" SCNi32, &d);
