...
int cmp1_2( const void *p, const void *q ) { return ((fr*)p) ->b - ((fr*)q)->b;}
int cmp2( const void *p, const void *q ) { return ((fr*)q) ->a - ((fr*)p)->a;}
void strsortup(fr*a,int n){qsort(a,n,sizeof(fr),cmp1);}
void strsortup2(fr*a,int n){qsort(a,n,sizeof(fr),cmp1_2);}
...
typedef struct{ ll y,x;  ll cost; }str_Heap;
str_Heap heap[MAX_N];
...
ll prim(){
    ...
    while(1){
        ...
        rep(i,0,ed_cnt[x]){
            ...
            if(d[to]) continue;
            heap[len].x=to;
            push_strHeap( cost,len++,0 );
        }
        while(1){
            if(len==0) return ans;
            if( d[heap[0].x]) pop_strHeap(len--,0);
            else break;
        }
        ans+= heap[0].cost ;
        x=heap[0].x;
...
void solve(){
    ...
    strsortup(a,n);
    rep(i,1,n){
        pushGraph( a[i].c , a[i-1].c , a[i].a-a[i-1].a , 1,0 );
    }
    strsortup2(a,n);
    rep(i,1,n){
        pushGraph( a[i].c , a[i-1].c , a[i].b-a[i-1].b , 1,0 );
    }
