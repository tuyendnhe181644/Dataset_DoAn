...
// void par_make(ll n){
//     rep(i,0,edge_cnt[n]){
//         if( graph[n][i*2] == par[n] ) continue;
//         // ( graph[n][i*2] == par[n] ) ? ( par[graph[n][i*2]]=  n ) : 0 ; 
//         par[graph[n][i*2]]=  n;
//         par_make(graph[n][i*2]);
//     }
//     return;
// }
ll po[200005];
...
//         if(graph[n][i*2]==par[n]) continue;
//         po[graph[n][i*2]]+=sum;
//         dfs(graph[n][i*2],po[graph[n][i*2]]);
//     }
//     return;
// }
void dfs(ll n, ll pre, ll sum){
    rep(i,0,edge_cnt[n]){
        if(graph[n][i*2]==pre) continue;
        po[graph[n][i*2]]+=sum;
        dfs(graph[n][i*2],n,po[graph[n][i*2]]);
    }
    return;
}
