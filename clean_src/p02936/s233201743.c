//Euler tour and Queries for subtree
//Mostly use with lazy Segtree
//ABC138-D
#include<stdio.h>
#include<string.h>
#include<stdlib.h>
...
//lazy Segtree - add to range and sum
...
//(node) -> cvrt[(node)]
//[sa,sb)...Subtree
long long sa[524288],sb[524288],cvrt[524288],cnt;
void euler_tour(long long v,rs g[],mkj x[]){
  if(cvrt[v]!=-1){return;}
  cvrt[v]=cnt;
  sa[cvrt[v]]=cnt;
  cnt++;
  ...
  for(i=x[v].st;i<x[v].st+x[v].kz;i++){
    if(cvrt[g[i].fi]!=-1){continue;}
    euler_tour(g[i].fi,g,x);
  }
  sb[cvrt[v]]=cnt;
}
...
int main(void){
  sinit();
  long long i,j,n,m,k,a,b,c,h,w,r=0,l,t,p,q;
  rs g[524288];
  mkj x[524288];
  scanf("%lld%lld",&n,&q);
  for(i=0;i<n-1;i++){
  ...
  makemkj(g,x,2*(n-1));
  cnt=0;
  for(i=0;i<524288;i++){cvrt[i]=-1;}
  euler_tour(1,g,x);
  for(i=0;i<q;i++){
    scanf("%lld%lld",&p,&a);
    addquery(sa[cvrt[p]],sb[cvrt[p]],a,0,0,ssize);
  }
  for(i=1;i<=n;i++){
    if(i!=1){printf(" ");}
    printf("%lld",sumquery(cvrt[i],cvrt[i]+1,0,0,ssize));
  }printf("\n");
  return 0;
}
