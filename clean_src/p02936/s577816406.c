...
#define BTREE_INIT_ALLOC (128)

static BTree_Node bTree_Node[200000];

static void bTree_Init(const uint64_t N) {
    for (uint64_t i = 0; i < N; i++) {
        bTree_Node[i].cnt = 0;
        bTree_Node[i].numChild = 0;
        bTree_Node[i].child = malloc(sizeof(size_t) * BTREE_INIT_ALLOC);
        bTree_Node[i].maxNumChild = BTREE_INIT_ALLOC;
    }
}

static void bTree_AddChild(const size_t parent, const size_t child) {
    if (bTree_Node[parent].maxNumChild <= bTree_Node[parent].numChild) {
        bTree_Node[parent].child = realloc(bTree_Node[parent].child, sizeof(size_t) * bTree_Node[parent].maxNumChild * 2);
        bTree_Node[parent].maxNumChild *= 2;
    }

    bTree_Node[parent].child[bTree_Node[parent].numChild] = child;
    bTree_Node[parent].numChild++;
}

static void conclude(const size_t idx, const uint64_t addVal) {
    bTree_Node[idx].cnt += addVal;

    for (size_t i = 0; i < bTree_Node[idx].numChild; i++) {
        conclude(bTree_Node[idx].child[i], bTree_Node[idx].cnt);
    }
}
...
int main(void) {
    ...
    for (uint64_t i = 0; i < N; i++) {
        printf("%"PRIu64, bTree_Node[i].cnt);
        if (i < N - 1) {
            putchar(' ');
        }
    }
