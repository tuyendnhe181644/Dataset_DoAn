...
static struct node dummy = {NULL, NULL, NULL, -1, 0};
static struct node *root = &dummy;
...
void trickle_down(struct node *node) {
    while(node->left || node->right) {
        ...
        if(!node->left) {
            ...
            // change parent to be node->right
...
int delete(int key) {
    struct node *node = find_last(key);
#ifdef DEBUG
    printf("delete: root[%p] vs node[%p]\n", root, node);
#endif
    // if not found, early return
    if(node->val != key) return 0;
    trickle_down(node);
    if(node == root) {
        init_root();
    } else {
        // erase parent's link and node
        struct node *parent = node->parent;
        if(parent->left == node) {
            parent->left = NULL;
        } else {
...
int main(void) {
    ...
    for(i = 0; i < qs; i++) {
        ...
        if(cmd != CMD_DELETE) ans[ans_idx++] = res;
