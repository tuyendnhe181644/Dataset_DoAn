...
static uint64_t dp[667][2001];
static uint64_t sp[667][2001];
...
int main(void) {
    ...
    for (uint64_t i = 0; i <= S; i++) {
        ...
        if (i == 0) {
            ...
            sp[0][i] = (sp[0][i - 1] + dp[0][i]) % MOD_VAL;
        }
    }
    ...
    for (uint64_t d = 1; d < (S + 2) / 3; d++) {
        dp[d][S] = 0;
        for (uint64_t s = 3 * (d + 1); s <= S; s++) {
            dp[d][s] = sp[d - 1][s - 3];

            sp[d][s] = ((s == 3 * (d + 1)) ? dp[d][s] : (sp[d][s - 1] + dp[d][s]) % MOD_VAL);
        }
