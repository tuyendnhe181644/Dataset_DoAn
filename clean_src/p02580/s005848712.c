...
static uint64_t h[300000], w[300000];

static uint64_t hc[300000], wc[300000];

static uint64_t hMaxList[300000], wMaxList[300000];

static uint64_t hMaxList_Size = 0;
static uint64_t wMaxList_Size = 0;

#define HASH_LIST_SIZE (10000000)

static bool bombExist[HASH_LIST_SIZE];

int main(void) {
    for (uint64_t i = 0; i < HASH_LIST_SIZE; i++) {
        bombExist[i] = false;
    }

    uint64_t H, W, M;

    scanf("%"PRIu64"%"PRIu64"%"PRIu64, &H, &W, &M);
    ...
    for (uint64_t i = 0; i < M; i++) {
        ...
        sprintf(c, "%"PRIu64" %"PRIu64, h[i], w[i]);
        uint32_t hs[5];
        sha1(c, 0, hs);
        bombExist[hs[0] % HASH_LIST_SIZE] = true;
    }
    ...
    for (uint64_t i = 0; i < W; i++) {
        if (wc[i] == wc[wMax]) {
            ...
            wMaxList[wMaxList_Size] = i;
            wMaxList_Size++;
        }
    }

    for (uint64_t i = 0; i < hMaxList_Size; i++) {
        for (uint64_t j = 0; j < wMaxList_Size; j++) {
            char c[17];

            sprintf(c, "%"PRIu64" %"PRIu64, hMaxList[i], wMaxList[j]);
            uint32_t hs[5];
            sha1(c, 0, hs);
            if (!bombExist[hs[0] % HASH_LIST_SIZE]) {
                //printf("> %"PRIu64" %"PRIu64"\n", hMaxList[i], wMaxList[j]);
