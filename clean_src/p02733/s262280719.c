...
void solve(){
    ...
    rep(i,0,1<<(h-1)){
        ...
        rep(j,0,w){
            ...
            next=0;
            rep(l,1,p){
                ll whi=rui[siki[l]][j+1]-rui[siki[l-1]][j+1]-rui[siki[l]][wal]+rui[siki[l-1]][wal];
                if(whi>k){cnt=INF; break;}
                if(j==w-1) continue;
                whi=rui[siki[l]][j+2]-rui[siki[l-1]][j+2]-rui[siki[l]][wal]+rui[siki[l-1]][wal];
                if(whi>k){next=1;}
            }
            if(next){
                wal=j+1; cnt++;
