...
#define BHEAP_NODE_H_BOMB (UINT64_MAX)

static BHeap_Node bh[400000];
...
int main(void) {
    ...
    scanf("%"PRIu64"%"PRIu64"%"PRIu64, &N, &D, &A);

    bHeap_Init(&ctx, _bHeap_Compare, NULL, bh);

    for (uint64_t i = 0; i < N; i++) {
        BHeap_Val val;
        scanf("%"PRIu64"%"PRIu64, &val.X, &val.H);
        val.numB = 0;
    ...
    while ((topV = bHeap_GetTopVal(&ctx)) != NULL) {
        if (topV->H != BHEAP_NODE_H_BOMB) {
            if (bSum * A < topV->H) {
                BHeap_Val val = { .H = BHEAP_NODE_H_BOMB,.X = topV->X + 2 * D,.numB = (topV->H - bSum * A + A - 1) / A };
