...
void run (void) {
  ...
  update_all (l);
  update_all (r);
  edge *e = (edge *) calloc (2 * n, sizeof (edge));
  i32 len = 0;
  for (i32 i = 0; i < n; ++i) {
    i32 v = p[i].index;
    if (v > 0) {
      segment_node t;
      find (l, 0, v, &t);
      e[len++] = (edge){v, t.index, p[i].val + t.val - (i64) (n - 1 - v) * d};
    }
    if (v + 1 < n) {
      segment_node t;
      find (r, v + 1, n, &t);
      e[len++] = (edge){v, t.index, p[i].val + t.val - (i64) v * d};
    }
    update (l, v, &ini);
    update (r, v, &ini);
  }
  qsort (e, len, sizeof (edge), cmp_edge);
  i64 sum = 0;
  UnionFind *u = newUnionFind (n);
  for (i32 i = 0; i < len && getSize (u, 0) < n; ++i) {
