...
int main(){
	...
	for(i = 0; i < M; i++){
		scanf("%d%d", &s, &t);
		set_edge_graph(s, t, 1, g);
	}
	build_graph(g);
	scanf("%d", &Q);
	int *u = (int *)malloc(sizeof(int) * Q);
	int *v = (int *)malloc(sizeof(int) * Q);
	for(i = 0; i < Q; i++){
		scanf("%d%d", &u[i], &v[i]);
	}
	DAG *dag = build_DAG(g, 0);
	int *depend_on = (int *)malloc(sizeof(int) * N);
	for(i = 0; i < dag->N; i++){
		SCC *scc = dag->sorted_SCC[i];
		for(j = 0; j < scc->vertex_num; j++){
			depend_on[scc->vertexs[j]] = i;
		}
	}
	for(i = 0; i < Q; i++){
		if(depend_on[u[i]] == depend_on[v[i]]){
			printf("1\n");
		}
		else{
			printf("0\n");
