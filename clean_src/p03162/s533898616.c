...
int main(void){
    In(how_data);
    long data[how_data][3];
    input_array2(how_data,3,data);
    long dp[how_data+1][3];
    for(loop1=0;loop1<how_data+1;loop1++){
        for(loop2=0;loop2<3;loop2++){
            dp[loop1][loop2]=(loop1==0?0:-1); //初期化
        }
    }
    for(loop1=0;loop1<how_data;loop1++){
        for(loop2=0;loop2<3;loop2++){
            update_max(dp[loop1+1][1],dp[loop1][0]+data[loop1][1]);
            update_max(dp[loop1+1][2],dp[loop1][0]+data[loop1][2]);
            update_max(dp[loop1+1][0],dp[loop1][1]+data[loop1][0]);
            update_max(dp[loop1+1][2],dp[loop1][1]+data[loop1][2]);
            update_max(dp[loop1+1][0],dp[loop1][2]+data[loop1][0]);
            update_max(dp[loop1+1][1],dp[loop1][2]+data[loop1][1]);
        }
    }
    long ans=sel_max(dp[how_data][0],dp[how_data][1]);
    update_max(ans,dp[how_data][2]);
    printf("%ld",ans);
    END;
