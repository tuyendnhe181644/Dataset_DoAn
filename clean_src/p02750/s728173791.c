...
ull solve () {
	...
	if (a1) qsort(xy, a1, sizeof(hwll), pfracomp);
	// for (i = 0; i < a1; i++) {
	// 	printf("%lld, %lld\n", xy[i].a, xy[i].b);
	// }
	...
	sll maxx = 33;
	dp[0] = 1;
	for (i = 1; i <= maxx; i++) dp[i] = k + 1;
	for (i = a1 - 1; i >= 0; i--) {
		for (j = maxx; j; j--) {
			dp[j] = smin(dp[j], dp[j - 1] * (xy[i].a + 1) + xy[i].b);
		}
	}

	if (a0) qsort(c, a0, sizeof(sll), psllcomp);
