...
int main(void) {
    ...
    for (uint64_t ofs = 0; ofs < N; ofs++) {
        if (X[ofs] == '1') {
            if (1 < pc) {
                mr[0] = (mr[0] + powU64WMod(2, N - ofs - 1, pc - 1)) % (pc - 1);
            }
            mr[1] = (mr[1] + powU64WMod(2, N - ofs - 1, pc + 1)) % (pc + 1);
        }
    }
    ...
    for (uint64_t ofs = 0; ofs < N; ofs++) {
        if (X[ofs] == '1') {
            if (pc == 1) {
                ...
                uint64_t v = (mr[0] + (pc - 1) - powU64WMod(2, N - ofs - 1, pc - 1)) % (pc - 1);
                uint64_t cnt = 1;
            ...
            uint64_t v = (mr[1] + powU64WMod(2, N - ofs - 1, pc + 1)) % (pc + 1);
            uint64_t cnt = 1;
