...
int main(void) {
    ...
    scanf("%"PRId64"%"PRId64"%"PRId64"%"PRId64, &sx, &sy, &tx, &ty);

    for (int64_t y = sy; y < ty; y++)
        putchar('U');
    for (int64_t x = sx; x < tx; x++)
        putchar('R');
    for (int64_t y = ty; sy < y; y--)
        putchar('D');
    for (int64_t x = tx; sx < x; x--)
        putchar('L');
    putchar('L');
    for (int64_t y = sy; y < ty + 1; y++)
        putchar('U');
    for (int64_t x = sx - 1; x < tx; x++)
        putchar('R');
    putchar('D');
    putchar('R');
    for (int64_t y = ty; sy - 1 < y; y--)
        putchar('D');
    for (int64_t x = tx + 1; sx < x; x--)
        putchar('L');
    putchar('U');
