...
ll *mf_arr,*inv_arr,*finv_arr,is_minv_made=0,is_mf_made=0,num_of_inv=1e5+10;
...
int main(void){
    ...
    rep(i,1,n){
        if(n-1>=i+1){
            ...
            // ans[i]+=m_mul3( m_f(n-1,MOD1) , m_inv(i+1,MOD1,1) , m_inv(n-i-2,MOD1,1) , MOD1 );
        }

        if(n-1>=i-1){
            ans[i]+=mod_nck(n-1,i-1,MOD1);
            // ans[i]+=m_mul3( m_f(n-1,MOD1) , m_inv(i-1,MOD1,1) , m_inv(n-i,MOD1,1) , MOD1 );
        }else ans[i]++;

        ans[i]+=2*mod_nck(n-1,i,MOD1);
        // ans[i]+=2*m_mul3( m_f(n-1,MOD1) , m_inv(i,MOD1,1) , m_inv(n-i-1,MOD1,1) , MOD1 );
        if(n-r+l>=i){
            // printf("%lld %lld\n",n-r+l,i);
            ll t=mod_nck(n-r+l,i,MOD1);
            // ll t=m_mul3( m_f(n-r+l,MOD1) , m_inv(i,MOD1,1) , m_inv(n-r+l-i,MOD1,1) , MOD1 );
            ans[i]+=mod_MOD1(-t);
        }
