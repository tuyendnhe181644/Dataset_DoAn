...
sll solve () {
	...
	if (n == 0) {
		...
		// xy[0] = (hwll){1, 1};
		// for (i = 1; i <= n; i++) {
		// 	xy[i] = (hwll){xy[i - 1].a - a[i], xy[i - 1].b * 2 - a[i]};
		// 	if (xy[i].b < 0) goto fail;
		// 	if (xy[i].a < 0) xy[i].a = 0;
		// 	// printf("%lld %lld\n", xy[i].a, xy[i].b);
		// }

		// if (xy[n].a != 0) goto fail;

		// item = 0;
		// result = 0;
		// for (i = n; i > 0; i--) {
		// 	item += a[i];
		// 	// item: depth i vertexs
		...
		xy[n] = (hwll){a[n], a[n]};
		for (i = n - 1; i >= 0; i--) {
			xy[i] = (hwll){smax((xy[i + 1].a + 1) / 2, 1), xy[i + 1].b};
			// printf("%lld %lld\n", xy[i].a, xy[i].b);

			xy[i].a += a[i];
			xy[i].b += a[i];
		}
		if (xy[0].a > 1) goto fail;
