...
int main(void) {
    ...
    for (uint64_t ofs = 0; ofs <= N; ofs++) {
        if (ofs == 0) {
            ...
            lCnt[N - ofs] = lCnt[N - ofs + 1] + A[N - ofs];
        }
    }
    ...
    uint64_t prevB = 0;
    for (uint64_t i = 0; i <= N; i++) {
        const uint64_t cnMin = A[i] + (i < N ? 1 : 0);
        if (i == 0) {
            const uint64_t cnMax = powU64(2, i);
            ...
            uint64_t nextB = 0;
            if (i < N) {
                const uint64_t bMax = cnMax - A[i];
                if (bMax * 2 < A[i + 1]) {
                    puts("-1");
                    return 0;
                }

                nextB = MIN(MIN(bMax, lCnt[i + 1]), 1);
            }
            ...
            uint64_t nextB = 0;
            if (i < N) {
                const uint64_t bMax = MIN(cnMax, prevB * 2) - A[i];
                if (bMax * 2 < A[i + 1]) {
                    puts("-1");
                    return 0;
                }

                nextB = MIN(bMax, lCnt[i + 1]);
            }
            ...
            uint64_t nextB = 0;
            if (i < N) {
                const uint64_t bMax = prevB * 2 - A[i];
                if (bMax * 2 < A[i + 1]) {
                    puts("-1");
                    return 0;
                }
