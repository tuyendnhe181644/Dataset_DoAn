...
typedef struct{ ll a;  ll cost; }str_Heap;
str_Heap heap[MAX_N];

void str_swap(str_Heap *a,str_Heap *b){ str_Heap c;c=*b;*b=*a;*a=c; }

ll push_strHeap(ll val, ll len , ll kind){
...
ll pop_strHeap(ll len, ll kind){
    ll i=0; ll val=heap[0].cost;  heap[0].cost=HeapValue(kind);
...
int main(void){
    ...
    heap[0].a=0;
    t[cnt++]=0;
    push_strHeap(a[0]+b[0]+c[0],l++,1);
    rep(i,0,k){
        printf("%lld\n",heap[0].cost);
        ll p=heap[0].a/POW(10,6); heap[0].a%=POW(10,6);
        ll q=heap[0].a/POW(10,3); heap[0].a%=POW(10,3);
        ll r=heap[0].a;
        pop_strHeap(l--,1);
        if(p!=x-1){
            ll tmp=(p+1)*POW(10,6)+(q)*POW(10,3)+r;
            rep(j,0,cnt){
                if(t[j]==tmp){ break; }
                if(j==cnt-1){
                    heap[l].a=tmp;
                    push_strHeap(a[p+1]+b[q]+c[r],l++,1);
                    t[cnt++]=tmp;
                    break;
                }
            }
        ...
        if(q!=y-1){
            ll tmp=(p)*POW(10,6)+(q+1)*POW(10,3)+r;
            rep(j,0,cnt){
                if(t[j]==tmp){ break; }
                if(j==cnt-1){
                    heap[l].a=tmp;
                    push_strHeap(a[p]+b[q+1]+c[r],l++,1);
                    t[cnt++]=tmp;
                    break;
                }
            }
        ...
        if(r!=z-1){
            ll tmp=(p)*POW(10,6)+(q)*POW(10,3)+r+1;
            rep(j,0,cnt){
                if(t[j]==tmp){ break; }
                if(j==cnt-1){
                    heap[l].a=tmp;
                    push_strHeap(a[p]+b[q]+c[r+1],l++,1);
                    t[cnt++]=tmp;
                    break;
                }
            }
