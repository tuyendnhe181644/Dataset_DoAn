    ...
    rep(i,0,nt){
        ...
        rep(j,0,ns){
            if(jud){
                ...
                if(s[j]==t[i]) dp[i+1][j+1]=max2( dp[i+1][j+1] , dp[i][j]+1 );
                dp[i+1][j+1]=max2( dp[i+1][j+1] , dp[i][j+1] );
                dp[i+1][j+1]=max2( dp[i+1][j+1] , dp[i+1][j] );
            }
        }
        // rep(j,0,ns+1) printf("%lld ",dp[i+1][j]);
        // PN;
    }
    ll l=dp[nt][ns];
    char ans[551551]="";
    while(l){
		if(dp[nt][ns]==dp[nt-1][ns]) nt--;
		else if(dp[nt][ns]==dp[nt][ns-1]) ns--;
		else{
    		// printf("%lld %lld\n",nt,ns);
			ans[--l]=t[nt-1];
			ns--;
