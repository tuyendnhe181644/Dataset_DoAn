...
int main(void){
    int how_student,how_check;
    scanf("%d %d",&how_student,&how_check);
    int data_stu[how_student][2];
    int data_che[how_check][2];
    input_array2(how_student,2,data_stu);
    input_array2(how_check,2,data_che);
    for(loop1=0;loop1<how_student;loop1++){
        int min=2147400000;
        i_temp=-1;
        for(loop2=0;loop2<how_check;loop2++){
            if(min>abs(data_stu[loop1][0]-data_che[loop2][0])+abs(data_stu[loop1][1]-data_che[loop2][1])){
                min=abs(data_stu[loop1][0]-data_che[loop2][0])+abs(data_stu[loop1][1]-data_che[loop2][1]);
                i_temp=loop2;
            }
        }
        printf("%d\n",i_temp+1);
