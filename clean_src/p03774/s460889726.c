...
long long int pow_int(int base, unsigned int exponent)
{
...
long long int pow_int_mod(int base, unsigned int exponent, int mod)
{
	long long int answer=1;
	while (exponent>0)
	{
...
unsigned long long int factorial_mod(unsigned int num, int mod)
{
	unsigned int i=0;
	unsigned long long int answer=1;
	rep(i, 2, num)
...
int convert_adic_int(char num[], unsigned int p_adic, unsigned int q_adic)
{
	int i=0;
	int answer=0;
	bool sgn=0;
...
int main()
{
	...
	scanf("%d %d", &N, &M);
	int a[N+1], b[N+1];
	scanf_int_array_2(a, b, 1, N);
	int c[M+1], d[M+1];
	scanf_int_array_2(c, d, 1, M);

	int distance[N+1][M+1];
	rep(i, 0, N)
	{
		fill_int(distance[i], 0, M, 0);
	}
	...
	rep(i, 1, N)
	{
		min_int_array_num(distance[i], 1, M, answer[i]);
	}
