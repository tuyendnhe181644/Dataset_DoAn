...
int main(void){
    long how_data,move;
    scanf("%ld%ld",&how_data,&move);
    long data[how_data];
    input_array(how_data,data);
    long dp[how_data];
    format_array(how_data,dp,2147483647);
    dp[0]=0;
    for(loop1=0;loop1<how_data-1;loop1++){
        for(loop2=1;(loop2<=move)&&(loop1+loop2)<how_data;loop2++){
            update_min(dp[loop1+loop2],dp[loop1]+labs(data[loop1]-data[loop1+loop2]));
        }
        //output_array(how_data,dp);
    }
    printf("%ld",dp[how_data-1]);
    END;
}
