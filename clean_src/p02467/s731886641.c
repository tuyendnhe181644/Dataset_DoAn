...
// ll factor_pre(){  rep(i,0,2){ factors[i] = realloc( factors[i] , sizeof(ll) ); factors[i][i?fac_cnt++:fac_cnt]=i?0:2; } repp(i,3,1e6+1,2) rep(j,0,fac_cnt) if( !(i%factors[0][j]) ){break;} else if(POW(factors[0][j],2)>i){  rep(k,0,2){ factors[k] = realloc( factors[k] , sizeof(ll)*( fac_cnt +1 ) );  factors[k][k?fac_cnt++:fac_cnt]=k?0:i;  } break; }  return 0; }
ll factor_pre(){
    rep(i,0,1){ if(is_factor_prepared) return 0; }
    ll fac_tmp[1000000/2+1];
    // fac_tmp[0]=2;
    rep(i,0,1000000/2+1){fac_tmp[i]=i?2*i+1:2;}
    rep(i,1,1000000/2+1){if(fac_tmp[i]){
        repp(j,3,(1000000/2+1)/(2*i+1)+1,2 ){
            if( j*(2*i+1)<(1000000/2+1) ) fac_tmp[ (j*(2*i+1)-1)/2 ]=0;
        }
    }else continue;}
    rep(i,0,1000000/2+1){if(fac_tmp[i]){
        rep(j,0,2){ factors[j] = realloc( factors[j] , sizeof(ll)*( fac_cnt +1 ) );  factors[j][j?fac_cnt++:fac_cnt]=j?0:fac_tmp[i];  }
    } }
    is_factor_prepared=1;
    // rep(i,0,100) printf("%lld\n",factors[0][i]);
    return 0;
}


ll factor(ll n, ll is_new){ factor_pre();  rep(i,0,fac_cnt){ ll cnt=0; rep(j,0,1){ while( ( cnt+= n %factors[0][i]==0 ? 1 : 0 )  && (n/=factors[0][i]) %factors[0][i]==0 ) continue;} factors[1][i]= is_new ? cnt : max2(factors[1][i],cnt);  if( factors[0][i]> n ) break; }    return n; }
ll judge_prime(ll n){ rep(i,0,1){if(!is_factor_prepared){factor_pre();}} rep(i,0,fac_cnt){ if(n<factors[0][i]*factors[0][i] || n==factors[0][i]) break; else if(n%factors[0][i]==0) n/=n; }  return n==1?0:1; }
