...
#define MOD 998244353//1000000007,1000000009,998244353 
...
LL fact(LL n,F*a){LL x,i,j,c=0;x=n;for(i=2;i*i<=x;i++){for(j=1;x%i==0;j++)x/=i;if(j>1){a[c].n=i;a[c].exp=j-1;c++;}}if(x-1){a[c].n=x;a[c].exp=1;c++;}return c;}

//snippet
#define N 100009

int main(){
  int n,m=0,a[N],b[N]={0};LL ans;
  scanf("%d",&n);
  rep(i,n){
    scanf("%d",a+i);
    b[a[i]]++;
  ...
  ans=(a[0]||b[0]-1?0:1);
  rep(i,n-1)ans=(ans*b[a[i+1]-1])%MOD;
  printf("%lld",ans);
}