...
int main(void) {
    ...
    for (uint64_t d = 1; d <= N; d++) {
        dpC[d] = MAX(dpC[d - 1], dpS[d - 1].sc * A[d] + dpS[d - 1].c);
        if (dpS[d - 1].sc < dpC[d - 1] / A[d]) {
            dpS[d].sc = dpC[d - 1] / A[d];
            dpS[d].c = dpC[d - 1] % A[d];
        }
        else if (dpS[d - 1].sc == dpC[d - 1] / A[d]) {
            if (dpS[d - 1].c < dpC[d - 1] % A[d]) {
                dpS[d].sc = dpC[d - 1] / A[d];
                dpS[d].c = dpC[d - 1] % A[d];
            }
            else {
        ...
        //printf("D: %"PRIu64" - C: %"PRIu64", S: %"PRIu64"\n", d, dpC[d], dpS[d]);
    }
