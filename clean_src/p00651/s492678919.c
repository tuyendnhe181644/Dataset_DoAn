...
{
  double a = dist(next, prev);
  double b = dist(now, prev);
  double c = dist(now, next);
...
typedef struct { int next; double ang; } T;
T tbl[27];
C c0;
PP p[2][27];
...
int main()
{
	...
	while (N = in()) {
		...
		for (k1 = 0, k2 = 1, j = 0; j < Q; j++, k1 = k2, k2 = !k2) {
			if ((nz = tbl[z].next) >= 0) a = tbl[z].ang;
			else {
				a = minAngle(&nz, z, N, p[k1]);
				if (j > 0) tbl[z].next = nz, tbl[z].ang = a;
			}
