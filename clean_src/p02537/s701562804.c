...
static struct {
    ...
    uint64_t depth;
    uint64_t bottomBase;
    uint64_t node[10000000];
} segTree_Info;

static void SegTree_Init(const uint64_t size) {
    segTree_Info.capacity = size;
    segTree_Info.depth = 1;
    for (uint64_t i = 1; i < size; i *= 2) {
        segTree_Info.depth++;
    }
    segTree_Info.size = powU64(2, segTree_Info.depth) - 1;
    for (uint64_t i = 0; i < segTree_Info.size; i++) {
        segTree_Info.node[i] = 0;
    }
    segTree_Info.bottomBase = powU64(2, segTree_Info.depth - 1) - 1;
}

static void SegTree_Update(const uint64_t idx, const uint64_t v) {
    uint64_t nodeIdx = idx + segTree_Info.bottomBase;

    while (0 < nodeIdx) {
        if (segTree_Info.bottomBase <= nodeIdx) {
            segTree_Info.node[nodeIdx] = v;
        }
        else {
            segTree_Info.node[nodeIdx] = MAX(segTree_Info.node[2 * (nodeIdx + 1) - 1], segTree_Info.node[2 * (nodeIdx + 1)]);
        }
        nodeIdx = (nodeIdx - 1) / 2;
    }

    if (1 < segTree_Info.depth) {
        segTree_Info.node[0] += MAX(segTree_Info.node[1], segTree_Info.node[2]);
    }
}

static uint64_t segTree_Find(const uint64_t cur, const uint64_t rl, const uint64_t rr, const uint64_t sl, const uint64_t sr) {
    if (sr < rl || rr < sl) {
        return 0;
    }

    if (rl <= sl && sr <= rr) {
        return segTree_Info.node[cur];
    }

    return MAX(segTree_Find(2 * (cur + 1) - 1, rl, rr, sl, (sl + sr) / 2), segTree_Find(2 * (cur + 1), rl, rr, (sl + sr) / 2 + 1, sr));
}

static uint64_t SegTree_GetMax(const uint64_t begin, const uint64_t end) {
    return segTree_Find(0, begin, end, 0, powU64(2, segTree_Info.depth - 1) - 1);
}
...
int main(void) {
    ...
    for (uint64_t i = 0; i < N; i++) {
        ...
        if (i == 0) {
            ...
            SegTree_Update(A, SegTree_GetMax(A - MIN(A, K), MIN(A + K, 300000)) + 1);
        }
    }

    printf("%"PRIu64, SegTree_GetMax(0, 300000));
