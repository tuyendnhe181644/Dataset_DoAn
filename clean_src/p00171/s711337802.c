...
int step3(int s6,int s7,int s4){
	...
	for(r=0;r<24;r++){
		...
		//printf("%c-%c %c-%c %c-%c\n",cube[s4/6][s4%6],cube[i][6*i+sa-1],cube[s6/6][s6%6],cube[i][6*i+sb-1],cube[s7/6][s7%6],cube[i][6*i+sc-1]);
		roll(&sb,&sc);
	}
	...
	for(i=0;i<6;i++){
		...
		for(j=0;j<4;j++){
			if(graph[db*6+sb-1][dice*6+ta-1]==SET){
				//printf("--%d %d %d %d %d %d\n",da,sa,db,sb,dice,tb);
				//printf("**%c-%c %c-%c\n",cube[da][sa-1],cube[dice][i],cube[db][sb-1],cube[dice][ta-1]);
				*tc=tb;
				return 1;
			}
			roll(&ta,&tb);
		...
		for(i=6;i<48;i++){
			...
			for(j=6;j<48;j++){
				...
				for(k=6;k<48;k++){
					...
					used[k/6]=1;
					//printf("%d %d %d [%d %d] [%d %d] [%d %d]\n",st,sa,sb,i/6,i%6,j/6,j%6,k/6,k%6);
					//printf("%c-%c %c-%c %c-%c\n",cube[0][st-1],cube[i/6][i%6],cube[0][sa-1],cube[j/6][j%6],cube[0][sb-1],cube[k/6][k%6]);
...
int main(){
	...
	while(scanf("%s",cube[0])){
		...
		//for(i=0;i<48;i++){for(j=0;j<48;j++){printf("%d",graph[i][j]);}printf("\n");}
