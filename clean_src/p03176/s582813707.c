...
//     if( l>=ql && r<=qr ) return seg[seg_num][x];
//     if( l>qr || r<ql ) return seg_def[seg_num]();
//     if( from>r || to<l ) return seg_def[seg_num]();

    return  S->op(
                seg_prod_inner(S,l,r,2*x+1,from,(from+to)/2),
                seg_prod_inner(S,l,r,2*x+2,(from+to)/2+1,to)
            );
    // return  seg_ope[seg_num](
    //             seg_query_inside(S,,qr,2*x+1,l,(l+r)/2),
    //             seg_query_inside(S,ql,qr,2*x+2,(l+r)/2+1,r)
    //         );
}
...
void seg_update(seg *S, int x, ll val){
    ...
    while(c>0){
        c--; c/=2;
        // printf("%lld %lld %lld  %lld %lld\n",c,2*(c+1),2*(c+1)-1,S->node[2*(c+1)],S->node[2*(c+1)-1]);
        S->node[c] = S->op(S->node[2*(c+1)], S->node[2*(c+1)-1]);
        // printf("%lld\n",S->node[c]);
        // seg[seg_num][c] = seg_ope[seg_num](seg[seg_num][2*(c+1)],
        // seg[seg_num][2*(c+1)-1]);
    }
}
...
// ll seg[3][351551],seg_size=1;


// void seg_make(int seg_num, int n, ll a[]){
//     while(seg_size<n) seg_size*=2;
//     ll t=seg_size;
//     rep(i,0,t){
//         if(i<n) seg[seg_num][t-1+i]=a[i]; 
//         else    seg[seg_num][t-1+i]=seg_def[0]();        
//     }
//     while(t/=2) 
//         rep(i,0,t)
//             seg[seg_num][t+i-1] = seg_ope[seg_num](seg[seg_num][2*(t+i)],
//             seg[seg_num][2*(t+i)-1]);
// }

// ll seg_query_inside(int seg_num, int ql, int qr, int x, int l, int r){
//     if( l>=ql && r<=qr ) return seg[seg_num][x];
//     if( l>qr || r<ql ) return seg_def[seg_num]();
//     return  seg_ope[seg_num](
//                 seg_query_inside(seg_num,ql,qr,2*x+1,l,(l+r)/2),
//                 seg_query_inside(seg_num,ql,qr,2*x+2,(l+r)/2+1,r)
//             );
// }
// ll seg_query(int seg_num, int l, int r){ 
//     return seg_query_inside(seg_num,l,r,0,0,seg_size-1);
// }

// void seg_update(int seg_num, int x, ll val){
//     ll c=seg_size-1+x;
//     seg[seg_num][c]=val;
//     while(c>0){
//         c--; c/=2;
...
void solve(){
    ...
    seg_op=seg_op_max;
    seg_e=seg_e_max;
    seg *s=seg_init(n+1,fl,seg_op,seg_e);
    // rep(i,0,s->_n){
    //     printf("%lld ",s->node[i]);
    // }PN;
    // PN;
    // rep(i,0,n+1){
    //     printf("%lld ",seg_prod(s,i,i)); PN;
    // }PN;
    // PN;
    // seg_def[0]=seg_def_max;
    // seg_ope[0]=seg_ope_max;

    // seg_make(0,n+1,fl);
    // YES;
    rep(i,0,n){
        cin(&a[i]);
        ll v=seg_prod(s,0,h[i]-1);
        // printf("%lld\n",v);
        // ll v=seg_query(0,0,h[i]-1);
        seg_update(s,h[i],v+a[i]);
        // seg_update(0,h[i],v+a[i]);
        // rep(j,0,s->_n){
        //     printf("%lld ",s->node[j]);
        // }PN;
        // rep(j,0,n+1){
        //     printf("%lld ",seg_prod(s,j,j));
        // }PN;
        // NO;
    }
    // printf("%d\n",s->_n);
    // rep(i,0,n+1){
    //     printf("%lld ",seg_prod(s,i,i));
    // }PN;
    // rep(i,0,s->_n){
    //     printf("%lld ",s->node[i]);
    // }PN;
