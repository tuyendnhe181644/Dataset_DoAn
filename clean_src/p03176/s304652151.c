...
ll seg_query_inside(ll ql, ll qr, ll x, ll l, ll r){
    return (l>=ql && r<=qr) ? seg[x] : (l>qr || r<ql) ? -INF : max2(seg_query_inside(ql,qr,2*x+1,l,(l+r)/2),seg_query_inside(ql,qr,2*x+2,(l+r)/2+1,r));
}

ll seg_query(int l, int r){
    return seg_query_inside(l,r,0,0,seg_t-1);
}
...
void solve(){
    ...
    rep(i,0,n){
        cin(&a[i]);
        // ll v=segmin(0,h[i]-1,0,0,seg_t-1);
        ll v=seg_query(0,h[i]-1);
        // printf("%lld\n",v);
        segupdate(h[i],v+a[i]);
    }
    printf("%lld\n",seg_query(0,n));
