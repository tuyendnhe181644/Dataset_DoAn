...
signed main(){
	...
	for(i = 0; i < M; i++){
		...
		u--;
		v--;
		set_edge_graph(u, v, a, g1);
		set_edge_graph(v, u, a, g1);
		set_edge_graph(u, v, b, g2);
		set_edge_graph(v, u, b, g2);
	}
	build_graph(g1);
	build_graph(g2);
	int *dis1 = Dijkstra(g1, s);
	int *dis2 = Dijkstra(g2, t);
	int *ans = (int *)malloc(sizeof(int) * (N + 1));
	ans[N] = 0;
	for(i = N - 1; i >= 0; i--){
		ans[i] = max(ans[i + 1], (int)(1e15) - (dis1[i] + dis2[i]));
	}
	for(i = 0; i < N; i++){
		printf("%lld\n", ans[i]);
	}
