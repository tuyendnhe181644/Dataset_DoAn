...
#define the_number_of_vertex 115115
#define the_number_of_edge 251154
typedef ll type_gr;
...
void solve(){
    ...
    rep(i,0,m){
        ...
        u[i]--;
        v[i]--;
        pushGraph(u[i],v[i],a[i],0);
    }
    dijkstra(s);
    rep(i,0,n) mon[i]+=ans[i];
    rep(i,0,n) edge_cnt[i]=0;
    rep(i,0,m){
        pushGraph(u[i],v[i],b[i],0);
    }
    dijkstra(t);
    rep(i,0,n) mon[i]+=ans[i];
    rep(i,0,n) set_insert(0,mon[i]);
    ll tot=POW(10,15);
    rep(i,0,n){
        printf("%lld\n",tot-set_min(0));
        set_erase(0,mon[i],0);
