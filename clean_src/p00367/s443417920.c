...
int cmp(T *a, T *b) { if (b->n - a->n) return b->n - a->n; return a->t - b->t; }
...
int main()
{
	...
	for (j = 0; j < 3; j++) {
		for (tp = tbl[j], i = 0; i < 1440; i++) if (a[j][i] > 0) {
			tp->b = a[j][i], tp->n = bitcount64(a[j][i]), tp->t = i, tp++;
		}
		sz[j] = tp - tbl[j];
...
/*
5
1:00 2:00   3:30 4:30    6:00 7:00
2:30 3:00   4:00 5:00    5:30 6:30
1:30 2:30   4:30 5:00    6:30 7:00
2:30 3:00   5:00 6:00    6:30 7:00
