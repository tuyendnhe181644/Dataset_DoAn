...
static uint64_t numList[60000];
static uint64_t numList_Size = 0;

static void addValToList(const char* buf) {
    numList[numList_Size] = (uint64_t)atol(buf);
    numList_Size++;
}

static bool contains753(const char* buf) {
    return strchr(buf, '7') != NULL && strchr(buf, '5') != NULL && strchr(buf, '3');
}

static void generateList(char * buf, const uint64_t pos, const uint64_t n) {
    buf[pos] = '7';
    buf[pos + 1] = '\0';
    if (contains753(buf))
        addValToList(buf);
    if (pos < n - 1)
        generateList(buf, pos + 1, n);
...
int main(void) {
    ...
    generateList(num, 0, 9);

    size_t cnt = 0;

    for (uint64_t i = 0; i < numList_Size; i++) {
        if (numList[i] <= N) {
            cnt++;
        }
    }
