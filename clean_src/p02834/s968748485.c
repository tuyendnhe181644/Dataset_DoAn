...
void dfs1(ll now, ll cur_d){
    d_u[now]=cur_d;
    if(now==v-1) return;
    rep(i,0,edge_cnt[now]){
        if(d_u[graph[now][i*2]]==-1){
            dfs1(graph[now][i*2],cur_d+1);
        }
    }
}
...
void dfs2(ll now, ll cur_d){
    d_v[now]=cur_d;
    rep(i,0,edge_cnt[now]){
        if(d_u[graph[now][i*2]]==-1) continue;
        if(d_v[graph[now][i*2]]!=-1) continue;
        if( d_u[graph[now][i*2]] > d_u[now] ){
            if( d_u[graph[now][i*2]] < cur_d+1 ) dfs2(graph[now][i*2],cur_d+1);
        }else dfs2(graph[now][i*2],cur_d+1);
    }
}
...
int main(void){
    ...
    rep(i,0,n-1){
        cin(&a);
        cin(&b);
        pushGraph(a-1,b-1,0,0,0);
    }
    rep(i,0,100005) d_u[i]=-1;
    rep(i,0,100005) d_v[i]=-1;
    dfs1(u-1,0);
    dfs2(v-1,0);
    // rep(i,0,n) printf("%d ",d_u[i]);
    // PN;
    // rep(i,0,n) printf("%d ",d_v[i]);
    // PN;
    ll piv=maxn(n,d_v);
    // rep(i,0,n){
    //     if(d_v[i]!=piv) continue;
    //     ans=max2(ans,  )
    // }
    printf("%lld\n",piv-1);
    return 0;
