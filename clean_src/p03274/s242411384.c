...
int main(void){
    ...
    rep(i,0,k+1){
        // printf("%lld\n",ans);
        if(k-i >plus || i>n-plus ) continue;
        if(!i){
            ans=min2(ans, x[n-plus-1+k] );
            continue;
        }
        if(i==k){
            ans=min2(ans, ABS(x[n-plus-k]) );
            continue;
        }
        
        ans=min3(ans, ABS(x[n-plus-i])*2+x[n-plus-1+(k-i)] , ABS(x[n-plus-i])+x[n-plus-1+(k-i)]*2 );
    }
