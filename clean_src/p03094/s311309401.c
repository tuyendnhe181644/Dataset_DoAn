...
int main(void){
  cfact();
  long long n,i,j,dp[4]={0},ndp[4],r=0,w,l;
  long long tp;
  scanf("%lld",&n);
  tp=modular_inverse(power(3,n-1));
  dp[1]=1;
  //colorful = i
  for(i=1;i<=n;i++){
    ndp[1]=(dp[2]+dp[3])%mod;
    ...
    w=dp[2];
    //i of n bothers are used
    w*=calcnCr(n,i);w%=mod;
    //printf("%lld\n",w);
    w*=tp;w%=mod;
    l=(i*modular_inverse(3*n))%mod;
    //l(length) bar divide into i pieces
    //Expected length of the smallest is l/(i*i)
    //https://math.stackexchange.com/questions/14190/average-length-of-the-longest-segment
    l*=modular_inverse((i*i)%mod);l%=mod;
    //printf("1 / %lld\n",3*n*i);
    r+=w*l;r%=mod;
  }
  printf("%lld\n",r);
