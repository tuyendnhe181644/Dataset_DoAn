...
int main(void){
    ...
    rep(i,0,n){
        rep(j,0,t){
            ...
            if(j-a[i]<0) continue;
            dp[i+1][j]=max2(dp[i+1][j], dp[i][j-a[i]]+b[i] );
        }
    }
    rrep(i,n-1,0){
        rep(j,0,t){
            dp2[i][j]=max2(dp2[i+1][j],dp2[i][j]);
            if(j-a[i]<0) continue;
            dp2[i][j]=max2(dp2[i][j], dp2[i+1][j-a[i]]+b[i] );
        }
    }
    // rep(i,0,t+1) printf("%2lld ",i);
    // PN;
    // rep(i,1,n+1){
    //     rep(j,0,t){
    //         printf("%2lld ",dp[i][j]);
    ...
    rep(i,0,n){
        ll tmp=0;
        rep(j,0,t){
            tmp=max2(tmp, dp[i][j]+dp2[1+i][t-1-j] );
        }
        ans=max2(tmp+b[i],ans);
    }
    printf("%lld\n",ans);
    return 0;
}
