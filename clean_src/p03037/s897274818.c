...
int main(void){
    ...
    In(N);In(M);
    long pass_count[N+2];
    long pass_count_ruiseki[N+2];
    format_array(N+1,pass_count,0);
    format_array(N+1,pass_count_ruiseki,0);
    for(loop1=0;loop1<M;loop1++){
        In(st);In(gl);
        /*for(loop2=st;loop2<=gl;loop2++){
            pass_count[loop2]++;
        }*///TLE
        pass_count[st]++;
        pass_count[gl+1]--;
    ...
    for(loop1=1;loop1<=N;loop1++){
        pass_count_ruiseki[loop1]=pass_count_ruiseki[loop1-1]+pass_count[loop1];
        //printf("%ld-%ld ",pass_count[loop1],pass_count_ruiseki[loop1]);
        /*if(pass_count[loop1]==M){
            ans++;
        }*///TLE
        if(pass_count_ruiseki[loop1]==M){
            ans++;
        }
    }
    Out(ans);
