...
void add_undirected_edge (graph * const g, const int32_t u, const int32_t v, const flow_type capa) {
  if (g->pointer == g->edge_length) {
    g->edge_length *= 2;
    g->edge = (flow_edge *) realloc (g->edge, sizeof (flow_edge) * g->edge_length);
  }
...
void run (void) {
  ...
  graph *g = new_graph (2 * h * w + h + w);
  i32 src = -1;
  i32 dst = -1;
  for (i32 i = 0; i < h * w; ++i) {
    if (a[i] == '.') continue;
    if (a[i] == 'o') {
      add_edge (g, i + h * w, i, 1);
    } else {
      add_edge (g, i + h * w, i, flow_inf);
    }
    if (a[i] == 'S') {
      src = i;
    } else if (a[i] == 'T') {
  ...
  for (i32 i = 0; i < h; ++i) {
    ...
    for (i32 j = 0; j < w; ++j) {
      i32 k = i * w + j;
      if (a[k] == '.') continue;
      add_edge (g, k, v, flow_inf);
      add_edge (g, v, h * w + k, flow_inf);
    }
  }
  for (i32 j = 0; j < w; ++j) {
    i32 v = 2 * h * w + h + j;
