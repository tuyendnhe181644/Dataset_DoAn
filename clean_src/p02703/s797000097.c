...
#define the_number_of_vertex 215115
#define the_number_of_edge 1101154
typedef ll type_gr;
int edge_cnt[the_number_of_vertex];
type_gr* gr[the_number_of_vertex];
type_gr ans[the_number_of_vertex];
bool is_conf[the_number_of_vertex];
// freeeeeeeeeeeeeeeeeeeeeeeeeeee!!!!!!!!!!!!

void pushGraph(ll v1, ll v2, type_gr weight , ll is_dir){
...
void graph_pushHeap(type_gr val, ll len , ll kind){
...
void dijkstra(ll start){
    rep(i,0,the_number_of_vertex){  ans[i] = INF;  is_conf[i] = false;  }
    ans[start] = 0;
    is_conf[start]  = true;
    int size=0;
    rep(i,0,edge_cnt[start]){
        ...
        graph_pushHeap( gr[start][i*2+1] , size++ , 0 );
    }
    ll piv;
    while(size){
        ...
        rep(i,0,edge_cnt[piv]){
            if( ans[ (ll) gr[ heap[0].to ][i*2] ] > ans[ heap[0].to ] + gr[ heap[0].to ][i*2+1] ){
                ...
                graph_pushHeap( ans[ heap[0].to ] + gr[ heap[0].to ][i*2+1] , size++ , 0 );
            }
        }
...
void solve(){
    ...
    rep(i,0,m){
        ...
        rep(j,0,2500+1){
            if(j>=a){
                pushGraph(u*3000+j,v*3000+j-a,b,1);
                pushGraph(v*3000+j,u*3000+j-a,b,1);
            }
        }
    }
    ll c,d;
    rep(i,0,n){
        ...
        cin(&d);
        rep(j,0,2500+1){
            pushGraph(i*3000+j,i*3000+ min2(2500,j+c) ,d,1);
        }
    }
    dijkstra(0*3000+ min2(s,2500) );
    rep(i,1,n){
        ll tot=INF;
        rep(j,0,2500+1){
            tot=min2(tot,ans[ i*3000+j ]);
        }
        printf("%lld\n",tot);
    }
}
