...
できる必要条件：石の数の合計がN*(N+1)/2の倍数(1回の操作で取り除く石の数の倍数)
十分条件ではない：反例 {4, 1, 1}
...
a, b, c, d の差が整数でない->a, b, c, dは整数なのでありえない！
