...
void factor_pre(){ 
    ...
    // factors[fac_cnt++][3]=5;
    // factors[fac_cnt++][3]=8;
    // factors = realloc( factors , sizeof(ll)*2 );  
    // // factors[1] = realloc( factors[1] , sizeof(ll)*1 );  
    // factors[fac_cnt][0]=3; 
    // factors[fac_cnt][1]=4; 
    // factors[fac_cnt++][2]=5; 
    // factors[fac_cnt][0]=1; 
    ...
    // factors[fac_cnt++][4]=7; 
    // factors[fac_cnt++][5]=8; 
    repp(i,3,1e6+1,2) rep(j,0,fac_cnt) if( !(i%factors[0][j]) ){break;} else if(factors[0][j]*factors[0][j]>i){
...
ll factor(ll n, ll is_update){
    rep(i,0,fac_cnt){
        if(n<factors[0][i]) break;
        ll cnt=0; rep(j,0,1){ while( ( cnt+= n %factors[0][i]==0 ? 1 : 0 )  && (n/=factors[0][i]) %factors[0][i]==0 ) continue;} factors[1][i]= is_update ? cnt : max2(factors[1][i],cnt); 
    }
    return n>1?1:0;
}
