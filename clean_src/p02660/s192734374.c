...
ll judge_prime(ll n){
    ...
    ll a=sqrt(n); if(a%2==0) a--;
    repp(i,3,a+1,2){if(n%i==0) return 0;}
    return 1;
}

ll *factors[2] , fac_cnt=0;
ll factor_pre(){  factors[0] = realloc( factors[0] , sizeof(ll) ); factors[1] = realloc( factors[1] , sizeof(ll) );  factors[0][fac_cnt]=2; factors[1][fac_cnt++]=0; repp(i,3,1e6+1,2) rep(j,0,fac_cnt) if( !(i%factors[0][j]) ){break;} else if(factors[0][j]*factors[0][j]>i){  factors[0] = realloc( factors[0] , sizeof(ll)*( fac_cnt +1 ) );  factors[1] = realloc( factors[1] , sizeof(ll)*( fac_cnt +1 ) );  factors[0][fac_cnt]=i;  factors[1][fac_cnt++]=0;  break; }  return 0; }
ll factor(ll n, ll is_update){ rep(i,0,fac_cnt){ ll cnt=0; rep(j,0,1){ while( ( cnt+= n %factors[0][i]==0 ? 1 : 0 )  && (n/=factors[0][i]) %factors[0][i]==0 ) continue;} factors[1][i]= is_update ? cnt : max2(factors[1][i],cnt);  }   return n>1?1:0; }
