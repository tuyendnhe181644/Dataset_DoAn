...
// ll factor_pre(ll x){  factors[0] = realloc( factors[0] , sizeof(ll) ); factors[1] = realloc( factors[1] , sizeof(ll) );  factors[0][fac_cnt]=2; factors[1][fac_cnt++]=0; repp(i,3,x+1,2) rep(j,0,fac_cnt) if( !(i%factors[0][j]) ){break;} else if(factors[0][j]*factors[0][j]>i){  factors[0] = realloc( factors[0] , sizeof(ll)*( fac_cnt +1 ) );  factors[1] = realloc( factors[1] , sizeof(ll)*( fac_cnt +1 ) );  factors[0][fac_cnt]=i;  factors[1][fac_cnt++]=0;  break; }  return 0; }
...
ll *factors[2] , fac_cnt=0;
ll factor_pre(ll x){  
    // factors[0] = realloc( factors[0] , sizeof(ll) ); 
    // factors[1] = realloc( factors[1] , sizeof(ll) );  
    // factors[0][fac_cnt]=2; factors[1][fac_cnt++]=0; 
    rep(i,0,2){
        factors[i] = realloc( factors[i] , sizeof(ll) ); 
        factors[i][i?fac_cnt++:fac_cnt]=i?0:2; 
    }
    repp(i,3,x+1,2) rep(j,0,fac_cnt) if( !(i%factors[0][j]) ){break;} else if(POW(factors[0][j],2)>i){ 
        // rep(i,0,2){
        // factors[0] = realloc( factors[0] , sizeof(ll)*( fac_cnt +1 ) );  

        // } 
        factors[0] = realloc( factors[0] , sizeof(ll)*( fac_cnt +1 ) );  
        factors[1] = realloc( factors[1] , sizeof(ll)*( fac_cnt +1 ) );  
        factors[0][fac_cnt]=i;  factors[1][fac_cnt++]=0;  break; }  return 0; }
ll factor(ll n, ll is_update){ rep(i,0,fac_cnt){ ll cnt=0; rep(j,0,1){ while( ( cnt+= n %factors[0][i]==0 ? 1 : 0 )  && (n/=factors[0][i]) %factors[0][i]==0 ) continue;} factors[1][i]= is_update ? cnt : max2(factors[1][i],cnt);  if( factors[1][i]*factors[1][i] > n ) break; }    return n; }
