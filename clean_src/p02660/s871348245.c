...
static uint64_t pf[100];
static uint64_t pfC[100];
static uint64_t pfList_Size = 0;
...
int main(void) {
    ...
    uint64_t cpf = 2;
    uint64_t lim = (uint64_t)sqrt((double)N) + 1;
    while (1 < N && cpf <= lim) {
        while (N % cpf == 0) {
            if (pfList_Size == 0 || cpf != pf[pfList_Size - 1]) {
                pf[pfList_Size] = cpf;
                pfC[pfList_Size] = 1;
    ...
    uint64_t r = 0;
    for (uint64_t i = 0; i < pfList_Size; i++) {
        uint64_t p = 1;
