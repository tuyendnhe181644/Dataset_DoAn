...
int main(void){
  ...
  long long bp,bq;
  long long ***dp=malloc(sizeof(long long**)*5005);
  for(i=0;i<5005;i++){
    dp[i]=malloc(sizeof(long long*)*5005);
    for(j=0;j<5005;j++){
      dp[i][j]=malloc(sizeof(long long)*2);
      dp[i][j][0]=0;
      dp[i][j][1]=0;
    }
  }
  long long ndp[5005][2]={0};
  long long wei[524288]={0};
  rs g[524288];
  mkj x[524288];
  sd dat[524288];
  long long f[8192]={0};
        ...
        for(q=1;q<=wei[g[j].fi];q++){
          for(bp=0;bp<2;bp++){
            for(bq=0;bq<2;bq++){
              ...
              k*=f[q];k%=mod;
              k*=dp[w][p][bp];k%=mod;
              ndp[p][t]=(ndp[p][t]+k)%mod;
              t=(bp+bq)%2;
              k=dp[g[j].fi][q][bq];
              k*=dp[w][p][bp];k%=mod;
              ndp[p+q][t]=(ndp[p+q][t]+k)%mod;
            }
          }
        }
      }
  ...
  for(i=2;i<=n;i+=2){
    r=(r+dp[1][i][0]*f[i])%mod;
    r=(r+mod-((dp[1][i][1]*f[i])%mod))%mod;
  }
  printf("%lld\n",r);
  return 0;
}
