		...
		for (j = 0; j <= num_elements; j++) {
			for (ki = 0; ki < 2; ki++) {
				...
				if (j > 0) {
					if (j == 1) {
						if (ki) {
							dp[idx][j][ki] += (dp[idx - 1][j][0] + dp[idx - 1][j][1]);
						}
					} else {
						dp[idx][j][ki] += dp[idx - 1][j][ki];
					}

					dp[idx][j][ki] += dp[idx - 1][j - 1][ki];
				}
				if (j < num_elements) {
					dp[idx][j][ki] += dp[idx - 1][j][ki];
					...
					if (j == 0) {
						if (ki) {
							dp[idx][j][ki] += (dp[idx - 1][j + 1][0] + dp[idx - 1][j + 1][1]);
						}
					} else {
						dp[idx][j][ki] += dp[idx - 1][j + 1][ki];
					}
