...
static size_t comb(const size_t num_elements, const size_t r) {
    size_t result = 1;

    for (size_t idx = 0; idx < r; idx++) {
        result *= num_elements - idx;
        result /= idx + 1;
    }
...
static uint64_t combU64(const uint64_t num_elements, const uint64_t r) {
    uint64_t result = 1;

    for (uint64_t idx = 0; idx < r; idx++) {
        result *= num_elements - idx;
        result /= idx + 1;
    }
...
static size_t gcdZu(size_t m, size_t num_elements) {
    size_t temp;

    while (m % num_elements != 0) {
        temp = num_elements;
...
static uint64_t gcdU64(uint64_t m, uint64_t num_elements)
{
    uint64_t temp;
...
static void bHeap_UpHeap(const size_t nodeIdx) {
    ...
    if (0 < compareResult) {
        const BHEAP_VAL_TYPE temp_val = bHeap_Node[parentIdx].val;
        bHeap_Node[parentIdx].val = bHeap_Node[nodeIdx].val;
        bHeap_Node[nodeIdx].val = temp_val;
...
static void bHeap_DownHeap(const size_t nodeIdx) {
    ...
    if (leftIdx < bHeap_Info.size) {
        ...
        if (swapTarget != BHEAP_INVALID_NODE) {
            const BHEAP_VAL_TYPE temp_val = bHeap_Node[nodeIdx].val;
            bHeap_Node[nodeIdx].val = bHeap_Node[swapTarget].val;
            bHeap_Node[swapTarget].val = temp_val;
...
static void tree_Init(const uint64_t N) {
    for (uint64_t idx = 0; idx < N; idx++) {
        tree_Node[idx].cnt = 0;
        tree_Node[idx].numChild = 0;
        tree_Node[idx].child = malloc(sizeof(size_t) * BREE_INIT_ALLOC);
        tree_Node[idx].maxNumChild = BREE_INIT_ALLOC;
    }
}
