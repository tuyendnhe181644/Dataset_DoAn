...
int getvalue_from_str(const char* input,int start,int end) {
	...
	if(isdigit(input[start])) {
		...
		for(idx=start;idx<end;idx++) {
			if(!isdigit(input[idx]))return -1;
			result=result*10+(input[idx]-'0');
		}
	} else {
		if(input[start+1]!='[')return -1;
		kakko_nest=1;
		for(idx=start+2;idx<end;idx++) {
			if(input[idx]=='[')kakko_nest++;
			if(input[idx]==']')kakko_nest--;
			if(kakko_nest<=0)break;
		}
		if(kakko_nest>0)return -1;
		result=getvalue_from_str(input,start+2,idx);
