...
ull mergeteq (ull a[], ull num_elements) {
	ull l = num_elements / 2, r = num_elements - l;
	if (num_elements == 1) return 0;
	ull ret = mergeteq(a, l) + mergeteq(a + l, r);

	ull temp_val[num_elements];
	sll idx = 0, j = 0;
	while (idx < l || j < r) {
		if (idx == l) {
			temp_val[idx + j] = a[idx + j];
			j++;
			continue;
		}
		if (j == r) {
...
ull solve () {
	...
	for (idx = 0; idx < h; idx++) {
		for (j = 0; j < w; j++) {
			if (s[idx][j] == '#') {
				if (idx && s[idx - 1][j] == '#') continue;
				if (j && s[idx][j - 1] == '#') continue;
				if (idx < h - 1 && s[idx + 1][j] == '#') continue;
				if (j < w - 1 && s[idx][j + 1] == '#') continue;
				goto fail;
			}
		}
	}
