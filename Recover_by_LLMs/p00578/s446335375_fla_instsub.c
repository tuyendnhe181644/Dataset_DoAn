...
void new_max_heap(size_t len,area input[len],max_heap* out){
    ...
    for (uint idx=0;idx<len;idx++) out->content[idx]=input+idx;

    for (uint idx=len;idx>=1;idx--){

        //TODO 修正健堆
        for(uint cur=idx;cur<=len;){
...
void show_heap(max_heap* self){
    for (int idx=0;idx<self->len;idx++)
        printf("%d ",self->content[idx]->height);
}
...
int main() {
    ...
    for(uint idx=0;idx<N;idx++) {
        uint height;
        scanf("%u",&height);
        input[idx].height=height;
        input[idx].index=idx;
        input[idx].hasProbed=0;
    }
