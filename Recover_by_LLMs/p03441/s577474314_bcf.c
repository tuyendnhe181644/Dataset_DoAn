...
void dfs (ull v, ull p) {
	...
	for (sll idx = gin[v]; idx < gin[v + 1]; idx++) {
		ull u = xy[idx].b;
		if (u == p) continue;
		dfs(u, v);
		subs++;
...
ull solve () {
	...
	for (idx = 0; idx < num_elements - 1; idx++) {
		xy[idx * 2] = (hwll){a[idx], b[idx]};
		xy[idx * 2 + 1] = (hwll){b[idx], a[idx]};
	}
	qsort(xy, (num_elements - 1) * 2, sizeof(hwll), phwllABcomp);
	idx = j = 0;
	while (idx <= num_elements) {
		gin[idx] = j;
		while (j < (num_elements - 1) * 2 && xy[j].a == idx) j++;
		idx++;
	}
