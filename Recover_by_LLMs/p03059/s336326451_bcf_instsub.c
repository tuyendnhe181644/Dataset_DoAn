...
void shuffledget(int x[],int num_elements){
    srand(time(0));
    int idx,b[524288],p,c;
    for(idx=0;idx<num_elements;idx++){
        b[idx]=idx;
    }
    for(idx=num_elements;idx>=1;idx--){
        p=rand()%idx;
        c=b[idx-1];b[idx-1]=b[p];b[p]=c;
    }
    for(idx=0;idx<num_elements;idx++){
        scanf("%d",&x[b[idx]]);
    }
...
int search(int x,int a[],int num_elements){
    int st=0,fi=num_elements-1,te;
    while(st<=fi){
        te=(st+fi)/2;
...
void prarr(int arr[],int num_elements){
  int idx;
  for(idx=0;idx<num_elements;idx++){
    if(idx){printf(" ");}
    printf("%d",arr[idx]);
...
int main(void){
  int idx,j,num_elements,m,k,a,b,c,h,w,r=0,l,t;
  scanf("%d%d%d",&a,&b,&t);
  printf("%d\n",b*(t/a));
  return 0;
}
