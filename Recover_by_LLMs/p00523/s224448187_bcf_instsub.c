...
double POW_d(double a, double b){double c=1;rep(idx,0,b) c*=a;return c;}
...
int main(void){
    ...
    cin(&num_elements);
    // scanf("%s",);
    ll a[2*num_elements],total[2*num_elements+1];
    total[0]=0;
    rep(idx,0,num_elements){
        cin(&a[idx]);
        a[idx+num_elements]=a[idx];
        total[idx+1]=total[idx]+a[idx];
    }
    rep(idx,0,num_elements){
        total[num_elements+1+idx]=total[num_elements+idx]+a[idx];
    }
    ll mark=total[num_elements]/3;
    ll head=0,tail=0,siz=a[0];
    // rep(idx,0,num_elements+1) printf("%lld ",total[idx]);
    // PN;
    while(head<=num_elements-1){
        ...
        ll piv = lower_bound(total,0,2*num_elements, total[tail+1]+siz , 2 );
        // printf("%lld ",head);
        // printf("%lld ",siz);
        // printf("%lld ",piv);
        // PN;
        if(piv==-1){
            if(head+1!=num_elements){
                siz-=a[head++];
                if(!siz) continue;
                goto NEXT;
            }else{
        ...
        ll b,c;
        b=total[piv]-total[tail+1];
        c=total[num_elements]-b-siz;

        // printf("%lld   %lld %lld\n",siz,b,c);
        // printf("%lld   %lld %lld\n",siz,head,tail);
        if(b>=siz&&c>=siz){
            result=max2(result,siz);
        }else{
            if(head+1!=num_elements){
                siz-=a[head++];
                goto NEXT;
