...
long factorial(long num_elements);
long fibonacci(long num_elements);
...
long ncr(long num_elements , long r);
long npr(long num_elements , long r);
long nhr(long num_elements , long r);
 
long loop1,loop2,loop3,loop4,loop5,i_temp;
char c_temp;
 
int main(void){
    I(num_elements);I(k);I(x);I(y);
    long result=0;
    if(num_elements>k){
        result=x*k+y*(num_elements-k);
    }else{
        result=x*num_elements;
    }
    printf("%ld\n",result);
    END;
}
...
long factorial(long num_elements){//num_elements!のMOD10^9+7を返すやつ
    unsigned long long int ret=1;
    for(long idx=1;idx<=num_elements;idx++)ret=(ret*idx)%1000000007;
    return (long)ret;
}
...
long fibonacci(long num_elements){
    switch(num_elements){
        case 0:return 0;
        case 1:return 1;
        default :return fibonacci(num_elements-1)+fibonacci(num_elements-2);
    }
}
...
long ncr(long num_elements , long r){
    ...
    return pascal[num_elements][r];
}
long npr(long num_elements, long r){
    return ncr(num_elements,r)*factorial(r);
}
long nhr(long num_elements , long r){
    return ncr(num_elements+r-1,r);
}
