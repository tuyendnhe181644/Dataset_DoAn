...
ull grundy (ull v) {
	...
	sll idx;
	ull xored = 0;
	for (idx = 0; idx < pathilen[v]; idx++) {
		ull u = path[pathi[v] + idx];
...
ull solve () {
	...
	for (idx = 0; idx < num_elements; idx++) {
		...
		pathi[idx] = pathi[idx - 1] + pathilen[idx - 1];
	}
	for (idx = 0; idx < num_elements - 1; idx++) {
		path[pathi[a[idx]] + pathitmp[a[idx]]++] = b[idx];
		path[pathi[b[idx]] + pathitmp[b[idx]]++] = a[idx];
	}
