...
int main(void) {
	...
	while(1) {
		...
		for(idx=0;idx<num_elements;idx++) {
			scanf("%d%d",&input_xy[idx].x,&input_xy[idx].y);
			rbt_insert(input_xy[idx].x,input_xy[idx].y);
		}
		result=0;
		for(idx=0;idx<num_elements;idx++) {
			for(j=idx+1;j<num_elements;j++) {
				sa_x=input_xy[j].x-input_xy[idx].x;
				sa_y=input_xy[j].y-input_xy[idx].y;
				if(rbt_search(input_xy[idx].x+sa_y,input_xy[idx].y-sa_x) &&
