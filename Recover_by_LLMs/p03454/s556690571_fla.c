...
void dijk_init (ull num_elements, struct_b arr[]) {
	edge x;
	for (int32_t idx = 0; idx < num_elements; idx++) {
		x.to = arr[idx].to;
		x.cost = arr[idx].cost;
		g[arr[idx].from].push_back(x);
	}
}
...
bool dijkstra (int s, sll distance[]) {
	...
	while (!que.empty()) {
		...
		for (int32_t idx = 0; idx < maxsize; idx++) {
			edge e = g[v][idx];
...
ull solve () {
	...
	for (idx = 1; idx <= 15; idx++) {
		p10[idx] = p10[idx - 1] * 10;
	}

	result = 0;

	for (idx = 1; idx * idx <= num_elements; idx++) {
		if (num_elements % idx == 0) {
			ull x;

			result = (result + singleContained(idx)) % MOD;
			// printf("\t%lld: %llu\n", idx, result);
			if (idx * idx != num_elements) {
				result = (result + singleContained(num_elements / idx)) % MOD;
				// printf("\t%lld: %llu\n", num_elements / idx, result);
			}
		}
	}
	// printf("%llu...\n", result);
	...
	for (idx = 1; idx <= fullfillable; idx++) {
		ull medfilled;
		medfilled = 0;
		for (j = idx; j <= fullfillable; j++) {
			// printf("%lld/%lld\n", idx, j);
			medfilled += p10[j - 1] * j * 9;
			// printf("%lld/%lld: %llu\n", idx, j, medfilled);

			if (medfilled > num_elements) {
				break;
			}
			ull remain = num_elements - medfilled;
	...
	while (idx <= num_elements) {
		if (num_elements % idx) {
			ull right = num_elements % idx;
			ull left = idx - right;
			ull x = num_elements / idx;
...
int32_t main (void) {
	...
	// }
	// for (idx = 0; idx < m; idx++) {
	// 	scanf("%llu%llu", &arr[idx].a, &arr[idx].b);
	// 	arr[idx].a--;
	// 	arr[idx].b--;
	// }
	// for (idx = 0; idx < k; idx++) {
	// 	scanf("%llu%llu", &brr[idx].a, &brr[idx].b);
	// 	brr[idx].a--;
	// 	brr[idx].b--;
	// }
	// for (idx = 0; idx < m; idx++) {
	...
	// }
	// for (idx = 0; idx < num_elements; idx++) {
	// 	scanf("%llu%llu%llu", &a[idx], &b[idx], &c[idx]);
	// }
	...
	// while (scanf("%llu%llu", &num_elements, &k), num_elements + k) {
	// 	for (idx = 0; idx < num_elements; idx++) {
	// 		scanf("%llu", &a[idx]);
	// 	}
	// 	solve();
