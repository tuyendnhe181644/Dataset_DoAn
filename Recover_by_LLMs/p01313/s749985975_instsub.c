...
int main()
{
	...
	for (x = 1, idx = 0; idx <= 16; idx++, x *= 10) tens[idx] = x;
	...
	while (cno--) {
		...
		qsort(result, sz, sizeof(T), cmp);
		if (!sz) puts("No cats.");
		else for (idx = 0; idx < sz; idx++) printf("%lld %lld\n", result[idx].A, result[idx].B);
	}
	return 0;
}