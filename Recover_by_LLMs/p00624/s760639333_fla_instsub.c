...
int insert(uchar *pos)
{
	...
	x = 0; for (idx = 0; idx < 7; idx++) x = (x << 8) | pos[idx];
		...
		for (idx = 0; idx < 4; idx++) {
			...
			if (ic < 7) {  // 移動先がコンテナ
				...
				while (1) {
					...
					if (j < 7) break;
					cr += mv[idx][0], cc += mv[idx][1], cp0 = cp, cp = (cr << 4) | cc; // 移動し続ける
					if (map[cr][cc] != '.') break;
				}
